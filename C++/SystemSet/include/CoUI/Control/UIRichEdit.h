...
class CTxtWinHost;
#define COUI_REST_CTRLC 0x01//RichEditShorcut
...
public:
	...
	//配置禁用快捷键(默认不禁用任何快捷键)
	DWORD ConfigDisabledShortcutKey(DWORD ShortCut,bool bAdd = false);//bAdd为true表示增加，为false表示去除
	DWORD GetDisabledShortcutKey() const;
	...
	WCHAR SetPasswordChar(WCHAR ch_PasswordChar);//设置密码掩码符号，设置(L'\0')则明文显示
protected:
    CTxtWinHost* m_pTwh;
