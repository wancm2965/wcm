...
//全局手势分析标记
#define UI_ANALYSIS_GESTURE_NONE	0//不分析任何手势
#define UI_ANALYSIS_GESTURE_MOUSE	1//仅分析鼠标手势
#define UI_ANALYSIS_GESTURE_TOUCH	2//仅分析触摸手势
#define UI_ANALYSIS_GESTURE_ALL (UI_ANALYSIS_GESTURE_MOUSE | UI_ANALYSIS_GESTURE_TOUCH)//分析所有手势--默认
...
typedef enum EDialogResType{
	DRT_FOLDER_FILE,//解析"自定义资源目录"下的"指定文件"(文件用文件名标识,下同)
	DRT_ZIP_EXTERNAL,//模块外置的zip资源
	DRT_ZIP_INTERNAL,//模块内置的zip资源(zip内置到可执行文件[dll或exe]中)
	DRT_GLOBAL_RES//全局资源[可为外置zip资源，也可为某资源模块内置的zip资源]
};
...
public:
	...
	bool EnableBlur(bool bEnable, HRGN hRgn = NULL, bool bTransitionOnMaximized = false);// 关闭模糊同时也会关闭背景透明
	bool IsLayerWnd() const;//是否为分层窗口
	bool SetLayerWnd(bool bLayer);//设置分层属性并托管绘制
	void SetBGTransparent(bool bTrans);	// 背景透明,关闭背景透明同时也会关闭背景模糊
	void SetBackgroundTransparent(bool bTrans);	// 静态背景透明，即绘制时绘制所在区域背景
	...
	void SetColorAdjust(bool bColorAdjust);
	bool IsColorAdjust() const;
	...
	//配置全局手势分析标识
	static void ConfigGlobalGestureAnalysis(DWORD  dwFlags);
	static DWORD GetGlobalGestureAnalysisMark();
...
private:
	...
	bool m_bUseAbsolutePath;	// 图片路径是否使用绝对路径
	bool m_bLayerWnd;//是否是分层窗口
	bool m_bColorAdjust;//是否开启颜色矫正，若此处开启，则其关联控件都开启，可以避免颜色合成溢出时导致色彩失真。（暂时在64位系统上出现此问题）
	//
	...
	static bool m_gbColorAdjust;//颜色矫正，全局量
	//全局手势分析控制标识
	static DWORD m_dwGlobalGestureAnalysis;

private:
	EDialogResType m_resType;//使用资源类型
	HINSTANCE m_hResInstance;//局部资源实例句柄
	unsigned int m_nResID;
	CStdString m_pStrResPath;//局部资源路径(或zip文件完整路径)
	//全局公共资源目录列表[当获取资源失败时，到这些公共资源目录处获取资源]
	//备注：由于原静态全局路径已被广泛使用，难于修改，故增加此路径
...
public:
	...
	//设置局部资源路径（如果是普通资源，则参数应为资源所在的文件夹，
	//如果是外置的zip资源，则参数应为zip资源的完整文件夹）
	...
	static bool SetgColorAdjust(bool bColorAdjust);//设置全局颜色矫正,返回原来的状态。
