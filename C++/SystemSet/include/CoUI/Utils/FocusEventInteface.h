...
namespace CoUI
{
	enum EditListenState{
		EDIT_LISTENING_NO = 0,//不监听
		EDIT_LISTENING_TOUCH,//监听触摸
		EDIT_LISTENING_FULL//监听所有
	};
	//开启(关闭)Edit焦点监听
	void UILIB_API SetEditFocusListeningState(EditListenState eState);
	//获取当前Edit焦点监听状态（是否在监听）
	EditListenState UILIB_API GetEditFocusListeningState();
