...
//{{gaoshizhong 20120208 Add
int get_bfcp_msg_len(struct mbuf *mb);
//}}gaoshizhong 20120208 Add
...
//{{gaoshizh add 20120215

struct bfcp_conn * bfcp_estab_conn_add(struct bfcp_sock *bs,
				  const struct sa *paddr,struct tcp_conn *tcp);

int bfcp_connect(struct bfcp_sock **sockp, enum bfcp_transp transp,struct tls *tls,
				 const struct sa * peer,bfcp_msg_h *msgh,struct tcp_conn **tcp, 
				 void *eh, void *rh, void *ch, void *arg);

void destructor_bfcp_conn(const struct bfcp_sock *bs);
