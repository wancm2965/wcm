...
#define VIDEC_NETWORK_3G				4			//3G
#define VIDEC_NETWORK_CDMA				VIDEC_NETWORK_3G
#define VIDEC_NETWORK_QUALITY			0x80000000	//是否质量优先
...
public:
	...
	static VIDEC_Image*CreateAutoFree(unsigned char*pData,int nLen,int nWidth,int nHeight,int nFrameRate,int nHeaderOffset);
};
...
public:
	...
	virtual void OnReceivedPreSend(int& nSend)=0;
...
#define VIDEC_OSD_ALIGN_TOP_CENTER_DOWN		6	//靠上中间偏下
#define VIDEC_OSD_ALIGN_BOTTOM_CENTER_UP	7	//靠下中间偏上
...
public:
	...
	virtual int HasVideoSignal(void){return 1;};
};


class VIDEC_WndCallback
{
public:
	VIDEC_WndCallback(void){};
	virtual~VIDEC_WndCallback(void){};
public:
...
public:
	...
	//成功返回0，失败返回-1
	virtual int Open(HWND hWndParent, bool bLocal = false, unsigned long*lpnVidFrameTimestamp = NULL, unsigned long*lpnAudFrameTimestamp = NULL, char* pBgBmp = NULL, int bgcolr = -1, int bgcolg = -1, int bgcolb = -1) = 0;
	//关闭显示窗口
	virtual void Close(void)=0;
	...
	static VIDEC_Wnd*Create(VIDEC_WndCallback&rCallback);
...
#define VIDEC_IMAGE_DISPLAY_MODE_MANUAL	1
	...
	*****************************************************************************/
	...
	static VIDEC_Wnd*CreateLocalVideo(VIDEC_WndCallback&rCallback);

	virtual void SetEnableVideo(int nEnable)=0;
	virtual int GetEnableVideo(void)=0;

	//{{20141031 新增图像处理接口,在调用下面接口前，需要先调用SupportPreProcess打开图像处理功能，打开此功能后，包括上面的 SetVideoParam、SetVideoGammaRGB两个接口也可以调用
	virtual void SupportPreProcess(void)=0;
	virtual void EnableOSD(bool bEnable)=0;
	virtual bool IsOSDEnable(void)=0;
	virtual void SetOSDText(const char*cszText,int nAlign,int nTop,int nLeft)=0;
	virtual void SetOSDFont(LOGFONTA fontOSD,COLORREF crText,COLORREF crBk)=0;
	virtual void SetDenoise(int nDenoise)=0;
	virtual int GetDenoise(void)=0;	
	//}}End 20141031
...
public:
	...
	//成功返回0，失败返回-1
	virtual int Open(int nBitrate,int nBitrateControlType,VIDEC_CODEC_TYPE CodecType,int nScaleType,int nWidth=0,int nHeight=0)=0;
	//关闭采集通道压缩通道
	virtual void Close(void)=0;
	//重设置压缩通道
	//nQuality,图象质量，0:32,1：64,2:128,3:256,4:384,5:512,6:768,7:1024,8:1536,9:2048,10:4096:11:6144
	...
	//nScaleType:缩放类型，0:same,1:1/4,2:1/16
	//成功返回0，失败返回-1
	virtual int Reset(int nBitrate,int nBitrateControlType,VIDEC_CODEC_TYPE CodecType,int nScaleType,int nWidth=0,int nHeight=0)=0;
	//请求关键帧
	//成功返回0，失败返回-1
	virtual int RequestKeyFrame(void)=0;
	virtual void SetSkipFrame(bool bSkip)=0;
...
public:
	...
	virtual int SetBitrate(int nBitrate)=0;
	virtual int Open(int nPayloadType,int nWidth,int nHeight,int nFrameRate,int nBitrate,int nBitrateControlType,VIDEC_CODEC_TYPE CodecType)=0;
};
