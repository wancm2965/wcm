...
XEncoderFrameH263::XEncoderFrameH263(MPVideo_EncoderCallback&rCallback,bool bRTPPacket)
...
int XEncoderFrameH263::Open(int nWidth,int nHeight,int nFrameRate,bool bDoubleField,int nBitrate,int nMinQP,int nMaxQP,int nBitrateControlType)
{
	...
	if (nBitrateControlType==MPVIDEO_NETWORK_PRIVATE ||
		nBitrateControlType==MPVIDEO_NETWORK_SATELLITE ||
		nBitrateControlType==MPVIDEO_NETWORK_3G)
	{
		nMinQP=2;
	}
	...
	if (m_bRTPPacket)
	{
		m_pRTPPacketBuffer=(unsigned char*)malloc(MAX_PAYLOAD_LEN*2);
...
int XEncoderFrameH263::DoEncodeFrame(unsigned char*pData[],int nStride[],unsigned char*pPacketData,int&nPacketLen,int&nKeyFrame,int&nBFrame)
{
	...
	int nQuant=m_nMinQP;
	if (m_nBitrateControlType!=MPVIDEO_NETWORK_LAN)
	{
