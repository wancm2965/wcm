...
XEncoderFrameH263P::XEncoderFrameH263P(MPVideo_EncoderCallback&rCallback,bool bRTPPacket)
...
int XEncoderFrameH263P::Open(int nWidth,int nHeight,int nFrameRate,bool bDoubleField,int nBitrate,int nMinQP,int nMaxQP,int nBitrateControlType)
{
	...
	if (m_bRTPPacket)
	{
		m_pH263PRTPFrame=new H263PRTPFrame(*this,(nBitrate>0?nBitrate:6144));
