...
void ScreenInput::AddMPScreen(MPScreen*pMPScreen)
{
	int nFrameRate=1;
	{
		XAutoLock l(m_csMPScreen);
		if (m_listMPScreen.find(pMPScreen)==m_listMPScreen.end())
		{
			m_listMPScreen.push_back(pMPScreen);
		}

		int nWidth=320;
		int nHeight=240;
		XListPtr::iterator iter=m_listMPScreen.begin();
		while (iter!=m_listMPScreen.end())
		{
			MPScreen*pMPScreenTemp=(MPScreen*)(*iter);
			if (pMPScreenTemp->GetWidth()>nWidth)
			{
				nWidth=pMPScreenTemp->GetWidth();
				nHeight=pMPScreenTemp->GetHeight();
			}
			if (pMPScreenTemp->GetFrameRate()>nFrameRate)
			{
				nFrameRate=pMPScreenTemp->GetFrameRate();
			}
			++iter;
		}
	...
	FrameRateCtrl::Open(nFrameRate);
	FrameRateCtrl::SetFrameRate(nFrameRate);
}

void ScreenInput::RemoveMPScreen(MPScreen*pMPScreen)
{
	int nFrameRate=1;
	{
		XAutoLock l(m_csMPScreen);
		m_listMPScreen.remove(pMPScreen);
...
MPVideo_Image*ScreenInput::GetImage(int nForce,int nFrameRate)
{
	...
	if (bGotImage)
	{
		pXImage=new XImage(m_pImageBuffer,m_nImageSize);
		pXImage->AddRef();
		m_pImageBuffer=NULL;
...
void ScreenInput::OnFrameRate(void)
{
	XAutoLock l(m_csMPScreen);
	if (m_listMPScreen.size()==0)
	{
	...
	MPVideo_Image*pImage=GetImage(1,30);
	if (pImage==NULL)
	{
		return ;
	}
	...
	while (iter!=m_listMPScreen.end())
	{
		MPScreen*pMPScreen=(MPScreen*)(*iter);
		pMPScreen->OnCaptureImage(pImage);
		++iter;
	}	
...
int ImageInputMgr::SetScreenPos(int nScreenIndex,int x,int y,int cx,int cy)
{
	...
	{
		pScreenInput=new ScreenInput(x,y,cx,cy);
...
void ImageInputMgr::AddMPScreen(MPScreen*pMPScreen)
{
	XAutoLock l(m_csMPScreen);
	if (m_listMPScreen.find(pMPScreen)==m_listMPScreen.end())
	{
...
void ImageInputMgr::RemoveMPScreen(MPScreen*pMPScreen)
{
	{
		XAutoLock l(m_csMPScreen);
		m_listMPScreen.remove(pMPScreen);
	...
	int nScreenIndex=pMPScreen->GetScreenIndex();
	if (nScreenIndex>=0 && nScreenIndex<=MAX_SCREEN_COUNT)
	{
		if (m_apScreenInput[nScreenIndex]!=NULL)
		{
			m_apScreenInput[nScreenIndex]->RemoveMPScreen(pMPScreen);
		}
	}
}

void ImageInputMgr::OnMPScreenScreenIndexChanged(MPScreen*pMPScreen,int nOldScreenIndex)
{
	int nNewScreenIndex=pMPScreen->GetScreenIndex();
	if (nNewScreenIndex!=nOldScreenIndex)
	{
		if (nOldScreenIndex>=0 && nOldScreenIndex<=MAX_SCREEN_COUNT)
		{
			...
			{
				m_apScreenInput[nOldScreenIndex]->RemoveMPScreen(pMPScreen);
			}
		}
		if (nNewScreenIndex>=0 && nNewScreenIndex<=MAX_SCREEN_COUNT)
		{
			...
			{
				m_apScreenInput[nNewScreenIndex]->AddMPScreen(pMPScreen);
			}
		}
	}
}
...
{
	x =x>>1 <<1; y =y >>1 <<1;  //20160427 解决新的VDP合成库在奇数时会出现偏色 YUV分离的问题 
