...
void * aligned_malloc( int size, int align )
{
	...
	if ( memory = (char*)malloc( size+align+8 ) ) {
		temp_mem = memory;
		memory += 4+align-1;
...
SimpleResize::SimpleResize(const VideoInfo &vi,unsigned int _width, unsigned int _height, double _hWarp, double _vWarp, bool _Interlaced)
{
	...
	else if (vi.IsYV12)
	{
		...
		{
			DebugOutput("malloc  1  Simple Resize  return  .\n");
				return;
		}
	}
	else
	{
		DebugOutput("Simple Resize  else     return  .\n");
	...
	{
		DebugOutput("malloc 2 Simple Resize    return  .\n");
		return;
	}
	if (DoYV12)
	{
...
public:
	int Open(void);
	void Close(void);
	int Resize(unsigned char*src[],int nSrcWidth,int nSrcHeight,unsigned char*dst[],int nDstWidth,int nDstHeight);
protected:
	...
	int				m_nSrcYImageSize;
	int				m_nSrcUVImageSize;
...
MPVideo_Resize*MPVideo_Resize::Create(void)
{
	return new XResize();
}
...
int XResize::Resize(unsigned char*src[],int nSrcWidth,int nSrcHeight,unsigned char*dst[],int nDstWidth,int nDstHeight)
{
	////ori:
	...
	if (m_pVDPixmapResampler==NULL)
	{
		//m_pVDPixmapResampler=VDResizeOpen( nSrcWidth,  nSrcHeight,  nSrcWidth,  nDstWidth,  nDstHeight,  nDstWidth);
		m_pVDPixmapResampler=VDResizeOpen( nSrcWidth,  nSrcHeight,  nSrcWidth,  nDstWidth,  nDstHeight,  nDstWidth);
	}
	...
	if (m_pVDPixmapResampler ==NULL)
	{
			...
			m_nDstHeight!=nDstHeight)
		{
			...
			m_pSimpleResize=new SimpleResize(vi,m_nDstWidth,m_nDstHeight,1.0,1.0,false);
			if (m_pSimpleResize==NULL)
			{
				DebugOutput("Resize  new return -1.\n");
				return -1;
			}
			if (!m_pSimpleResize->ok)
			{
				delete m_pSimpleResize;
				m_pSimpleResize=NULL;
				DebugOutput("Resize   Simple Resize init failed return -1.\n");
				return -1;
			}

			m_nSrcYImageSize=m_nSrcWidth*m_nSrcHeight;
			m_nSrcUVImageSize=m_nSrcYImageSize>>2;
