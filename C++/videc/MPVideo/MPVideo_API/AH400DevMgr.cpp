...
{
	int nDevCount=0;
	nDevCount+=MPVIDEO_AH400::GetDevCount();
	return nDevCount;
}
...
int AH400DevMgr::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
	...
	{
		return MPVIDEO_AH400::GetDevName(nDevIndex,szName,nMaxCount);
	}
...
int AH400DevMgr::AddImageSink(unsigned int nDevIndex,MPVIDEO_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapAH400DevInst);
	...
	if (iter!=g_MapAH400DevInst.end())
	{
		if (iter->second.pAH400Dev==NULL)
		{
			...
			if (nDevIndex<nDevCount)
			{
				pAH400Dev=new AH400Dev(nDevIndex);
...
void AH400DevMgr::RemoveImageSink(unsigned int nDevIndex,MPVIDEO_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapAH400DevInst);
...
int AH400DevMgr::AddStreamSink(unsigned int nDevIndex,MPVIDEO_StreamSink*pStreamSink)
{
	XAutoLock l(g_csMapAH400DevInst);
...
void AH400DevMgr::RemoveStreamSink(unsigned int nDevIndex,MPVIDEO_StreamSink*pStreamSink)
{
	XAutoLock l(g_csMapAH400DevInst);
...
void AH400DevMgr::SetOSDText(unsigned int nDevIndex,const char*cszText,int nAlign)
{
	XAutoLock l(g_csMapAH400DevInst);
	...
	{
		pAH400Dev=iter->second.pAH400Dev;
		pAH400Dev->SetOSDText(cszText,nAlign);
	}
}
