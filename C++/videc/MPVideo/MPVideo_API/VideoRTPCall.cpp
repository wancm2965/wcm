...
MPVIDEO_RTPCall*MPVIDEO_RTPCall::Create(void)
{
...
int VideoRTPCall::Open(HWND hLocalWnd,int nDevID,int nLocalWidth,int nLocalHeight,int nFrameRate,int nBitrate,MPVIDEO_CODEC_TYPE nCodecType,int nPayloadType,const char*cszLocalIP,unsigned short nLocalPort,HWND hPeerWnd,const char*cszPeerIP,unsigned short nPeerPort)
{
	m_pPeerWnd=MPVIDEO_Wnd::Create();
	if (m_pPeerWnd==NULL)
	{
		return -1;
	}
	...
	m_pPlayChanRTP=MPVIDEO_PlayChanRTP::Create();
	...
	m_pVideoRTPSession=new VideoRTPSession(*this);
	...
	m_pLocalWnd=MPVIDEO_Wnd::Create();
	if (m_pLocalWnd==NULL)
	{
		return -1;
	}
	...
	m_pCapChanRTP=MPVIDEO_CapChanRTP::Create(*this);
	...
	while (true)
	{
		MPVIDEO_CameraDev*pDev=MPVIDEO_CameraDev::Create();
		//MPVIDEO_AH400Dev*pDev=MPVIDEO_AH400Dev::Create();
		//MPVIDEO_AH2KDev*pDev=MPVIDEO_AH2KDev::Create();
		if (pDev==NULL)
		{
			break;
		}
...
void VideoRTPCall::OnMPVIDEO_CapChanRTPCallbackPacketData(MPVIDEO_CapChanRTP*pCapChanRTP,const void*pPacketData,int nPacketLen)
{
	if (m_pVideoRTPSession!=NULL)
	{
