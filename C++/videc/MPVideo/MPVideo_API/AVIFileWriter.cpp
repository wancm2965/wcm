...
{
	bool bKeyFrame=MPVIDEO_HEADER_EXT_GET_KEYFRAME(pVideoData)!=0;
...
bool AVIFileWriter::WriteAudio( unsigned char* pData, const int nLen ,unsigned long ulTimestamp,int&nBytes)
{
	...
	if (!m_bGotFirstAudio)
	{
		...
		short*pSilenceSamples=(short*)malloc(m_nSampleRate*2);
...
int AVIFileWriter::OpenAudioStream(int nSampleRate)
{
	...
	pWaveFormat=(PWAVEFORMATEX) malloc(sizeof(WAVEFORMATEX)+12);
