...
{
	char szDevName[1024]="";
	MPVIDEO_Camera::GetDeviceName(m_nDevIndex,szDevName);
...
int XCameraDev::Open(int nWidth,int nHeight,int nFrameRate,bool bDefaultFormat)
{
	...
	if (m_bLifeCamera)
	{
		if (nWidth==1280 || nWidth==960)
		{
			if (nWidth==1280)
			{
				m_nVirtualWidth=1280;
				m_nVirtualHeight=720;
			}
			if (nWidth==960)
			{
				m_nVirtualWidth=960;
		...
		else if (nWidth>1280)
		{
			...
			m_nVirtualWidth=1920;
			m_nVirtualHeight=1080;
		}
	}
	...
	m_pCamera=MPVIDEO_Camera::Create(*this,0);
	if (m_pCamera!=NULL)
	{
		if (0!=m_pCamera->Open(m_nDevIndex,nTempWidth,nTempHeight,m_nFrameRate))
		{
			m_pCamera->Close();
			delete m_pCamera;
			m_pCamera=NULL;
	...
	if (m_pCamera==NULL)
	{
		...
		m_pCamera=MPVIDEO_Camera::Create(*this,1);
		if (m_pCamera!=NULL)
		{
			if (0!=m_pCamera->Open(m_nDevIndex,nTempWidth,nTempHeight,m_nFrameRate))
			{
...
void XCameraDev::OnMPVIDEO_CameraCallbackImageData(MPVIDEO_Image*pImage)
{
...
void XCameraDev::OnIXPreProcessNotifyOutputData(MPVIDEO_Image*pImage)
{
	SendImage(pImage);
}

int XCameraDev::AddImageSink(MPVIDEO_ImageSink*pImageSink)
{
	return XImageSource::AddImageSink(pImageSink);
}

void XCameraDev::RemoveImageSink(MPVIDEO_ImageSink*pImageSink)
{
...
void XCameraDev::SetOSDText(const char*cszText,int nAlign)
{
	if (m_pIXPreProcess!=NULL)
	{
