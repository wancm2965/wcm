...
XStreamSource::XStreamSource(MPVideo_StreamSource*pStreamSource)
...
void XStreamSource::Close(void)
{
	...
	for (MapStreamSink::iterator iter=m_MapStreamSink.begin();
		...
		++iter)
	{
		MPVideo_StreamSink*pStreamSink=iter->second;
...
int XStreamSource::AddStreamSink(MPVideo_StreamSink*pStreamSink)
{
	if (pStreamSink!=NULL)
	{
...
void XStreamSource::RemoveStreamSink(MPVideo_StreamSink*pStreamSink)
{
	if (pStreamSink!=NULL)
	{
		pStreamSink->OnDetachedFromStreamSource(m_pStreamSource);
