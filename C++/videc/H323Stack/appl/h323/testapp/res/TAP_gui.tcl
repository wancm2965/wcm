...
# clearCalls    - 1 to clean up calls and channel listboxes while we're at it.
proc test:updateGui {clearCalls} {
    ...
    if {[rgrq:IsAutoRas] == 0} {
        ...
        # During updates we assume we have no status for the gatekeeper connection yet
    ...
    if {[test.SecSupported] == 0} {
        ...
        # set according to the saved value
        $tmp(secButton) configure -state normal
    }

    # Are we supporting H.460 ?
    ...
    # Deal with multi-homed addresses
    ...
    if {$clearCalls} {
        ...
        # also reset a few variables, if we're already restarting
        set tmp(adHoc,use) 0
