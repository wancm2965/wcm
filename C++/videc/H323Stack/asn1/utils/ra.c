    ...
    IN const RvChar*    name)
{
...
#if defined(RV_RA_SUPPORT_WORKING_SET)
    /* Calculate a default size for our working-set */
    ...
    {
        int i;
        ra->allocs = (void **)ra->arrayLocation;
        for (i = 0; i < maxNumOfElements; i++)
        {
            RvMemoryAlloc(NULL, RvMax(sizeof(vacantNode), (RvSize_t)elemSize), NULL, &ra->allocs[i]);
        }
    }
#endif
...
static void RVCALLCONV RvWatchdogResourceDeletedCallback(
    IN void*                        context,
    IN RvUint32                     resource)
{
    ...
    /* Make sure we forget this watchdog so we don't try to delete it when we're done */
    ra->watchdog = NULL;
}
...
#ifdef RV_DEBUG
/******************************************************************************
 * RvWatchdogResourcePrintCallback
 * ----------------------------------------------------------------------------
 * General: Print the items currently allocated in RA for the watchdog.
 *          Used strictly for debugging.
 *
 * Return Value: RV_OK on success, other on failure.
 * ----------------------------------------------------------------------------
 * Arguments:
 * Input:  context                  - RA to print out.
 *         logSource                - Log to print the data out to.
 * Output: None
 *****************************************************************************/
static void RVCALLCONV RvWatchdogResourcePrintCallback(
    IN void*                        context,
    IN RvLogSource*                 logSource)
{
    ...
    {
        if (!raElemIsVacant((HRA)ra, i))
        {
            RvLogDebug(logSource, (logSource, "Element %i: %p",
                i, RV_RA_ELEM_DATA(ra, i)));
        }
    }
}
#else
#define RvWatchdogResourcePrintCallback NULL
#endif
 ...
 *****************************************************************************/
static RvStatus RVCALLCONV RaWatchdogResourceCallback(
    IN  void*                   context,
    IN  RvUint32                resource,
...
raDestruct(HRA raH)
{
    if (raH != NULL)
    {
        ...
        if (ra->threadSafe || ra->userThreadSafe)
...
#if defined(RV_RA_DEBUG_MALLOC)
    if (RvMemoryCheckOverrun((void *)delElem, ra->logMgr) != RV_OK)
        return RV_ERROR_UNKNOWN;
#endif
...
raGetByPtr(IN HRA raH, IN void *ptr)
{
    ...
    for (position = 0; position < ra->maxNumOfElements; position++)
    {
        if (ra->allocs[position] == ptr)
            break;
    }
    if (position >= ra->maxNumOfElements)
    {
        RvLogExcep(ra->pLog,
            (ra->pLog, "raGetByPtr (%s): Pointer %p not found", ra->name, ptr));
        return RV_ERROR_UNKNOWN;
    }
#else /* defined(RV_RA_DEBUG_MALLOC) */
...
#endif /* defined(RV_RA_DEBUG_MALLOC) */
    ...
    IN const char   *description)
{
    ...
    if ((ra == NULL) || (ra->watchdog != NULL))
        return RV_ERROR_UNKNOWN;

    status = RvWatchdogAddResource(watchdog, name, group, description,
        RaWatchdogResourceCallback, RvWatchdogResourceDeletedCallback,
        RvWatchdogResourcePrintCallback,
        (void *)raH, &ra->watchdogResource);
    if (status >= 0)
