 ...
 * demonstrates how to implement call sending using the         *
 ...
 * 6. When the call is connected, opens one outgoing channel    *
 *    and accepts remote side request to open an incoming       *
 *    channel                                                   *
 * 7. When both channels are connected, enables each side to    *
                                           ...
                                           IN   cmCallStateMode_e  stateMode)
{
    ...
    switch(state)
    {
        /* This callback function is called with the following state when
           ...
           establishment processes are complete). In this case, the receiving
           side opens an outgoing audio channel. */
                                              ...
                                              IN  cmChannelStateMode_e stateMode)
{
    ...
    case cmChannelStateOffering:
        ...
        /* The callback function is called with this state when the channel
           is connected. The application counts the open channels and when
           there are 2 (outgoing and incoming) the receiving side drops its
           outgoing channel. */
...
int AppMain(int argc, char* argv[])
{
    ...
    /* This loop is responsible for activating the process that
       ...
       function after a call is connected and then disconnected.
       The user then prompts the application to terminate. */
