 ...
 * demonstrates how to implement tunneling call sending using   *
 ...
 *    callbacks.                                                *
 * 4. Creates a single tunneling call.                          *
 * 5. Waits for a response from the receiving side.             *
 * 6. When the tunneling call is connected, opens one outgoing  *
 *    channel and accepts the remote side request to open an    *
 *    incoming channel.                                         *
 * 7. When both channels are connected each side drops the      *
 *    channel it opened.                                        *
 * 8. Enables the sending side to drop the call.                *
 ***************************************************************/
                                    ...
                                    IN   cmCallStateMode_e  stateMode)
{
    ...
    /* This callback function is called with the following state when
       ...
       In this case, the receiving side opens an outgoing audio channel. */
                                              ...
                                              IN  cmChannelStateMode_e stateMode)
{
    ...
    /* This callback function is called with the following state when
       the channel is connected. The application counts the open channels
       and when there are 2 (outgoing and incoming) the receiving side
       drops its outgoing channel. */
...
int AppMain(int argc, char* argv[])
{
    ...
    /* This loop is responsible for activating the process that
       ...
       The application creates a single tunneling call to the IP
       in 'dest'.
       The loop stops when 'stop'=true. This is performed by the
       callback function after a tunneling call has been connected
       and then disconnected. The user then prompts the application
       to terminate.*/
    ...
    while(!stop)
    {
        if (!stop && !madeCall)
        {
            ...
            printf("Making a tunneling call to %s.\n", dest);
            cmCallSetParam(hsCall, cmParamH245Tunneling, 0, RV_TRUE, NULL);
