 ...
 * demonstrates how to implement tunneling call receiving using *
 * the RADVISION H.323 Protocol Stack.                          *
 * The program does the following:                              *
 *                                                              *
 * 1. Gets the host IP address and displays it on the screen for*
 *    the use of the sending side.                              *
 * 2. Initializes the H.323 Stack.                              *
 * 3. Installs the H.323 Stack general, call and channel        *
 *    callbacks.                                                *
 * 4. Waits for an incoming tunneling call and when it arrives, *
 *    prompts the user to answer it.                            *
 * 5. When the call is connected, opens one outgoing channel    *
 *    and accepts the remote side request to open an incoming   *
 *    channel.                                                  *
 * 6. When both channels are connected, each side drops the     *
 *    channel it opened.                                        *
...
/* The Stack handle for the call. */
HCALL hs_Call;

/* The Stack handle for the outgoing channel. */
HCHAN hsOutChan;
 ...
 *           lphaCall :Pointer to the application handle for    *
 *                     the new call.                            *
...
{
    RV_UNUSED_ARG(hApp)
    RV_UNUSED_ARG(lphaCall)

    /*This function obtains a specific call parameter (tunneling in
      this case) from the application. */
    cmCallSetParam(hsCall,cmParamH245Tunneling,0,1,NULL);
    printf("I support tunneling\n");
    return RV_OK;
}
                                    ...
                                    IN  cmCallStateMode_e   stateMode)
{
    ...
    case cmCallStateOffering:
        ...
        printf("Receiving a tunneling call offering.\n");
    ...
    case cmCallStateConnected:
        if (stateMode==cmCallStateModeConnectedCall)
        {
            ...
            printf("Opening an outgoing audio channel\n");
                                              ...
                                              IN  cmChannelStateMode_e stateMode)
{
    ...
    {
    /* This callback function is called with the following state when
       the Stack receives a channel offering. The application answers
       the channel. */
    ...
    case cmChannelStateOffering:
        ...
        /* This callback function is called with the following state when
           the channel is connected. The application counts the open
           channels and when there are 2 (outgoing and incoming) the
           receiving side drops its outgoing channel. */
    ...
    case cmChannelStateConnected:
        ...
        /* This callback function is called with the following state after
           the channel is disconnected. The 'cmChannelClose' function is
           called and after two channels are closed, the user is prompted
           to drop the call. */
...
int AppMain(int argc, char* argv[])
{
    ...
    /* Initializes the structures that contain pointers to the callback functions. */
    memset((void*)&cmEvent,        0, sizeof(SCMEVENT));
    memset((void*)&cmCallEvent,    0, sizeof(SCMCALLEVENT));
    memset((void*)&cmChannelEvent, 0, sizeof(SCMCHANEVENT));
    ...
    /* Sets the general event handler. */
    ...
    /* This loop is responsible for activating the process that
       ...
       The loop stops when 'stop'=true. This is performed by
       the callback function after a tunneling call has been
       connected and then disconnected. The user then prompts
