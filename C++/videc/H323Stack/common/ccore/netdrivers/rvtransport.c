                    ...
                    OUT RvAddress*          pRemoteAddr)
{
    ...
    {
        return (*transp->iface->pfnAccept)(transp, options, newTransp, pRemoteAddr);
    }
    else
    {
        return RV_ERROR_NOTSUPPORTED;
                            ...
                            OUT RvTransport*            pTransp)
{
    ...
    retv = RvMemoryAlloc(NULL, size, NULL/*logMgr*/, (void**)pTransp);
    if (retv != RV_OK)
    {
        retv = RvErrorGetCode(retv);
        return retv;
    }
    memset((void*)*pTransp, 0, size);
    (*pTransp)->size = size;
    
    retv = (*pfnConstructor)(cfg, *pTransp);
    if (retv != RV_OK)
    {
        RvMemoryFree((void*)*pTransp, NULL/*logMgr*/);
        *pTransp = NULL;
        return retv;
    }

    /* Increment the Reference Counter to indicate, that the object is in use.
    */
    RvTransportAddRef(*pTransp);
                            ...
                            OUT RvAddress*      pRemoteAddr)
{
    ...
    retv = RvMemoryAlloc(NULL, transp->size, NULL/*logMgr*/, (void**)pNewTransp);
    if (retv != RV_OK)
    {
        retv = RvErrorGetCode(retv);
        return retv;
    }
    memset((void*)*pNewTransp, 0, transp->size);
    (*pNewTransp)->size = transp->size;
    
    retv = RvTransportAccept(transp, options, *pNewTransp, pRemoteAddr);
    if (retv != RV_OK)
    {
        RvMemoryFree((void*)*pNewTransp, NULL/*logMgr*/);
        return retv;
    }
    return RV_OK;
}
