 ...
 * To do this the user should call the RvTransportCreate API function, while
 * providing the specific transport interface. The transport interface
 * represents set of functions that implement behavior of the specific transport
 * These functions reflect the Transport API, and they are invoked from within
 ...
 ****************************************************************************@*/
typedef RvInt32 RvTransportEvents;
#define RVTRANSPORT_EVENT_READ    0x01
#define RVTRANSPORT_EVENT_WRITE   0x02
#define RVTRANSPORT_EVENT_ACCEPT  0x04
#define RVTRANSPORT_EVENT_CONNECT 0x10
#define RVTRANSPORT_EVENT_CLOSE   0x20
 ...
 *  If you don't want to bother with memory allocation, you should consider
 *  to use the RvTransportCreateAndAccept utility function.
 *
 ...
 *  RvTransportCreate / transport-specific-create functions.
 *
 *  Note:
...
struct RvTransportBase
{
    ...
    RvSize_t                     size;       /* Size of memory allocated for
                                                the object.
                                                We need it in order to implement the RvTransportCreateAndAccept function. */
    RvInt32  refCounter;
        /* Reference counter.
...
/*@*****************************************************************************
 * RvTransportCreate (rvtransport)
 * -----------------------------------------------------------------------------
 * General:
 *  The RvTransportCreate function enables the user to create the Transport
 *  object of specific type.
 *  It allocates memory for the object and calls the transport-type specific
 *  Constructor function (RvTransportConstructF), provided by the caller.
 *
 * Arguments:
 * Input:   pfnConstructor - The transport type specific Constructor function.
 *          size           - The size of the memory to be allocated
 *                           for the transport object.
 *          cfg            - The configuration of the Transport to be created.
 *                           This pointer is passed to the Constructor,
 *                           where it should be casted to the transport type
 ...
 ****************************************************************************@*/
RVCOREAPI
RvStatus RVCALLCONV RvTransportCreate(
                    IN  RvTransportConstructF   pfnConstructor,
                    IN  RvSize_t                size,
                    IN  void*                   cfg,
                    OUT RvTransport*            pTransp);

/*@*****************************************************************************
 * RvTransportCreateAndAccept (rvtransport)
 * -----------------------------------------------------------------------------
 * General:
 *  The RvTransportCreateAndAccept function is a wrapper
 *  for the RvTransportCreateAccept function.
 *  The RvTransportCreateAndAccept allocates memory for the new Transport object
 *  of the same type, as the object, on which the new transport should be
 *  accepted, and calls the RvTransportCreateAccept.
 *  See the RvTransportAccept for more details.
 *
 *  Note you have to use the RvTransportDestroy function in order to destruct
 *  the object, created the RvTransportCreateAndAccept function, and to free
 *  it's memory.
 *
 * Arguments:
 * Input:   transp      - The listening Transport object.
 ...
 *                        The possible option values should be defined
 *                        by the implementation of the specific Transport.
 * Output:  pNewTransp  - The accepted Transport.
 *          pRemoteAddr - The address where from the connection was initiated.
 *
 * Return Value: RV_OK on success, error code otherwise (RV_ERROR_X).
 ****************************************************************************@*/
RVCOREAPI
RvStatus RVCALLCONV RvTransportCreateAndAccept(
                            IN  RvTransport     transp,
                            IN  RvUint32        options,
...
******* the RvTransportCreate / RvTransportCreateSocketTransport function and
******* you can provide them options as the 'options' function argument,
******* while calling the RvTransportAccept / RvTransportCreateAndAccept.
*/
