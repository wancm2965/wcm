...
#define MAX_LOCAL_ADDRS 30
    ...
    *numOfIps = 0;
    ...
    if(s != INVALID_SOCKET)
    {
		...
		//{{gaoshizhong 20130808 Add IP 0. rule out
		for(j=0;j< numLocalAddr;)
		{
			ULONG IPFirstByte=0;
			int sizeLen=0;
			INTERFACE_INFO localAddrTemp[MAX_LOCAL_ADDRS];
			pAddrInet = (SOCKADDR_IN*)&localAddr[j].iiAddress;
			IPFirstByte=(int)(pAddrInet->sin_addr.S_un.S_addr >>0) & 0xFF;
			if(IPFirstByte <= 0 )
			{
				sizeLen=sizeof(INTERFACE_INFO) * (numLocalAddr - j -1);
				memcpy(localAddrTemp,&localAddr[j+1], sizeLen);
				memcpy(&localAddr[j],&localAddrTemp,sizeLen);
				numLocalAddr--;
			}
			else j++;
		}
		j=0;
		//}}gaoshizhong 20130808 Add
		...
		for(j=0;j< numLocalAddr;)
		{
			...
			pAddrInet = (SOCKADDR_IN*)&localAddr[j].iiAddress;
			IPFirstByte=(int)(pAddrInet->sin_addr.S_un.S_addr >>0) & 0xFF;
			IPByte2=(int)(pAddrInet->sin_addr.S_un.S_addr >>8) & 0xFF;
			IPByte3=(int)(pAddrInet->sin_addr.S_un.S_addr >>16) & 0xFF;
			IPByte4=(int)(pAddrInet->sin_addr.S_un.S_addr >>24) & 0xFF;
			if(IPFirstByte == 127 && IPByte2 ==0 && IPByte3 ==0 && IPByte4 ==1)
			{
				sizeLen=sizeof(INTERFACE_INFO) * (numLocalAddr - j -1);
				memcpy(localAddrTemp,&localAddr[j+1], sizeLen);
				memcpy(&localAddr[j],&localAddrTemp,sizeLen);
        ...
        {
			//char * pIP = NULL;
			//pIP =inet_ntoa(localAddr[j].iiAddress.AddressIn.sin_addr);
