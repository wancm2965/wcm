...
#define FUNCTION_NAME "RvSockTranspConstruct"
    ...
    if (pCfg->pLocalAddr != NULL)
    {
        ...
        TRANSPORT_LOG_DBG_4("%p: goes to bind sock=%d to %s:%d",
...
#define FUNCTION_NAME "RvSockTranspDestruct"
    ...
    */
    ...
    TRANSPORT_LOG_DBG_1("%p: transport was destructed", pSockTransp);
    return RV_OK;
...
#define FUNCTION_NAME "RvSockTranspAccept"
    ...
    retv = RvSockTranspConstruct(&cfg, newTransp);
    if (retv != RV_OK)
    {
    ...
    retv = SetCreationOptions((RvSocketTransport*)newTransp, options);
    if (retv != RV_OK)
    {
        TRANSPORT_LOG_ERR_2("failed to set 0x%x options (sock=%d, retv=%d)",
            pSockTransp->sock, retv);
        RvSockTranspDestruct(newTransp);
        TRANSPORT_UNLOCK(transp);
        return retv;
    }

    /* Mark transport as connected in order to enable shutdown on destruct */
    ((RvSocketTransport*)newTransp)->status |= SOCKTRANSP_STATUS_CONNECTED;
...
#if (RV_LOGMASK != RV_LOGLEVEL_NONE)
    {
        ...
        TRANSPORT_LOG_DBG_6("%p(sock=%d): accepted connection from %s:%d, new Transport=%p(sock=%d)",
            pSockTransp, pSockTransp->sock, strIp, port, newTransp, newSock);
    }
#endif
...
RvStatus RvSockTranspSendBuffer(
                    IN  RvTransport             transp,
                    IN  RvUint8*                buff,
                    IN  RvSize_t                len,
...
#define FUNCTION_NAME "RvSockTranspAddRef"
    ...
    TRANSPORT_LOG_DBG_2("%p: Updated Reference Counter: %d", pSockTransp, pSockTransp->base.refCounter);
    TRANSPORT_UNLOCK(transp);
...
#define FUNCTION_NAME "RegisterEvent"
    ...
    if (selectEvents == 0)
    {
        if (pSockTransp->status & SOCKTRANSP_STATUS_WAITINGEVENTS)
        {
            ...
            TRANSPORT_LOCK(pSockTransp);
            pSockTransp->status &= ~SOCKTRANSP_STATUS_WAITINGEVENTS;
            TRANSPORT_LOG_DBG_2("%p(sock=%d): transport was removed from select",
                pSockTransp, pSockTransp->sock);
        }
        return RV_OK;
    }
