...
#define FUNCTION_NAME "RvAresCacheDRemoveQuery"
    ...
    if(self->inCallbackQid == qid) {
        ...
        RV_CONDVAR_WAITL(self->inCallbackQid != qid, &self->inCallbackCond, &self->lock, logMgr);
        return RV_OK;
    }
...
#define FUNCTION_NAME
    ...
    for(curAddr = &arec->firstAddr; curAddr != 0 && continueCallbacks; curAddr = curAddr->next) {
        ...
        if(RvErrorGetCode(s) == RV_ERROR_DESTRUCTED) {
            CACHE_LOG_DEBUG_1("Query qid=%d served from ehd", qid);
...
#define FUNCTION_NAME "RvAresCacheDAnalyzePacket"
    ...
    RV_CONDVAR_WAITL(self->inCallbackTid == 0, &self->inCallbackCond, &self->lock, logMgr);
    ...
    switch(query->data.discriminator) {
        ...
        case ADDRLIST:
            ...
            break;
        default:
            CACHE_LOG_EXCEP_2("Unknown discriminator (%d), for qid=%d, shouldn't happen", 
    ...
    if(query->data.discriminator == RDATA) {
        ...
        page->lockCnt--;
        CACHE_LOG_DEBUG_4("Serviced query qid=%d referencing rdata=%p referencing page=%p with lockCnt=%d",
...
#define FUNCTION_NAME "RvAresCacheDFind"
    ...
    if(qdata.discriminator == ADDRLIST) {
        ...
        CACHE_LOG_DEBUG_4("Query qid=%d references rdata=%p from page=%p with lockCnt=%d", 
...
void RvAresCacheDSetLogSrc(RvAresCacheD *self, RvLogSource *logSrc) {
    ...
    cache->pageAllocator.logSrc = logSrc;
}
