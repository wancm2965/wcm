...
#define FUNC "RvAresDestruct"
    ...
    if(dnsEngine->inCallbackQid != 0) {
        ...
        if(curThread == dnsEngine->inCallbackTid) {
            RvLockRelease(&dnsEngine->lock, logMgr);
            DnsLogExcep((LOG_FUNC "destroying resolver=%p from it's callback isn't supported", dnsEngine));
            return RV_ERROR_NOTSUPPORTED;
        }

        dnsEngine->destroyed = RV_TRUE;

        RV_CONDVAR_WAITL(dnsEngine->inCallbackQid == 0,
