...
#define FUNC "rv_ares__send_query"
    ...
    else  /* Send using UDP */
    {
        ...
        if (status != RV_OK)
        {
            DnsLogError((LOG_FUNC "Sending query for qid=%d to %s failed", query->user_qid, server->sAddr));
        ...
        DnsLogDebug((LOG_FUNC "Query with qid=%d sent using UDP to %s", query->user_qid, server->sAddr));
    }
...
RvStatus bind_socket(RvDnsEngine *channel, RvSocket *sock, RvInt addrType) {
    ...
    s = RvSocketBind(sock, &localAddress, NULL,  channel->logMgr);
