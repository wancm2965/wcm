...
typedef struct _parentLock parentLock;
...
struct _cmTransHost
{
    ...
    cmTransportAddress  localAddressInMsg; /*NAT support - the local address as it should be in the message*/
    HTPKT               hTpkt;
    HTPKT               h245Listen;
    void                *hashEntry;
...
struct _cmTransSession
{
    ...
    */
    ...
    RvUint8             sentButNotAckedTunneledMessages;
    FSSTATE             faststartState;
    EFCSTATE            extFastConnectState;
    cmAnnexEUsageMode   annexEUsageMode;
...
typedef struct
{
    ...
    parentLock              *parentLockArray; /* Array holding the parent locks. */
    parentLock              *parentLockFreeList; /* the list of free parent locks */
    parentLock              *parentLockFreeListTail; /* the end of the list of free parent locks */
    RvLock                  parentLockFreeListLock; /* lock for the list of free parent locks */
    int                     numParentLocks; /* the number of locks created */
...
typedef enum
{
    ...
    eTrans_callSupportNat,
    eTrans_setupSent,
    eTrans_parallelBeforeFS,
    eTrans_receivedRemoteFeatureSet,
...
#define m_sessionset(_session,_field,_value) \
    ((_session)->flags = ( ((_session)->flags  & ~(trans_field_mask(_field))) | (trans_field_mask(_field) & ((RvUint32)(_value) << ((int)eTrans_##_field))) ))
#define m_sessionget(_session,_field) \
    (((_session)->flags  & (trans_field_mask(_field))) >> ((int)eTrans_##_field))
