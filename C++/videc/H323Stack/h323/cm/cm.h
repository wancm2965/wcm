...
/********************************************************************************************************
    indicates the NAT mode of the endpoint:
    cmNatModeNoNat   -  the endpoint is not behind a NAT machine.
    cmNatModeNAT     -  the endpoint is behind a MAT machine, calls can be established with endpoints
                        inside or outside the same NAT machine.
    cmNatModeOnlyNAT -  the endpoint is behind a MAT machine, calls can be established only with endpoints
                        outside the NAT machine.
**********************************************************************************************************/
typedef enum
{
    cmNatModeNoNAT,
...
typedef enum
{
    cmDnsQueryTypeIpv4 = 1,   /*IPv4 address resolution*/
    cmDnsQueryTypeIpv6,       /*IPv6 address resolution*/
    cmDnsQueryTypeSrvIpv4,    /*service resolution for IPv4 addresses*/
    cmDnsQueryTypeSrvIpv6     /*service resolution for IPv6 addresses*/
} cmDnsQueryType;
...
typedef enum
{
    ...
    cmCallStateWaitAddressAck = cmCallStateTransfering + 4
} cmCallState_e;

typedef enum
{
    cmCallStateModeDisconnectedBusy = 0,
    cmCallStateModeDisconnectedNormal = 1,
    cmCallStateModeDisconnectedReject = 2,
    cmCallStateModeDisconnectedUnreachable = 3,
    cmCallStateModeDisconnectedUnknown = 4,
    cmCallStateModeDisconnectedLocal = 5,

    cmCallStateModeConnectedCallSetup = 7,
    cmCallStateModeConnectedCall = 8,
    cmCallStateModeConnectedConference = 9,

    /* The next 2 state modes are here for backward compatibility */
    cmCallStateModeCallSetupConnected = 7,
    cmCallStateModeCallConnected = 8,

    cmCallStateModeOfferingCreate = 10,
    cmCallStateModeOfferingInvite = 11,
    cmCallStateModeOfferingJoin = 12,
    cmCallStateModeOfferingCapabilityNegotiation = 13,
    cmCallStateModeOfferingCallIndependentSupplementaryService = 14,

    cmCallStateModeDisconnectedIncompleteAddress = 15,

    cmCallStateModeTransferingForwarded = 16,
    cmCallStateModeTransferingRouteToMC = 17,
    cmCallStateModeTransferingRouteToGatekeeper = 18,

    /* next state modes serves the AltEp feature */
    cmCallStateModeSetupNoReply = 19,
    cmCallStateModeSetupGotReleaseComplete = 20
...
typedef enum
{
    ...
    cmReasonTypeNoH245,                     /* FacilityReason */
    ...
    cmReasonTypeNeededFeatureNotSupported,
    cmReasonTypeHopCountExceeded
} cmReasonType;
...
/* cmCallDisconnectionReason
 * ------------------------------------------------------------------------
 * Reason that caused a given call to disconnect.
 * This enumeration can be used with cmCallGetParam() to find out why a
 * call got disconnected. It should be called when the call reaches the
 * cmCallStateDisconnected or cmCallStateIdle.
 */
typedef enum
{
    cmCallDisconnectionReasonUnknown = -1,          /* Reason for disconnection is unknown */
    cmCallDisconnectionReasonProtocolError,         /* Call got disconnected due to a protocol error (probably a good
                                                       message in the wrong state was received) */
    cmCallDisconnectionReasonLocalDropped,          /* Local application called cmCallDrop() */
    cmCallDisconnectionReasonRemoteDropped,         /* Remote side sent releaseComplete message */
    cmCallDisconnectionReasonNotRegistered,         /* The endpoint cannot register to gatekeeper. Call dialing got aborted */
    cmCallDisconnectionReasonReceivedARJ,           /* Got an ARJ from the gatekeeper for this call */
    cmCallDisconnectionReasonReceivedDRQ,           /* Gatekeeper sent a DRQ message for this call */
    cmCallDisconnectionReasonNoRasResponse,         /* ARQ request got timedout without any response from the GK */
    cmCallDisconnectionReasonNoDialResponse,        /* No response on dialing procedures from the gatekeeper or the remote endpoint */
    cmCallDisconnectionReasonQ931Closed,            /* Q931 connection was closed prematurely without any releaseComplete message on it */
    cmCallDisconnectionReasonH245Closed,            /* H.245 connection was closed */
    cmCallDisconnectionReasonTimeoutT301,           /* Call didn't get Q931 Connect message but got an Alerting message */
    cmCallDisconnectionReasonTimeoutT302,           /* Call didn't get additional address information it needed */
    cmCallDisconnectionReasonTimeoutT303,           /* Call didn't get any response for the Q931 Setup message */
    cmCallDisconnectionReasonTimeoutT304,           /* Call didn't get acknowledgment for its SetupAck message */
    cmCallDisconnectionReasonTimeoutT310            /* No additional response for call after Q931 Call Proceeding message */
...
typedef enum
{
    ...
    cmParamReleaseCompleteReason,           /* cmReasonType  4110 */
    ...
    cmParamExtension,                       /* cmAlias  4120 */
    cmParamExtention = cmParamExtension,    /* cmAlias - reserved for backward compatibility  4120 */
    ...
    cmParamConnectNonStandardData,          /* RvPvtNodeId  4140 */
    ...
    cmParamAlternativeExtention = cmParamAlternativeExtension, /* cmAlias - reserved for backward compatibility */
    cmParamSetupFastStart,                  /* RvPvtNodeId  */
    cmParamConnectFastStart,                /* RvPvtNodeId  4150 */
    ...
    cmParamIncompleteAddress,               /* Reserved for backward compatibility - not used */
    cmParamH245Tunneling,                   /* RvBool  4160 */
    cmParamFastStartInUseObsolete,          /* Reserved for backward compatibility - not used*/
    ...
    cmParamIsMultiplexed,                   /* RvBool  4170 */
    ...
    cmParamHopCount,                        /* RvInt32 - hopCount value to set for the outgoing Setup message. 1-31 (0 to remove/doesn't exist) */
    cmParamDisconnectionReason,             /* cmCallDisconnectionReason - reason for call's disconnection */
    cmParamExtendedFastConnectState,        /* cmExtendedFastConnectState_e */
    cmParamSupportNat,                      /* RvBool */
    cmParamParallelBeforeFS,                /* RvBool - allows sending of parallel H.245 reply before FS when set to RV_TRUE. */
    cmParamAltEpProperty,                   /* alternateEndpoints node, got from ACF */
    cmParamAltEpNum                         /* the alternate EP number, that was connected */
} cmCallParam;


typedef enum
{
    cmParamLocalCallSignalAddress,          /* cmTransportAddress, Get only */
    cmParamLocalAnnexEAddress,              /* cmTransportAddress, Get only */
    cmParamLocalRASAddress,                 /* cmTransportAddress, Get only */
    cmParamTerminalAliasesNode              /* RvPvtNodeId */
} cmParam;
 ...
 * cmEvDnsQueryResult
 * ----------------------------------------------------------------------------
 * General: A callback of the application called after a dns query is completed.
 *
 * Return Value: None.
 * ----------------------------------------------------------------------------
 * Arguments:
 ...
 *****************************************************************************/
typedef void
    (RVCALLCONV *cmEvDnsQueryResultT)(
        IN   HAPP                     hApp,
        IN   void*                    context,
        IN   RvStatus                 status,
 ...
 * cmEvNatRequest
 * ----------------------------------------------------------------------------
 * General: a request from the application of a NAT address translation.
 *
 *
 ...
 * Arguments:
 * Input:  hApp     - The stack's application handle.
 *         addrType - Type of address : (cmProtocol)
 *
 *         address  - The address to be translated.
 *
 * Output: address  - The translated address.
 *****************************************************************************/
typedef int
    (RVCALLCONV *cmEvNatRequestT)(
        IN      HAPP                 hApp,
...
typedef  struct
{
    ...
    cmEvNatRequestT cmEvNatRequest;
} SCMEVENT,*CMEVENT;
 ...
 * cmEvCallNatRequest
 * ----------------------------------------------------------------------------
 * General: a request from the application of a NAT address translation for
 *          a specific call.
 *
 * Return Value: RV_OK  - if successful.
 *               Other on failure
 ...
 *                      (rasAddressType, q931AddressType, h245AddressType)
 *         address        - The address to be translated.
 *
 * Output: address        - The translated address.
 *****************************************************************************/
typedef int
    (RVCALLCONV *cmEvCallNatRequestT)(
        IN      HAPPCALL             haCall,
        IN      HCALL                hsCall,
        IN      cmProtocol           addrType,
        INOUT   cmTransportAddress*  address);
...
/******************************************************************************
 * cmEvExtFastConnectProposalCanceledT
 * ----------------------------------------------------------------------------
 * General: Indication that a channel proposed in EFC was rejected by the other
 *          side.
 *
 * Return Value: None
 * ----------------------------------------------------------------------------
 ...
 * Input:  haCall       - Application's handle for the call.
 *         hsCall       - Stack's handle for the call.
 *         sessionId    - Session Id of the proposal.
 *         proposalNode - Node Id of the proposal.
 * Output: None.
 *****************************************************************************/
typedef int
    (RVCALLCONV*cmEvExtFastConnectProposalCanceledT)(
        IN      HAPPCALL            haCall,
        IN      HCALL               hsCall,
...
typedef  struct
{
    ...
    cmEvExtFastConnectProposalCanceledT cmEvExtFastConnectProposalCanceled;
    cmEvCallNatRequestT             cmEvCallNatRequest;
...
/************************************************************************
 * cmRestartListen
 * purpose: Stops and destruct all current listening q931 (tpkt & annexE) sockets
 *          and restart the listening process again on the current q931 listening
 ...
 *****************************************************************************/
RVAPI
RvStatus RVCALLCONV cmAddFeatureParameter(IN HAPP          hApp,
 ...
 *****************************************************************************/
RVAPI
RvBool RVCALLCONV cmIsFoundFeatureParameter(IN HAPP          hApp,
                                 IN RvPvtNodeId   featureNode,
                                 IN RvInt32       paramIdValue);
 ...
 * cmCallCatAddCall
 * ----------------------------------------------------------------------------
 * General: Adds a call to the CAT.
 * Return Value: RV_OK on success/ negative value on failure.
 * ----------------------------------------------------------------------------
 * Arguments:
 ...
 ************************************************************************/
RVAPI
int RVCALLCONV cmCallDialAltEp(
        IN  HCALL       hsCall,
        IN  int         altEpNum);
 ...
 ************************************************************************/
RVAPI
int RVCALLCONV cmRasModuleInit(
        IN   HAPP        hApp);
