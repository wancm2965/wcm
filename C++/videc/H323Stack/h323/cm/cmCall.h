 ...
 */
#define field_mask(_field) (RV_H323CALL_FLAG_TYPE)(((int)s_##_field)<<((RvUint32)e_##_field))

/* Set and get macros for fields inside callInfo.flags */
#define m_callset(_call,_field,_value) \
    ((_call)->flags = ( ((_call)->flags  & ~(field_mask(_field))) | (field_mask(_field) & ((RV_H323CALL_FLAG_TYPE)(_value) << ((int)e_##_field))) ))
#define m_callget(_call,_field) \
    (((_call)->flags  & (field_mask(_field))) >> ((int)e_##_field))
...
typedef struct __callElem {
    ...
    struct
    {
        ...
        RvBool                  callSupportNat;            /*if RV_TRUE call is with endpoint outside the NAT*/
    } flags;
#else
    ...
    cmCallDisconnectionReason   disconnectReason;   /* Reason this call disconnected */
    ...
    RvPvtNodeId                 altEpProperty;          /* PVT root of alternateEndpoints from ACF */
    int                         altEpNum;               /* the number of Alternate EPs that was connected */
    ...
    RvUint8                     hopCount;               /* Hop Count to use/used for the Setup message */
...
/* Structure holding message parameters for call when not using a property DB */
typedef struct
{
