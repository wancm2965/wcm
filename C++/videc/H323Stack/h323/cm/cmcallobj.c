...
{
    if (!app || !hsCall || !callObj) return RV_ERROR_UNKNOWN;
    ...
    switch(call)
    {
        case Outgoing:
            if (!app->gatekeeper)
                if (cmCallGetParam(hsCall,cmParamCRV,0,(RvInt32*)&callObj->rasCRV,NULL)>=0)
                    callObj->flags |= catRasCRV;
        break;
        case Incoming:
            if (cmCallGetParam(hsCall,cmParamRASCRV,0,(RvInt32*)&callObj->rasCRV,NULL)>=0)
                callObj->flags |= catRasCRV;
        break;
    ...
    switch(call)
    {
        case Outgoing:
            ...
            callObj->answerCall=RV_FALSE;
        break;
        case Incoming:
            callObj->flags |= catAnswerCall;
            callObj->answerCall=RV_TRUE;
        break;
...
callObjectCreateFromMessage(
          ...
          OUT catStruct *callObj)
{
    /* The message here is always a SETUP message */
    int q931NodeId,perNodeId;
    int tmpNodeId;
    HPVT hVal=app->hVal;

    if (!app || message<0 || !callObj) return RV_ERROR_UNKNOWN;

    memset(callObj, 0, sizeof(catStruct));

    /*CRV*/
    pvtGet(hVal,pvtChild(hVal,pvtGetChild(hVal,message,__q931(callReferenceValue),NULL)),NULL,NULL,(RvInt32*)&(callObj->crv),NULL);
    callObj->crv^=0x8000;
    ...
    /*answerCall: We're always the answering side for incoming SETUP messages*/
    callObj->answerCall = RV_TRUE;
    callObj->flags |= catAnswerCall;
    return 0;
}
