...
int  compareG(RvUint8 *key,int kSize ,int pos,tableHandleT table);




tableT *  tableConstruct(int nElements,int keySize,int elementSize, tableGetDataT get,
                         RAECompare compare, tableGetIndexT getIndex)
{
    tableT * table;
    if (RvMemoryAlloc(NULL, sizeof(tableT), TMP_LOG, (void**)&table) != RV_OK)
        return NULL;
    table->size=0;
    ...
    if (RvMemoryAlloc(NULL, sizeof(sortDataHandleT)*nElements, TMP_LOG, (void**)&table->sortData) != RV_OK)
    {
        RvMemoryFree(table, TMP_LOG);
        return NULL;
    }
    table->data = raConstruct(elementSize, nElements, RV_FALSE, "SNMP TABLE", NULL);
    raSetCompareFunc(table->data, compare);
    if (table->data == NULL)
    {
        RvMemoryFree(table->sortData, TMP_LOG);
        RvMemoryFree(table, TMP_LOG);
        return NULL;
