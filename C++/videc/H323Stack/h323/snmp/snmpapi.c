...
void h341InstEnd(h341InstanceHandle hInstHandle)
{
    ...
    RvMemoryFree(hSnmp, TMP_LOG);
}


RvBool isContinue(int errorCode)
{
    if (
        (errorCode == lastRaw)||
                        ...
                        OUT h341InstanceHandle * instHandle)
{
    ...
    if (RvMemoryAlloc(NULL, sizeof(h341InstanceHandleT), TMP_LOG, (void**)&hSnmp) != RV_OK)
        return noMemoryRc;
                                ...
                                OUT h341AgentHandleT  * agentHandle)
{
    ...
    if (RvMemoryAlloc(NULL, sizeof(h341AgentT), TMP_LOG, (void**)&hSnmp) != RV_OK)
        return noMemoryRc;
    if (RvMemoryAlloc(NULL, sizeof(h341RegisterInfoT)*maxInstanceNumber, TMP_LOG, (void**)&hSnmp->info) != RV_OK)
    {
        RvMemoryFree(hSnmp, TMP_LOG);
        return noMemoryRc;
    }
    ...
    {
        if (RvMemoryAlloc(NULL, MAX_ID_SIZE, TMP_LOG, (void**)&hSnmp->subtrees[ii].id) == RV_OK)
            hSnmp->subtrees[ii].length=str2oid(subtrees[ii],hSnmp->subtrees[ii].id);
    }

    if (RvMemoryAlloc(NULL, maxRequestNumber*sizeof(RequestDataT), TMP_LOG, (void**)&hSnmp->multReq) != RV_OK)
    {

        for (ii=0;ii<SUPPORT_SUBTREE_SIZE;ii++)
        {
            if (hSnmp->subtrees[ii].id !=NULL)
                RvMemoryFree(hSnmp->subtrees[ii].id, TMP_LOG);

        }
        RvMemoryFree(hSnmp->info, TMP_LOG);
        RvMemoryFree(hSnmp, TMP_LOG);
        return noMemoryRc;
    ...
    if (res == noMemoryRc)
    {
        RvMemoryFree(hSnmp->multReq, TMP_LOG);
        for (ii=0;ii<SUPPORT_SUBTREE_SIZE;ii++)
        {
            if (hSnmp->subtrees[ii].id !=NULL)
                RvMemoryFree(hSnmp->subtrees[ii].id, TMP_LOG);
...
void h341AgentEnd(h341AgentHandleT hSnmp)
{
    ...
    for (ii=0;ii<SUPPORT_SUBTREE_SIZE;ii++)
    {
        if(((h341AgentT *)hSnmp)->subtrees[ii].id !=NULL)
            RvMemoryFree(((h341AgentT *)hSnmp)->subtrees[ii].id, TMP_LOG);
    }
    RvMemoryFree(((h341AgentT *)hSnmp)->info, TMP_LOG);
    RvMemoryFree(((h341AgentT *)hSnmp)->multReq, TMP_LOG);
    destroyMib(((h341AgentT *)hSnmp)->root);
    RvMemoryFree((h341AgentT *)hSnmp, TMP_LOG);
...
int h341AgentRegister(h341AgentHandleT hAgent,h341RegisterHandleT reg)
{
    ...
    for(ii=0;ii<((h341AgentT*)hSnmp)->maxInstanceNumber;ii++)
        ...
        {
            if (RvMemoryAlloc(NULL, hSnmp->maxRequestNumber*sizeof(instanceRequestDataPtrT),
                TMP_LOG, (void**)&hSnmp->info[ii].iRequestsPtr) != RV_OK)
                return RV_ERROR_UNKNOWN;
...
void h341AgentUnRegister(h341AgentHandleT hAgent,int ifIndex)
{
    ...
    if (ifIndex)
    {
        if (hSnmp->info[ifIndex-1].h341AgentRegs!=NULL)
        {
            ...
            RvMemoryFree(hSnmp->info[ifIndex-1].iRequestsPtr, TMP_LOG);
            hSnmp->instNum--;
        }
    }
}
