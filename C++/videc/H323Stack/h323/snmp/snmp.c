...
{
    int ii;
    if (oid->length < (int)(sizeof(mmRootId) / sizeof(int)))
        return RV_FALSE;
    for (ii = 0; ii < (int)(sizeof(mmRootId) / sizeof(int)); ii++)
    {
                            ...
                            getH323ParameterT   getH323Parameter,RvBool table)
{
    ...
    if (!table)
    {
        RvMemoryAlloc(NULL, childNumber*sizeof(mibNodeT), TMP_LOG, (void**)&node->children);
...
h341ErrorT  createMib(h341AgentT* hSnmp)
{
    ...
    RvMemoryAlloc(NULL, sizeof(mibNodeT), TMP_LOG, (void**)&parent.children);
    parent.parent=NULL;
