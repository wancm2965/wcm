...
#define VERSION_STR    "Add-on RTP/RTCP Version 3.6.0.13"
#define VERSION_NUM    BUILD_VER_NUM(3, 6, 0, 13)
...
RvRtpSession RVCALLCONV RvRtpOpenEx(
								  ...
								  IN  char *        cname)
{
	int port= RvAddressGetIpPort((RvAddress*)pRtpAddress->address);
return rtpOpenEx(port,ssrcPattern,ssrcMask,cname);
 //   RvRtpSession res;
...
//{{gaoshizhong 20131129 Add reference RvRtpPackEx
RVAPI
RvInt32 RVCALLCONV RvRtpPackExCustom(
							   IN  RvRtpSession  hRTP,
							   IN  RvInt32	      ssrc,
							   IN  void *        buf,
							   IN  RvInt32       len,
	...
	IN RvRtpEncryptionKeyPlugIn* keyPlugInPtr)
{
	...
	//{{gaoshizhong 20140320 Add //add, because SetEncryption is can call two times , first call only create
	s->encryptionPlugInPtr     = NULL;
	//}}gaoshizhong 20140320 Add
	...
	IN RvRtpEncryptionPlugIn *pCrypto)
{
	RvRtpSessionInfo *s = (RvRtpSessionInfo *)hRtp;
	//add
	RvRtpEncryptionKeyPlugIn* keypluginPtr=NULL;
	if(s->encryptionKeyPlugInPtr ==NULL)
	{
		keypluginPtr = RvRtpDoubleKeyCreate(pEKey, pDKey);
	...
	{
		keypluginPtr = s->encryptionKeyPlugInPtr;
		if(pEKey!=NULL)
		{
			RvRtpDoubleEncKeySetKey((RvRtpDoubleKey *)keypluginPtr,pEKey);
			s->encryptionPlugInPtr     = (RvRtpEncryptionPlugIn*) pCrypto;
			if(s->hRTCP !=NULL)
			{
				rtcpSession * rtcp = (rtcpSession *)s->hRTCP;
				rtcp->encryptionPlugInPtr = (RvRtpEncryptionPlugIn*)pCrypto;
			}
			return RV_OK;
		}
		else if(pDKey!=NULL)
		{
			RvRtpDoubleDecKeySetKey((RvRtpDoubleKey *)keypluginPtr,pDKey);
			s->decryptionPlugInPtr     = (RvRtpEncryptionPlugIn*) pCrypto;
			if(s->hRTCP !=NULL)
			{
				rtcpSession * rtcp = (rtcpSession *)s->hRTCP;
				rtcp->decryptionPlugInPtr = (RvRtpEncryptionPlugIn*)pCrypto;
			}
			return RV_OK;
		}
		else return -1;
