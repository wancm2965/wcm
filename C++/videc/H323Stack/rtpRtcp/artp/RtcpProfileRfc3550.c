     ...
     OUT    RvAddress*                 remoteAddressPtr)
{
        ...
        (!RvRtpEncryptionModeIsPlainTextRtcp(s->encryptionMode)))
    {
		...
		if(decryptionKey.bitLength ==128 || decryptionKey.bitLength ==256) //aes
		{
			...
			AES_cbc_encrypt(buf->buffer,buf->buffer,bytesReceived, &aesKey,h235IV, (const int)AES_DECRYPT);
			//notice RV_RTPENCRYPTIONMODE_RFC1889 wait add...
		}
		else 
			RvRtpEncryptionPlugInDecrypt(s->decryptionPlugInPtr,
            buf->buffer,
            buf->buffer,
        ...
        INOUT RvUint32*     sentLenPtr)
{
   ...
   /* Check to see if sending has been disabled, if yes, abort. */
       ...
       (!RvRtpEncryptionModeIsPlainTextRtcp(thisPtr->encryptionMode)))
    {
		...
		else blockSize = RvRtpEncryptionPlugInGetBlockSize(thisPtr->encryptionPlugInPtr);
    ...
    while(destAddress != NULL)
    {
            ...
            (!RvRtpEncryptionModeIsPlainTextRtcp(thisPtr->encryptionMode)))
        {
			...
			if(encryptionKey.bitLength ==128 || encryptionKey.bitLength ==256) //aes
			{
				...
				AES_KEY  aesKey;
				int nMovePtr=0;
				int nDataTotalSize=DataLen;
				AES_set_encrypt_key(encKey,keyBitSize, &aesKey);
				AES_cbc_encrypt(bufferPtr + nMovePtr, workBufferPtr + nMovePtr, nDataTotalSize, &aesKey,h235IV, (const int)AES_ENCRYPT);
				//notice RV_RTPENCRYPTIONMODE_RFC1889 wait add...
			}
			else RvRtpEncryptionPlugInEncrypt(thisPtr->encryptionPlugInPtr,
                bufferPtr,
                workBufferPtr,
                DataLen,
                &encryptionData,
