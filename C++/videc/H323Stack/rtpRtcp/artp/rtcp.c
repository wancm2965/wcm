    ...
    switch(type)
    {
        ...
        case RTCP_RR:
        {
            if ((fInfo != NULL) && ((RvInt32)scanned < dataLen))
            {
                ...
                {
                    RvUint32 AValue         = reduceNNTP(myTime);
                    RvUint32 roundTripDelay = AValue - fInfo->eFromRR.tLSR - fInfo->eFromRR.tDLSR; /* 16:16 */
                    RvUint32 rtdMs          = (roundTripDelay*1000)>>16;
					...
					if (s->roundTripDelayCallbackEx != NULL)
						s->roundTripDelayCallbackEx((RvRtcpSession) s, s->roundTripDelayContext, fInfo->ssrc, rtdMs, RVRTCP_ROUND_TRIP_DELAY_RTCPSR);                            
					else 
						s->roundTripDelayCallback((RvRtcpSession) s, fInfo->ssrc, rtdMs, RVRTCP_ROUND_TRIP_DELAY_RTCPSR);                            
                }
            }
