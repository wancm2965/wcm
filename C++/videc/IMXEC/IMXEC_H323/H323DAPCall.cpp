...
void H323MediaCall::OnAudioRecvChannelConnected(void)
{
	if (m_pAudioRecvChannel!=NULL)
	{
		...
		if (nRet>=0)
		{
			...
			//CallMgr::Instance().OnAudioRecvChannelInfo(m_strCallID.c_str(),AudioCodecID);
		}
	}
}
	...
	if (m_pMainVideoRecvChannel!=NULL)
	{
		...
		if (nRet>=0)
		{
			...
			if (codecType!=VIDEC_CODEC_COUNT)
			{
				...
				//CallMgr::Instance().OnMainVideoRecvChannelInfo(m_strCallID.c_str(),codecType,tChannelInfo.VideoFormatType,nBitrate);
			}
		}
	}
}
