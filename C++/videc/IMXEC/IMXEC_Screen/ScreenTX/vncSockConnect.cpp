...
void *vncSockConnectThread::run_undetached(void * arg)
{
	...
	while (!m_shutdown) {
		...
		if (new_socket != NULL) {
			...
			const std::string&strPeerName=m_server->GetPeerName();
			if (strPeerName.size()==0 || strPeerName==new_socket->GetPeerName())
			{
...
BOOL vncSockConnect::Init(vncServer *server, UINT& port)
{
	...
	VCard nListeningPort=port;
	// Bind it
	if (!m_socket.Bind(nListeningPort))
		return FALSE;

	// Save the port id
	m_port = port = nListeningPort;
