...
{
    return fread(buf, 1, (size_t)size, (FILE *)file->internal);
}

static int64_t file_write_linux(BD_FILE_H *file, const uint8_t *buf, int64_t size)
{
    return fwrite(buf, 1, (size_t)size, (FILE *)file->internal);
}

static BD_FILE_H *file_open_linux(const char* filename, const char *mode)
{
    FILE *fp = NULL;
    BD_FILE_H *file = (BD_FILE_H *)malloc(sizeof(BD_FILE_H));
    ...
    MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, filename, -1, wfilename, sizeof(wfilename) / sizeof(*wfilename));

    wchar_t wmode[20];
    MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, mode, -1, wmode, sizeof(wmode) / sizeof(*wmode));

    errno_t err = _wfopen_s(&fp, wfilename, wmode);
#else
