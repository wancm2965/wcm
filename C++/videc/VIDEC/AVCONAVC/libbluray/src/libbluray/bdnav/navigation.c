...
NAV_TITLE_LIST* nav_get_title_list(const char *root, uint32_t flags, uint32_t min_title_length)
{
    ...
    title_list = (NAV_TITLE_LIST *)calloc(1, sizeof(NAV_TITLE_LIST));
    title_list->title_info = (NAV_TITLE_INFO *)calloc(title_info_alloc, sizeof(NAV_TITLE_INFO));
    ...
    for (res = dir_read(dir, &ent); !res; res = dir_read(dir, &ent)) {
        ...
        if (ii >= pl_list_size) {
            ...
            pl_list_size += 100;
            tmp = (MPLS_PL **)realloc(pl_list, pl_list_size * sizeof(MPLS_PL*));
            if (tmp == NULL) {
                X_FREE(path);
                break;
            }
        ...
        if (pl != NULL) {
            ...
            if (ii >= title_info_alloc) {
                ...
                tmp = (NAV_TITLE_INFO *)realloc(title_list->title_info,
...
NAV_TITLE* nav_title_open(const char *root, const char *playlist, unsigned angle)
{
    ...
    title = (NAV_TITLE *)calloc(1, sizeof(NAV_TITLE));
    if (title == NULL) {
        return NULL;
    }
    ...
    title->clip_list.clip = (NAV_CLIP *)calloc(title->pl->list_count, sizeof(NAV_CLIP));
    title->packets = 0;
    ...
    if (title->pl->sub_count > 0) {
        ...
        title->sub_path       = (NAV_SUB_PATH *)calloc(title->sub_path_count, sizeof(NAV_SUB_PATH));
        ...
        for (ss = 0; ss < title->sub_path_count; ss++) {
            ...
            sub_path->clip_list.clip  = (NAV_CLIP *)calloc(sub_path->clip_list.count, sizeof(NAV_CLIP));
    ...
    title->chap_list.mark = (NAV_MARK *)calloc(chapters, sizeof(NAV_MARK));
    title->mark_list.count = title->pl->mark_count;
    title->mark_list.mark = (NAV_MARK *)calloc(title->pl->mark_count, sizeof(NAV_MARK));
