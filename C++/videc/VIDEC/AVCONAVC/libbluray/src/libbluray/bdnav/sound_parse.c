...
static int _sound_read_samples(BITSTREAM *bs, SOUND_OBJECT *obj)
{
    ...
    obj->samples = (uint16_t *)calloc(num_samples, sizeof(uint16_t));
...
SOUND_DATA *sound_parse(const char *file_name)
{
    ...
    data_offsets = (uint32_t *)calloc(num_sounds, sizeof(uint32_t));
    data = (SOUND_DATA *)calloc(1, sizeof(SOUND_DATA) + num_sounds * sizeof(SOUND_OBJECT));
