...
_parse_clipinfo(BITSTREAM *bits, CLPI_CL *cl)
{
    ...
    if (cl->clip.is_atc_delta) {
        ...
        cl->clip.atc_delta = (CLPI_ATC_DELTA *)
...
_parse_sequence(BITSTREAM *bits, CLPI_CL *cl)
{
    ...
    atc_seq = (CLPI_ATC_SEQ *)malloc(cl->sequence.num_atc_seq * sizeof(CLPI_ATC_SEQ));
    ...
    for (ii = 0; ii < cl->sequence.num_atc_seq; ii++) {
        ...
        stc_seq = (CLPI_STC_SEQ *)malloc(atc_seq[ii].num_stc_seq * sizeof(CLPI_STC_SEQ));
...
_parse_program(BITSTREAM *bits, CLPI_CL *cl)
{
    ...
    CLPI_PROG *progs;
    progs = (CLPI_PROG *)malloc(cl->program.num_prog * sizeof(CLPI_PROG));
    ...
    for (ii = 0; ii < cl->program.num_prog; ii++) {
        ...
        CLPI_PROG_STREAM *ps;
        ps = (CLPI_PROG_STREAM *)malloc(progs[ii].num_streams * sizeof(CLPI_PROG_STREAM));
        progs[ii].streams = ps;
...
_parse_ep_map_stream(BITSTREAM *bits, CLPI_EP_MAP_ENTRY *ee)
{
    ...
    coarse = (CLPI_EP_COARSE *)malloc(ee->num_ep_coarse * sizeof(CLPI_EP_COARSE));
    ee->coarse = coarse;
    ...
    fine = (CLPI_EP_FINE *)malloc(ee->num_ep_fine * sizeof(CLPI_EP_FINE));
    ee->fine = fine;
...
_parse_cpi(BITSTREAM *bits, CLPI_CL *cl)
{
    ...
    entry = (CLPI_EP_MAP_ENTRY *)malloc(cl->cpi.num_stream_pid * sizeof(CLPI_EP_MAP_ENTRY));
    cl->cpi.entry = entry;
...
_clpi_parse(const char *path, int verbose)
{
    ...
    cl = (CLPI_CL *)calloc(1, sizeof(CLPI_CL));
    if (cl == NULL) {
        return NULL;
    }
...
clpi_copy(const CLPI_CL* src_cl)
{
    ...
    if (src_cl) {
        ...
        dest_cl->clip.atc_delta = (CLPI_ATC_DELTA *)malloc(src_cl->clip.atc_delta_count * sizeof(CLPI_ATC_DELTA));
        ...
        dest_cl->sequence.atc_seq = (CLPI_ATC_SEQ *)malloc(src_cl->sequence.num_atc_seq * sizeof(CLPI_ATC_SEQ));
        for (ii = 0; ii < src_cl->sequence.num_atc_seq; ii++) {
            ...
            dest_cl->sequence.atc_seq[ii].stc_seq = (CLPI_STC_SEQ *)malloc(src_cl->sequence.atc_seq[ii].num_stc_seq * sizeof(CLPI_STC_SEQ));
        ...
        dest_cl->program.progs = (CLPI_PROG *)malloc(src_cl->program.num_prog * sizeof(CLPI_PROG));
        for (ii = 0; ii < src_cl->program.num_prog; ii++) {
            ...
            dest_cl->program.progs[ii].streams = (CLPI_PROG_STREAM *)malloc(src_cl->program.progs[ii].num_streams * sizeof(CLPI_PROG_STREAM));
        ...
        dest_cl->cpi.entry = (CLPI_EP_MAP_ENTRY *)malloc(src_cl->cpi.num_stream_pid * sizeof(CLPI_EP_MAP_ENTRY));
        for (ii = 0; ii < dest_cl->cpi.num_stream_pid; ii++) {
            ...
            dest_cl->cpi.entry[ii].coarse = (CLPI_EP_COARSE *)malloc(src_cl->cpi.entry[ii].num_ep_coarse * sizeof(CLPI_EP_COARSE));
            ...
            dest_cl->cpi.entry[ii].fine = (CLPI_EP_FINE *)malloc(src_cl->cpi.entry[ii].num_ep_fine * sizeof(CLPI_EP_FINE));
