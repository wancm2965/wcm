...
_parse_stn(BITSTREAM *bits, MPLS_STN *stn)
{
    ...
    ss = NULL;
    if (stn->num_video) {
        ss = (MPLS_STREAM *)calloc(stn->num_video, sizeof(MPLS_STREAM));
    ...
    ss = NULL;
    if (stn->num_audio) {
        ss = (MPLS_STREAM *)calloc(stn->num_audio, sizeof(MPLS_STREAM));
    ...
    if (stn->num_pg  || stn->num_pip_pg) {
        ss = (MPLS_STREAM *)calloc(stn->num_pg + stn->num_pip_pg, sizeof(MPLS_STREAM));
    ...
    ss = NULL;
    if (stn->num_ig) {
        ss = (MPLS_STREAM *)calloc(stn->num_ig, sizeof(MPLS_STREAM));
    ...
    if (stn->num_secondary_audio) {
        ss = (MPLS_STREAM *)calloc(stn->num_secondary_audio, sizeof(MPLS_STREAM));
        for (ii = 0; ii < stn->num_secondary_audio; ii++) {
            ...
            if (ss->sa_num_primary_audio_ref) {
                ss->sa_primary_audio_ref = (uint8_t *)calloc(ss->sa_num_primary_audio_ref, sizeof(uint8_t));
    ...
    if (stn->num_secondary_video) {
        ss = (MPLS_STREAM *)calloc(stn->num_secondary_video, sizeof(MPLS_STREAM));
        for (ii = 0; ii < stn->num_secondary_video; ii++) {
           ...
           bs_skip(bits, 8);
            if (ss->sv_num_secondary_audio_ref) {
                ss->sv_secondary_audio_ref =  (uint8_t *)calloc(ss->sv_num_secondary_audio_ref, sizeof(uint8_t));
            ...
            if (ss->sv_num_pip_pg_ref) {
                ss->sv_pip_pg_ref = (uint8_t *)calloc(ss->sv_num_pip_pg_ref, sizeof(uint8_t));
...
_parse_playitem(BITSTREAM *bits, MPLS_PI *pi)
{
    ...
    pi->clip = (MPLS_CLIP *)calloc(pi->angle_count, sizeof(MPLS_CLIP));
...
_parse_subplayitem(BITSTREAM *bits, MPLS_SUB_PI *spi)
{
    ...
    spi->clip = (MPLS_CLIP *)calloc(spi->clip_count, sizeof(MPLS_CLIP));
...
_parse_subpath(BITSTREAM *bits, MPLS_SUB *sp)
{
    ...
    spi = (MPLS_SUB_PI *)calloc(sp->sub_playitem_count,  sizeof(MPLS_SUB_PI));
...
_parse_playlistmark(BITSTREAM *bits, MPLS_PL *pl)
{
    ...
    plm = (MPLS_PLM *)calloc(pl->mark_count, sizeof(MPLS_PLM));
...
_parse_playlist(BITSTREAM *bits, MPLS_PL *pl)
{
    ...
    pi = (MPLS_PI *)calloc(pl->list_count,  sizeof(MPLS_PI));
    ...
    sub_path = (MPLS_SUB *)calloc(pl->sub_count,  sizeof(MPLS_SUB));
...
_mpls_parse(const char *path, int verbose)
{
    ...
    pl = (MPLS_PL *)calloc(1, sizeof(MPLS_PL));
    if (pl == NULL) {
        return NULL;
    }
