...
typedef void*   (*plibaacs_open)(const char *device, const char *key);
typedef int     (*plibaacs_decrypt_unit)(void *bd, uint8_t *buf);

typedef void*   (*pbdplus_init)(const char *device, const char *key, const uint8_t *buf);
typedef int32_t (*pbdplus_fixup)(void *bd, int len, uint8_t *buf);
typedef int32_t (*pbdplus_seek)(void *bd, uint64_t pos);
...
struct bluray {
    ...
    void           *aacs;
    plibaacs_open         libaacs_open;
    plibaacs_decrypt_unit libaacs_decrypt_unit;

    /* BD+ */
    void           *h_libbdplus; // library handle
    void           *bdplus;
    pbdplus_init   bdplus_init;
    pbdplus_seek   bdplus_seek;
    pbdplus_fixup  bdplus_fixup;
...
#define DL_CALL(lib,func,sig,param,...)             \
     do {                                           \
          fptr_p_void fptr = (fptr_p_void)dl_dlsym(lib, #func);  \
          if (fptr) {                               \
            ((void* (*)sig)fptr)(param, ##__VA_ARGS__);           \
          }                                         \
      } while (0)
...
{
    if (!bd->event_queue) {
        bd->event_queue = (BD_EVENT_QUEUE *)calloc(1, sizeof(struct bd_event_queue_s));
...
static int _open_m2ts(BLURAY *bd, BD_STREAM *st)
{
    ...
    if ((st->fp = file_open(f_name, "rb"))) {
        ...
        if ((st->clip_size = file_tell(st->fp))) {
            ...
            if (bd->bdplus) {
                DL_CALL(bd->h_libbdplus, bdplus_set_title, (void *, uint32_t),
                        bd->bdplus, st->clip->clip_id);
            }

            if (bd->aacs) {
                uint32_t title = bd_psr_read(bd->regs, PSR_TITLE_NUMBER);
                DL_CALL(bd->h_libaacs, aacs_select_title, (void *, uint32_t),
                        bd->aacs, title);
            }
...
static int _preload_m2ts(BLURAY *bd, BD_PRELOAD *p)
{
    ...
    p->buf       = (uint8_t *)realloc(p->buf, p->clip_size);
...
{
    if (bd->aacs) {
        DL_CALL(bd->h_libaacs, aacs_close, (void*), bd->aacs);
        bd->aacs = NULL;
    }
}
...
static int _libaacs_load(BLURAY *bd)
{
    ...
    if ((bd->h_libaacs = dl_dlopen("libaacs", "0"))) {
        ...
        bd->libaacs_open         = (plibaacs_open)dl_dlsym(bd->h_libaacs, "aacs_open");
        bd->libaacs_decrypt_unit = (plibaacs_decrypt_unit)dl_dlsym(bd->h_libaacs, "aacs_decrypt_unit");
        ...
        if (bd->libaacs_open && bd->libaacs_decrypt_unit) {
            ...
            if (file_open != file_open_default()) {
                ...
                DL_CALL(bd->h_libaacs, aacs_register_file, (BD_FILE_H* (*)(const char* filename, const char *mode)), file_open);
            }
...
static uint8_t *_libaacs_get_vid(BLURAY *bd)
{
    if (bd->aacs) {
        ...
        if (fptr) {
            return ((uint8_t* (*)(void *))fptr)(bd->aacs);
        }
...
{
    if (bd->bdplus) {
        DL_CALL(bd->h_libbdplus, bdplus_free, (void*), bd->bdplus);
        bd->bdplus = NULL;
    }
}
...
static int _libbdplus_load(BLURAY *bd)
{
    ...
    if ((bd->h_libbdplus = dl_dlopen("libbdplus", "0"))) {
        ...
        bd->bdplus_init  = (pbdplus_init)dl_dlsym(bd->h_libbdplus, "bdplus_init");
        bd->bdplus_seek  = (pbdplus_seek)dl_dlsym(bd->h_libbdplus, "bdplus_seek");
        bd->bdplus_fixup = (pbdplus_fixup)dl_dlsym(bd->h_libbdplus, "bdplus_fixup");
...
BLURAY *bd_open(const char* device_path, const char* keyfile_path)
{
    ...
    BLURAY *bd = (BLURAY *)calloc(1, sizeof(BLURAY));
...
static BLURAY_TITLE_INFO* _fill_title_info(NAV_TITLE* title, uint32_t title_idx, uint32_t playlist)
{
    ...
    title_info = (BLURAY_TITLE_INFO *)calloc(1, sizeof(BLURAY_TITLE_INFO));
    ...
    title_info->chapters = (BLURAY_TITLE_CHAPTER *)calloc(title_info->chapter_count, sizeof(BLURAY_TITLE_CHAPTER));
    ...
    title_info->clips = (BLURAY_CLIP_INFO *)calloc(title_info->clip_count, sizeof(BLURAY_CLIP_INFO));
    for (ii = 0; ii < title_info->clip_count; ii++) {
        ...
        ci->video_streams = (BLURAY_STREAM_INFO *)calloc(ci->video_stream_count, sizeof(BLURAY_STREAM_INFO));
        ci->audio_streams = (BLURAY_STREAM_INFO *)calloc(ci->audio_stream_count, sizeof(BLURAY_STREAM_INFO));
        ci->pg_streams = (BLURAY_STREAM_INFO *)calloc(ci->pg_stream_count, sizeof(BLURAY_STREAM_INFO));
        ci->ig_streams = (BLURAY_STREAM_INFO *)calloc(ci->ig_stream_count, sizeof(BLURAY_STREAM_INFO));
        ci->sec_video_streams = (BLURAY_STREAM_INFO *)calloc(ci->sec_video_stream_count, sizeof(BLURAY_STREAM_INFO));
        ci->sec_audio_streams = (BLURAY_STREAM_INFO *)calloc(ci->sec_audio_stream_count, sizeof(BLURAY_STREAM_INFO));
