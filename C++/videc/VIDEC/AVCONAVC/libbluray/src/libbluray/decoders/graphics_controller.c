...
static int _save_page_state(GRAPHICS_CONTROLLER *gc)
{
    ...
    gc->saved_bog_data = (BOG_DATA *)calloc(page->num_bogs, sizeof(*gc->saved_bog_data));
...
static void _reset_page_state(GRAPHICS_CONTROLLER *gc)
{
    ...
    gc->bog_data = (BOG_DATA *)realloc(gc->bog_data, size);
...
{
    GRAPHICS_CONTROLLER *p = (GRAPHICS_CONTROLLER *)calloc(1, sizeof(*p));
