...
static void _join_fragments(PES_BUFFER *p1, PES_BUFFER *p2, int data_pos)
{
    ...
    if (p1->size < new_len) {
        p1->size = new_len + 1;
        p1->buf  = (uint8_t *)realloc(p1->buf, p1->size);
    }
...
static int _decode_ods(PG_DISPLAY_SET *s, BITBUFFER *bb, PES_BUFFER *p)
{
    ...
    s->object = (BD_PG_OBJECT *)realloc(s->object, sizeof(s->object[0]) * (s->num_object + 1));
...
static int _decode_pds(PG_DISPLAY_SET *s, BITBUFFER *bb, PES_BUFFER *p)
{
    ...
    s->palette = (BD_PG_PALETTE *)realloc(s->palette, sizeof(s->palette[0]) * (s->num_palette + 1));
...
static int _decode_pcs(PG_DISPLAY_SET *s, BITBUFFER *bb, PES_BUFFER *p)
{
    ...
    s->pcs = (BD_PG_COMPOSITION *)calloc(1, sizeof(*s->pcs));
...
{
    ig_free_interactive(&s->ics);
    s->ics = (BD_IG_INTERACTIVE *)calloc(1, sizeof(*s->ics));
...
int graphics_processor_decode_pes(PG_DISPLAY_SET **s, PES_BUFFER **p, int64_t stc)
{
    ...
    if (*s == NULL) {
        *s = (PG_DISPLAY_SET *)calloc(1, sizeof(PG_DISPLAY_SET));
    }
...
{
    GRAPHICS_PROCESSOR *p = (GRAPHICS_PROCESSOR *)calloc(1, sizeof(*p));
