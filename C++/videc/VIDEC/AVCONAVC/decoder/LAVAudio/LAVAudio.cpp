...
HRESULT CLAVAudio::LoadDefaults()
{
  ...
  for(int i = 0; i < AudioCodec_NB; ++i)
    m_settings.bFormats[i] = TRUE;

  // Disable WMA codecs by default
  m_settings.bFormats[Codec_WMA2]   = TRUE;
  m_settings.bFormats[Codec_WMAPRO] = TRUE;
  m_settings.bFormats[Codec_WMALL]  = TRUE;
...
{
  if (aCodec < 0 || aCodec >= AudioCodec_NB)
    return FALSE;
