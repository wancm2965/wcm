 ...
 * no message
 *
 * Revision 1.1.2.5  2004/10/28 06:27:57  lixq
 * no message
 *
...
int P64Encoder::ProcessOneFrame() {
      ...
      )
{
 ...
 //modify by lixq 2004-5-10
 /*
 if(length!=0) {                          //Check to see if a packet was read.
	
	 memcpy(buffer,b_ptr,len_buff);
 }
 */
}
...
VideoFrame::VideoFrame(u_char *cr, int newWidth, int newHeight):width(0),height(0)
{
  crvec = cr;
  frameptr = NULL;
...
VideoFrame::VideoFrame(int newWidth, int newHeight):width(0),height(0)
{
  crvec = NULL;
  frameptr = NULL;
  SetSize(newWidth,newHeight);
