...
参数说明：
	FrameRate:帧率(fps)
*/
void XRateControl::SetFrameRateTimer(int iFrameRate)
{
...
BOOL XRateControl::DoFrameRateCheck()
{
	BOOL bResult=TRUE;
...
BOOL XRateControl::CanEncThisFrame(DWORD dwClock,BOOL bBitRateControl)
{
	BOOL bResult=TRUE;
    /*>>>>>>>>>>>>>>>>>>>>>>>add by lixq */
...
void XRateControl::RateControlInit(int iWidth,int iHeight,int iBitrate, int iFrameRate,int iIFrameInterval)
{
...
void XRateControl::RateControlGetParam(int& iQuant,BOOL& bisKey)
{
...
void XRateControl::RateControlUpdate(int ms,XRC_DATA encresult)
{
...
void XRateControl::SetSentBW(int iSentBW)
{
	m_iSentBW=iSentBW;
