...
XDecoderFrameH263::XDecoderFrameH263(VIDEC_DecoderCallback&rCallback,int nH263Plus)
...
int XDecoderFrameH263::Open(int nWidth,int nHeight)
{
	...
	if (m_nH263Plus)
	{
		NEWRECORD( m_pRTPFrame,new H263PRTPFrame(*this) )
	}
	else
	{
		NEWRECORD( m_pRTPFrame,new H263RTPFrame(*this) )
	}
	NULLReturnRet( m_pRTPFrame ,-1 )  
...
int XDecoderFrameH263::DecodeFramePacket(void*pPacketData,int nPacketSize)
{
	...
	while (nLeftLen>0)
	{
		...
		if (nGotPicture>0 && m_bGotFirstKeyFrame)
		{
			...
			SetFrameRate(m_nFrameRateActual);
...
void XDecoderFrameH263::OnBaseRTPFrameCallbackFramePacket(BaseRTPFrame*pBaseRTPFrame,void*pPacketData,int nPacketLen,int nKeyFrame,int nFrameRate)
{
