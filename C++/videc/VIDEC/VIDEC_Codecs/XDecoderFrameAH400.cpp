...
XDecoderFrame*CreateXDecoderFrameAH400(VIDEC_DecoderCallback&rCallback,VIDEC_CODEC_TYPE CodecType)
{
	//return new XDecoderFrameAH400(rCallback,CodecType);
	NEWRECORDret3(  XDecoderFrame* ,p , new XDecoderFrameAH400(rCallback,CodecType)  )  
}


XDecoderFrameAH400::XDecoderFrameAH400(VIDEC_DecoderCallback&rCallback,VIDEC_CODEC_TYPE CodecType)
...
void XDecoderFrameAH400::OnImageData(unsigned char*pData,int nLen,int nWidth,int nHeight,long nStamp)
{
	if (m_pImageBuf==NULL || m_nImageBufLen<nLen)
	{
		...
		m_nImageBufLen=nLen;
		m_pImageBuf=(unsigned char*)MALLOCMSG(m_nImageBufLen+(m_nImageBufLen>>2));
		if (m_pImageBuf==NULL)
		{
			return;
		}
	...
	NEWRECORD3( XImage*,pXImage,new XImage(m_pImageBuf,nLen) )
	NULLRecord( pXImage  )  
	pXImage->AddRef();
	m_pImageBuf=NULL;
