 ...
 */
static av_always_inline void do_a_deblock_C(uint8_t *src, int step, stride_t stride, PPContext *c){
    int y;
    const int QP= c->QP;
    const int dcOffset= ((c->nonBQP*c->ppMode.baseDcDiff)>>8) + 1;
    const int dcThreshold= dcOffset*2 + 1;
...
static inline void postProcess(const uint8_t src[], stride_t srcStride, uint8_t dst[], stride_t dstStride, int width, int height,
...
//static void postProcess(uint8_t src[], stride_t srcStride, uint8_t dst[], stride_t dstStride, int width, int height,
...
static void reallocBuffers(PPContext *c, int width, int height, stride_t stride, stride_t qpStride){
                     ...
                     pp_mode *vm,  void *vc, int pict_type)
{
    ...
    PPContext *c = (PPContext*)vc;
    stride_t minStride= FFMAX(FFABS(srcStride[0]), FFABS(dstStride[0]));
    stride_t absQPStride = FFABS(QPStride);
