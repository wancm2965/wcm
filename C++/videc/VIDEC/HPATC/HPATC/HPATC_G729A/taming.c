...
{
  int i;
  for(i=0; i<4; i++) st->exc_err[i] = (FLOAT)1.;
  return;
}
...
)
{
    ...
    for(i=zone2; i>=zone1; i--) {
        if(st->exc_err[i] > maxloc) maxloc = st->exc_err[i];
    }
...
)
{
    ...
    n = t0- L_SUBFR;
    if(n < 0) {
        temp = (FLOAT)1. + gain_pit * st->exc_err[0];
        ...
        for(i = zone1; i <= zone2; i++) {
            temp = (FLOAT)1. + gain_pit * st->exc_err[i];
            if(temp > worst) worst = temp;
        }
    }

    for(i=3; i>=1; i--) st->exc_err[i] = st->exc_err[i-1];
    st->exc_err[0] = worst;
