                                 ...
                                 int absolute_region_power_index[MAX_NUMBER_OF_REGIONS])
{
    ...
    for (region = 0;  region < number_of_regions;  region++)
    {
        ...
        for (iterations = 0;  iterations < 6;  iterations++)
        {
            ...
            if (ftemp0 < region_power_table_boundary1[index - 1])
                index_max = index;
            else
                index_min = index;
        }
    ...
    for (region = 1;  region < number_of_regions;  region++)
    {
        ...
        number_of_bits += differential_region_power_bits1[region][j];
        drp_num_bits[region] = differential_region_power_bits1[region][j];
        drp_code_bits[region] = differential_region_power_codes1[region][j];
    }
                          ...
                          int32_t *word_ptr)
{
    ...
    bitcount_table_ptr = table_of_bitcount_tables1[category];
    code_table_ptr = table_of_code_tables1[category];

    inv_of_step_size_times_std_dev = step_size_inverse_table1[category]
                                   * standard_deviation_inverse_table1[power_index];
    ...
    for (n = 0;  n < num_vecs;  n++)
    {
        ...
        for (j = 0;  j < vec_dim;  j++)
        {
            k = (int) (fabs(*raw_mlt_ptr) * inv_of_step_size_times_std_dev + dead_zone1[category]);
            if (k != 0)
            {
                number_of_non_zero++;
                signs_index <<= 1;
...
int g722_1_encode(g722_1_encode_state_t *s, uint8_t g722_1_data[], const int16_t amp[], int len)
{
    ...
    for (i = 0, j = 0;  i < len;  i += s->frame_size, j += s->bytes_per_frame)
    {
        for (k = 0;  k < s->frame_size;  k++)
            famp[k] = amp[k + i];
        g7221_samples_to_rmlt_coefs(famp, s->history, mlt_coefs, s->frame_size);
