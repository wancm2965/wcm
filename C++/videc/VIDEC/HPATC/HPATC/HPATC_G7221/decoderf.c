...
static int index_to_array1(int index,
                    ...
                    int frame_error_flag)
{
    ...
    if (frame_error_flag == 0)
    {
        ...
        categorize1(s->number_of_regions,
                            ...
                            int absolute_region_power_index[MAX_NUMBER_OF_REGIONS])
{
    ...
    for (region = 1;  region < number_of_regions;  region++)
    {
        ...
        do
        {
            if (g722_1_bitstream_get(&s->bitstream, &(s->code_ptr), 1) == 0)
                index = differential_region_power_decoder_tree1[region][index][0];
            else
                index = differential_region_power_decoder_tree1[region][index][1];
            s->number_of_bits_left--;
        }
        while (index > 0);
    ...
    for (region = 0;  region < number_of_regions;  region++)
    {
        ...
        decoder_region_standard_deviation[region] = region_standard_deviation_table1[i];
    }
}
/*- End of function --------------------------------------------------------*/
                                                ...
                                                int rmlt_scale_factor)
{
    ...
    for (region = 0;  region < number_of_regions;  region++)
    {
        ...
        {
            decoder_table_ptr = table_of_decoder_tables1[category];
            vec_dim = vector_dimension1[category];
            num_vecs = number_of_vectors1[category];
            ...
            for (n = 0;  n < num_vecs;  n++)
            {
                ...
                if (ran_out_of_bits_flag == 1)
                    break;
                index = -index;
                num_sign_bits = index_to_array1(index, k, category);
                ...
                if (s->number_of_bits_left >= num_sign_bits)
                {
                    ...
                    for (j = 0;  j < vec_dim;  j++)
                    {
                        ...
                        decoder_mlt_value = standard_deviation
                                          * mlt_quant_centroid1[category][k[j]]
...
static int index_to_array1(int index, int array[MAX_VECTOR_DIMENSION], int category)
{
    int j;
    int q;
    int p;
    ...
    p = index;
    max_bin_plus_one = max_bin1[category] + 1;
    inverse_of_max_bin_plus_one = max_bin_plus_one_inverse1[category];

    for (j = vector_dimension1[category] - 1;  j >= 0;  j--)
    {
...
int g722_1_decode(g722_1_decode_state_t *s, int16_t amp[], const uint8_t g722_1_data[], int len)
{
    ...
    for (i = 0, j = 0;  j < len;  i += s->frame_size, j += s->number_of_bits_per_frame/8)
    {
        ...
        g7221_rmlt_coefs_to_samples(decoder_mlt_coefs, s->old_samples, famp, s->frame_size);
