...
**
*/
void Calc_Exc_Rand(enum Crate WrkRate,Word16 curGain, Word16 *PrevExc, Word16 *DataExc,
                                      Word16 *nRandom, LINEDEF *Line)
{
  ...
  */
    ...
    for(iblk=0; iblk<SubFrames/2; iblk++) {
        ...
        /* decode LTP only */
        Decod_Acbk(WrkRate,curExc, &PrevExc[0], Line->Olp[iblk],
                    Line->Sfs[i_subfr].AcLg, Line->Sfs[i_subfr].AcGn);
        Decod_Acbk(WrkRate,&curExc[SubFrLen], &PrevExc[SubFrLen], Line->Olp[iblk],
