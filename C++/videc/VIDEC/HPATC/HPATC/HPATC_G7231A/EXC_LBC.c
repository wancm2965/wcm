...
**
*/
void  Find_Fcbk( CODSTATDEF*pCodStat, Word16 *Dpnt, Word16 *ImpResp, LINEDEF *Line, Word16 Sfc )
{
    int   i  ;
...
**
*/
void  Fcbk_Unpk( DECSTATDEF*pDecStat, Word16 *Tv, SFSDEF Sfs, Word16 Olp, Word16 Sfc )
{
    int   i,j   ;
...
**
*/
void  Find_Acbk( CODSTATDEF*pCodStat, Word16 *Tv, Word16 *ImpResp, Word16 *PrevExc, LINEDEF
*Line, Word16 Sfc )
{
    int   i,j,k,l  ;
    ...
    off_filt = Test_Err( pCodStat, Lag1, Lag2);
...
(Word16) 1 ) ) ;
    ...
    Decod_Acbk( pCodStat->WrkRate, RezBuf, PrevExc, Olp, Lid, Gid ) ;
...
**
*/
void  Decod_Acbk( enum Crate WrkRate, Word16 *Tv, Word16 *PrevExc, Word16 Olp, Word16 Lid,
Word16 Gid )
{
    int   i,j   ;
...
*/
PFDEF Comp_Lpf( DECSTATDEF*pDecStat, Word16 *Buff, Word16 Olp, Word16 Sfc )
{
    int   i,j   ;
    ...
    if ( (Bindx != (Word16) 0) && ( Findx == (Word16) 0) )
        Pf = Get_Ind( pDecStat, Bindx, Scr[0], Scr[1], Scr[2] ) ;

    if ( (Bindx == (Word16) 0) && ( Findx != (Word16) 0) )
        Pf = Get_Ind( pDecStat, Findx, Scr[0], Scr[3], Scr[4] ) ;
...
**
*/
PFDEF Get_Ind( DECSTATDEF*pDecStat, Word16 Ind, Word16 Ten, Word16 Ccr, Word16 Enr )
{
    Word32   Acc0,Acc1 ;
    Word16   Exp   ;
    ...
    if ( Acc1 > Acc0 ) {
        ...
        if ( Ccr >= Enr )
            Pf.Gain = LpfConstTable[(int)pDecStat->WrkRate] ;
        else {
            Pf.Gain = div_s( Ccr, Enr ) ;
            Pf.Gain = mult( Pf.Gain, LpfConstTable[(int)pDecStat->WrkRate] ) ;
        }
