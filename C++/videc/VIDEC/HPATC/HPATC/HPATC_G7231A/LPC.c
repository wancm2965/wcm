...
**
*/
void  Comp_Lpc(CODSTATDEF*pCodStat, Word16 *UnqLpc, Word16 *PrevDat, Word16 *DataBuff )
{
    int   i,j,k ;
    ...
    if ( k >= 14 )
        pCodStat->SinDet |= 0x8000 ;

    /* Update CNG Acf memories */
    Update_Acf(pCodStat,Acf_sf, ShAcf_sf);
...
** Inputs:
**
**  pCodStat->WghtFirDl[] FIR filter memory from previous frame (10 words)
**  pCodStat->WghtIirDl[] IIR filter memory from previous frame (10 words)
...
**
*/
void  Error_Wght(CODSTATDEF*pCodStat,  Word16 *Dpnt, Word16 *PerLpc )
{
    int   i,j,k ;
 ...
 */
            ...
            for ( j = 0 ; j < LpcOrder ; j ++ )
                Acc0 = L_msu( Acc0, PerLpc[j], pCodStat->WghtFirDl[j] ) ;

            /* Update memory */
            for ( j = LpcOrder-1 ; j > 0 ; j -- )
                pCodStat->WghtFirDl[j] = pCodStat->WghtFirDl[j-1] ;
            pCodStat->WghtFirDl[0] = *Dpnt ;
  ...
  */
            ...
            for ( j = 0 ; j < LpcOrder ; j ++ )
                Acc0 = L_mac( Acc0, PerLpc[LpcOrder+j],
                                                    pCodStat->WghtIirDl[j] ) ;
            for ( j = LpcOrder-1 ; j > 0 ; j -- )
                pCodStat->WghtIirDl[j] = pCodStat->WghtIirDl[j-1] ;
            Acc0 = L_shl( Acc0, (Word16) 2 ) ;

            /* Update memory */
            pCodStat->WghtIirDl[0] = round( Acc0 ) ;
            *Dpnt ++ = pCodStat->WghtIirDl[0] ;
        }
        PerLpc += 2*LpcOrder ;
    }
}
...
** Inputs:
**
**  pCodStat->RingFirDl[] Perceptual weighting filter FIR memory from 
**               previous subframe (10 words)
**  pCodStat->RingIirDl[] Perceptual weighting filter IIR memory from 
**               previous subframe (10 words)
**
** Outputs:     
**
...
**
*/
void  Sub_Ring(CODSTATDEF*pCodStat,  Word16 *Dpnt, Word16 *QntLpc, Word16 *PerLpc, Word16
*PrevErr, PWDEF Pw )
{
    int   i,j   ;
    Word32   Acc0,Acc1 ;
  ...
  */
    ...
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        FirDl[i] = pCodStat->RingFirDl[i] ;
        IirDl[i] = pCodStat->RingIirDl[i] ;
    }
...
**  pCodStat->RingFirDl[] Updated perceptual weighting filter FIR memory 
**  pCodStat->RingIirDl[] Updated perceptual weighting filter IIR memory 
**
** Return value:    None
**
*/
void  Upd_Ring(CODSTATDEF*pCodStat,  Word16 *Dpnt, Word16 *QntLpc, Word16 *PerLpc, Word16
*PrevErr )
{
    int   i,j   ;
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_mac( Acc0, QntLpc[j], pCodStat->RingFirDl[j] ) ;
        Acc1 = L_shl( Acc0, (Word16) 2 ) ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_msu( Acc0, PerLpc[j], pCodStat->RingFirDl[j] ) ;

        /* Update FIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            pCodStat->RingFirDl[j] = pCodStat->RingFirDl[j-1] ;
        pCodStat->RingFirDl[0] = round( Acc1 ) ;

        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_mac( Acc0, PerLpc[LpcOrder+j], pCodStat->RingIirDl[j] ) ;
        Acc0 = L_shl( Acc0, (Word16) 2 ) ;

        /* Update IIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            pCodStat->RingIirDl[j] = pCodStat->RingIirDl[j-1] ;
        pCodStat->RingIirDl[0] = round( Acc0 ) ;

        /* Update harmonic noise shaping memory */
        PrevErr[PitchMax-SubFrLen+i] = pCodStat->RingIirDl[0] ;
    }
}
...
** Inputs:
**
**  pDecStat->SyntIirDl[] Synthesis filter memory from previous
subframe (10 words)
**
** Outputs:     
**
**  Word16 Dpnt[]       Synthesized speech vector sy[n]
**  pDecStat->SyntIirDl[] Updated synthesis filter memory 
**
** Return value:    None
**
*/
void     Synt( DECSTATDEF*pDecStat, Word16 *Dpnt, Word16 *Lpc )
{
    int   i,j   ;
  ...
  */
        ...
        /* Filter */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_mac( Acc0, Lpc[j], pDecStat->SyntIirDl[j] ) ;

        /* Update memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            pDecStat->SyntIirDl[j] = pDecStat->SyntIirDl[j-1] ;

        Acc0 = L_shl( Acc0, (Word16) 2 ) ;

        pDecStat->SyntIirDl[0] = round( Acc0 ) ;
  ...
  */
        if ( pDecStat->UsePf )
            Dpnt[i] = pDecStat->SyntIirDl[0] ;
        else
            Dpnt[i] = shl( pDecStat->SyntIirDl[0], (Word16) 1 ) ;
...
** Inputs:
**
**  pDecStat->PostIirDl[] Postfilter IIR memory from previous
subframe (10 words)
**  pDecStat->PostFirDl[] Postfilter FIR memory from previous
subframe (10 words)
**  pDecStat->Park        Previous value of compensation filter parameter
**
** Outputs:
**
**  Word16 Tv[]     Postfiltered speech vector pf[n] (60 words)
**  pDecStat->PostIirDl[] Updated postfilter IIR memory
**  pDecStat->PostFirDl[] Updated postfilter FIR memory
**  pDecStat->Park        Updated compensation filter parameter
**
** Return value: Input vector energy
**
*/
Word32  Spf( DECSTATDEF*pDecStat, Word16 *Tv, Word16 *Lpc )
{
    int   i,j   ;
  ...
  */
    Acc0 = L_deposit_h( pDecStat->Park ) ;
    Acc0 = L_msu( Acc0, pDecStat->Park, (Word16) 0x2000 ) ;
    Acc0 = L_mac( Acc0, Tmp, (Word16) 0x2000 ) ;
    pDecStat->Park = round( Acc0 ) ;

    Tmp  = mult( pDecStat->Park, PreCoef ) ;
    Tmp &= (Word16) 0xfffc ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_msu( Acc0, FirCoef[j], pDecStat->PostFirDl[j] ) ;

        /* Update FIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            pDecStat->PostFirDl[j] = pDecStat->PostFirDl[j-1] ;
        pDecStat->PostFirDl[0] = Tv[i] ;

        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = L_mac( Acc0, IirCoef[j], pDecStat->PostIirDl[j] ) ;

        /* Update IIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            pDecStat->PostIirDl[j] = pDecStat->PostIirDl[j-1] ;

        Acc0 = L_shl( Acc0, (Word16) 2 ) ;
        Acc1 = Acc0 ;

        pDecStat->PostIirDl[0] = round( Acc0 ) ;

 /*
  * Compensation filter
  */
        Acc1 = L_mac( Acc1, pDecStat->PostIirDl[1], Tmp ) ;
