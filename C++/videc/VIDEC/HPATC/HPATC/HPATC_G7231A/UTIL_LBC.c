...
** Inputs:
**
**  pCodStat->HpfZdl  FIR filter memory from previous frame (1 word)
**  pCodStat->HpfPdl  IIR filter memory from previous frame (1 word)
**
** Outputs:
**
**  Word16 *Dpnt
...
**
*/
void  Rem_Dc(CODSTATDEF*pCodStat,  Word16 *Dpnt )
{
    int   i  ;
    ...
    if ( pCodStat->UseHp ) {
        for ( i = 0 ; i < Frame ; i ++ ) {
            ...
            Acc0 = L_mac ( Acc0, pCodStat->HpfZdl, (Word16) 0xc000 ) ;
            pCodStat->HpfZdl = Dpnt[i] ;

            /* Do the Iir part */
            Acc1 = L_mls( pCodStat->HpfPdl, (Word16) 0x7f00 ) ;
            Acc0 = L_add( Acc0, Acc1 ) ;
            pCodStat->HpfPdl = Acc0 ;
            Dpnt[i] = round(Acc0) ;
        }
    }
    else {
...
**
*/
void    Line_Pack( CODSTATDEF*pCodStat, LINEDEF *Line, char *Vout, Word16 Ftyp )
{
    int     i ;
    int     BitCount ;
...
**
*/
LINEDEF  Line_Unpk( DECSTATDEF*pDecStat, char *Vinp, Word16 *Ftyp, Word16 Crc )
{
    int   i  ;
    Word16  BitStream[192] ;
    Word16 *Bsp = BitStream ;
  ...
  */
    *Ftyp = 1;
    ...
    /* Decode the bit-rate */
    pDecStat->WrkRate = (Info == 0) ? Rate63 : Rate53;
    ...
    for ( i = 0 ; i < SubFrames ; i ++ ) {
        ...
        if ( (pDecStat->WrkRate == Rate63) && (Line.Olp[i>>1] < (SubFrLen-2) ) ) {
...
**
*/
void  Scale( DECSTATDEF*pDecStat, Word16 *Tv, Word32 Sen )
{
    int   i ;
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        /* Update gain */
        Acc0 = L_deposit_h( pDecStat->Gain ) ;
        Acc0 = L_msu( Acc0, pDecStat->Gain, (Word16) 0x0800 ) ;
        Acc0 = L_mac( Acc0, SfGain, (Word16) 0x0800 ) ;
        pDecStat->Gain = round( Acc0 ) ;

        Exp = add( pDecStat->Gain, shr( pDecStat->Gain, (Word16) 4) ) ;
