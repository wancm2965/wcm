...
/* Define EDMA Event Id's Array EDMA_CHA_AREVTE0 EDMA_CHA_AREVTO0*/
static Uns eventIds[NUMPORTS][2] = {
    { EDMA_CHA_AREVTO0, EDMA_CHA_AXEVT0 },
#if NUMPORTS >= 2
                        ...
                        Ptr chanParams, IOM_TiomCallback cbFxn, Ptr cbArg)
{
    ...
    Uns mcaspAddr;
    Int edmaChanEvent;
    unsigned char *nousebufferIn,*nousebufferOut;
    Int i;
#if ENABLELOOPINTR
    Int loopTcc;
#endif
	...
	EDMA_Config tempEDMAcfg;
	EDMA_Handle htempEDMAcfg;

	EDMA_Config tempEDMAcfgOut;
	EDMA_Handle htempEDMAcfgOut;
    ...
    if (chan->mode == INPUT) {
		...
		EDMA_FSETSA(&loopEdmaCfg.opt, OPT, ESIZE,16BIT);
    ...
    if(chan->mode==INPUT)
    {
    	nousebufferIn=(unsigned char *)MEM_valloc(EXTERNALHEAP,4,0,0);
    	htempEDMAcfg=EDMA_open(EDMA_CHA_AREVTE0,EDMA_OPEN_RESET);
    	memcpy(&tempEDMAcfg,params->edmaCfgPtr,sizeof(EDMA_Config));
    	EDMA_FSETA(&tempEDMAcfg.opt,OPT,TCC,NULL);
    	EDMA_FSETA(&tempEDMAcfg.opt,OPT,SUM,NULL);
    	EDMA_FSETA(&tempEDMAcfg.opt,OPT,DUM,NULL);
  		EDMA_FSETSA(&tempEDMAcfg.opt, OPT, ESIZE, 16BIT);
    	EDMA_RSETA(&tempEDMAcfg.src,SRC,mcaspAddr);
    	EDMA_RSETA(&tempEDMAcfg.dst,DST,nousebufferIn);
    	EDMA_config(htempEDMAcfg,&tempEDMAcfg);
    	EDMA_link(htempEDMAcfg,htempEDMAcfg);
    	EDMA_enableChannel(htempEDMAcfg);
    ...
    if (chan->mode == INPUT) {
		...
		else if ((port->enableFsyncg == C6X1X_EDMA_MCASP_RCV)) {
