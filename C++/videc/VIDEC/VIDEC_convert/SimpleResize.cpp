...
//
//      This program is FREEMSG software; you can redistribute it and/or modify
...
//  feel the need to reward or inspire the author then please feel FREEMSG (but
//  not obligated) to consider joining or donating to the Electronic Frontier
//  Foundation. This will help keep cyber space FREEMSG of barbed wire and bullsh*t.  
//
...
void * aligned_malloc( int size, int align )
{
	...
	char *memory, *temp_mem;
	if ( memory = (char*)MALLOCMSG( size+align+8 ) ) {
		temp_mem = memory;
		memory += 4+align-1;
		memory = memory - ((long)memory%align);
		((char**)memory)[-1] = temp_mem;
		return( (void*)memory );
	}
	return( 0 );
}
...
SimpleResize::SimpleResize(const VideoInfo &vi,unsigned int _width, unsigned int _height, double _hWarp, double _vWarp, bool _Interlaced)
{
	...
	SSE2enabled = AVMPlayer::m_cpu_flags&FF_CPU_SSE2?true:false;
	SSEMMXenabled = AVMPlayer::m_cpu_flags&FF_CPU_MMXEXT?true:false;
	...
	else if (vi.IsYV12)
	{
		...
		{
			DebugOutput( logcoreerror ,"malloc  1  Simple Resize  return  .\n");
			return;
		}
	}
	else
	{
		DebugOutput( logcallstack ,"Simple Resize  else     return  .\n");
	...
	{
		DebugOutput( logcoreerror ,"malloc 2 Simple Resize    return  .\n");
		return;
	}
	if (DoYV12)
	{
...
public:
	XResize(void);
	virtual~XResize(void);
public:
	int Open(void);
	void Close(void);
	int Resize(unsigned char*src[],int nSrcWidth,int nSrcHeight,unsigned char*dst[],int nDstWidth,int nDstHeight,int nSrcStride,int nDstStride);
protected:
	SimpleResize*	m_pSimpleResize;
	int				m_nSrcWidth;
	int				m_nSrcHeight;
	int				m_nDstWidth;
	int				m_nDstHeight;
	SimpleResize::PVideoFrame m_SrcFrame;
	SimpleResize::PVideoFrame m_DstFrame;

	int				m_nDstYImageSize;
	int				m_nDstUVImageSize;

	void			*m_pVDPixmapResampler;
};

VIDEC_Resize*VIDEC_Resize::Create(void)
{
	//return new XResize();
	NEWRECORDret3(  VIDEC_Resize* ,p , new XResize( )  )       
}

XResize::XResize(void)
:m_pSimpleResize(NULL)
,m_nSrcWidth(0)
,m_nSrcHeight(0)
,m_nDstWidth(0)
,m_nDstHeight(0)
...
int XResize::Open(void)
{
	return 0;
}

void XResize::Close(void)
{
	if (m_pSimpleResize!=NULL)
	{
		delete m_pSimpleResize;
		m_pSimpleResize=NULL;
	}
	if (m_pVDPixmapResampler)
	{
		VDResizeClose(m_pVDPixmapResampler);
		m_pVDPixmapResampler=NULL;
	}
}

int XResize::Resize(unsigned char*src[],int nSrcWidth,int nSrcHeight,unsigned char*dst[],int nDstWidth,int nDstHeight,int nSrcStride,int nDstStride)
{
	////ori:
	//if (m_pVDPixmapResampler==NULL ||
	...
	//	m_nDstWidth=nDstWidth;
	//	m_nDstHeight=nDstHeight;
	//	m_pVDPixmapResampler=VDResizeOpen( nSrcWidth,  nSrcHeight,  nSrcStride,  nDstWidth,  nDstHeight,  nDstStride);
	...
	//call videcvdp.dll success
	if (m_pVDPixmapResampler)
	{
		if(m_nSrcWidth!=nSrcWidth ||
			m_nSrcHeight!=nSrcHeight ||
			m_nDstWidth!=nDstWidth ||
			m_nDstHeight!=nDstHeight)
		{
	...
	//videcvdp.dll process
	if (m_pVDPixmapResampler)
	{
		VDResizeGetFrame(m_pVDPixmapResampler,src,dst);
	}

	//not call videcvdp.dll, call this videc.dll 
	if (m_pVDPixmapResampler ==NULL)
	{
		if (m_pSimpleResize==NULL ||
			m_nSrcWidth!=nSrcWidth ||
			...
			m_nDstHeight!=nDstHeight)
		{
			...
			vi.width=m_nSrcWidth;
			vi.height=m_nSrcHeight;
			vi.IsYV12=true;
			vi.IsYUY2=!vi.IsYV12;

			NEWRECORD( m_pSimpleResize,new SimpleResize(vi,m_nDstWidth,m_nDstHeight,1.0,1.0,false) )
			//NULLReturnRet( m_pSimpleResize , -1 )  
			if (m_pSimpleResize==NULL)
			{
				DebugOutput( logcoreexcep ,"Resize  new return -1.\n");
				return -1;
			}
			if (!m_pSimpleResize->ok)
			{
				delete m_pSimpleResize;
				m_pSimpleResize=NULL;
				DebugOutput( logcoreexcep ,"Resize   Simple Resize init failed return -1.\n");
				return -1;
			}

			if (nSrcStride==0)
				nSrcStride=m_nSrcWidth;

			m_SrcFrame.pitch[0]=nSrcStride;
			m_SrcFrame.pitch[1]=nSrcStride>>1;
			m_SrcFrame.pitch[2]=nSrcStride>>1;
			//m_SrcFrame.pitch[0]=m_nSrcWidth;
			//m_SrcFrame.pitch[1]=m_nSrcWidth>>1;
			//m_SrcFrame.pitch[2]=m_nSrcWidth>>1;

			m_SrcFrame.rowSize[0]=m_nSrcWidth;
			m_SrcFrame.rowSize[1]=m_nSrcWidth>>1;
			m_SrcFrame.rowSize[2]=m_nSrcWidth>>1;

			m_SrcFrame.height[0]=m_nSrcHeight;
			m_SrcFrame.height[1]=m_nSrcHeight>>1;
			m_SrcFrame.height[2]=m_nSrcHeight>>1;

			m_nDstYImageSize=m_nDstWidth*m_nDstHeight;
			m_nDstUVImageSize=m_nDstYImageSize>>2;

			m_DstFrame.pitch[0]=m_nDstWidth;
			m_DstFrame.pitch[1]=m_nDstWidth>>1;
			m_DstFrame.pitch[2]=m_nDstWidth>>1;

			m_DstFrame.rowSize[0]=m_nDstWidth;
			m_DstFrame.rowSize[1]=m_nDstWidth>>1;
			m_DstFrame.rowSize[2]=m_nDstWidth>>1;

			m_DstFrame.height[0]=m_nDstHeight;
			m_DstFrame.height[1]=m_nDstHeight>>1;
			m_DstFrame.height[2]=m_nDstHeight>>1;

		}

		m_SrcFrame.ptr[0]=src[0];
		m_SrcFrame.ptr[1]=src[1];
		m_SrcFrame.ptr[2]=src[2];


		m_DstFrame.ptr[0]=dst[0];
		m_DstFrame.ptr[1]=dst[1];
		m_DstFrame.ptr[2]=dst[2];


		m_pSimpleResize->GetFrame(&m_SrcFrame,&m_DstFrame);
