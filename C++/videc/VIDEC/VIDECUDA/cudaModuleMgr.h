...
public:
    ...
    CUmoduleManager(const std::string& ptx_source, int nKernels, int nGlobalMem, int nTexRef);
    ~CUmoduleManager();
	...
	bool GetLoalModuleOK(void){return m_bLoadModuleOK;}
protected:
    // This stores all of the relevant data for the Module (PTX or CUBIN)
    sCtxModule mCtx;
	bool			m_bLoadModuleOK;
};
