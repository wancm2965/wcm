...
ImageDX::registerAsCudaResource(int field_num)
{
	...
	CUresult result = cuD3D9RegisterResource(pTexture_[field_num], 0);
	...
	result = cuD3D9ResourceSetMapFlags(pTexture_[field_num], CU_D3D9_MAPRESOURCE_FLAGS_WRITEDISCARD);
...
ImageDX::unregisterAsCudaResource(int field_num)
{
	...
	cuD3D9UnregisterResource(pTexture_[field_num]);
	bIsCudaResource_ = false;
...
ImageDX::map(CUdeviceptr * ppImageData, size_t * pImagePitch, int active_field)
{
	...
	cuD3D9MapResources(nFrames, reinterpret_cast<IDirect3DResource9 **>(pTexture_));
	cuD3D9ResourceGetMappedPointer(ppImageData, pTexture_[active_field], 0, 0);
	cuD3D9ResourceGetMappedPitch  (pImagePitch, NULL, pTexture_[active_field], 0, 0);
}
...
ImageDX::unmap(int active_field)
{
	...
	cuD3D9UnmapResources(nFrames, reinterpret_cast<IDirect3DResource9 **>(&pTexture_));
}
...
ImageDX::clear(unsigned char nClearColor)
{
	...
	cuD3D9MapResources(nFrames, (IDirect3DResource9 **)&pTexture_);

	for (int field_num=0; field_num < nFrames; field_num++)
	{
		cuD3D9ResourceGetMappedPointer(&pData, pTexture_[field_num], 0, 0);
		cuD3D9ResourceGetMappedSize   (&nSize, pTexture_[field_num], 0, 0);
		// clear the surface to solid white
		cuMemsetD8(pData, nClearColor, nSize);
	}
	cuD3D9UnmapResources(nFrames, (IDirect3DResource9 **)&pTexture_);
}
