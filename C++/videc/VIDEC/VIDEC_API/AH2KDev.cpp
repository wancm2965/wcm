...
void AH2KDev::OnVIDEC_AH2KCallbackPreSendImage(int&nSend)
{
	nSend = PreSendImage();
}

void AH2KDev::OnVIDEC_AH2KCallbackImage(VIDEC_Image*pImage)
{
#ifdef AH2KDEV_USE_THREAD
	{
		XAutoLock l(m_csListImage);
	...
	if (m_pIXPreProcess!=NULL)
	{
		if (pImage->GetFrameRate()>=50 && pImage->GetWidth()>=1920 && pImage->GetHeight()>=1080)
		{
			m_pIXPreProcess->EnableDenoise(false);
		}
		else
		{
			m_pIXPreProcess->EnableDenoise(m_bDenoise);
...
void AH2KDev::OnVIDEC_AH2KCallbackAudioData(short*pSamples,int nSamples,int nSampleRate)
{
	SendAudioSamples(pSamples,nSamples,nSampleRate);

	//char szDebug[1024]="";
	//sprintf(szDebug,"OnVIDEC_AH2KCallbackAudioData nSamples=%u\n",nSamples);
	//DebugOutput(szDebug);
}

void AH2KDev::OnIXPreProcessNotifyOutputData(VIDEC_Image*pImage)
{
	SendImage(pImage);
}

int AH2KDev::AddImageSink(VIDEC_ImageSink*pImageSink)
{
	return XImageSource::AddImageSink(pImageSink);
}

void AH2KDev::RemoveImageSink(VIDEC_ImageSink*pImageSink)
{
	XImageSource::RemoveImageSink(pImageSink);
}

int AH2KDev::AddStreamSink(VIDEC_StreamSink*pStreamSink)
{
	return XStreamSource::AddStreamSink(pStreamSink);
}

void AH2KDev::RemoveStreamSink(VIDEC_StreamSink*pStreamSink)
{
...
void AH2KDev::SetOSDText(const char*cszText,int nAlign,int nTop,int nLeft)
{
	if (m_pIXPreProcess!=NULL)
	{
