...
int XCameraDevMgr::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
	...
	{
		char szTemp[1024]="";
		if (0!=VIDEC_Camera::GetDeviceName(nDevIndex,szTemp))
		{
			return -1;
		}
...
int XCameraDevMgr::AddImageSink(unsigned int nDevIndex,VIDEC_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapXCameraDevInst);
	...
	if (iter!=g_MapXCameraDevInst.end())
	{
		if (iter->second.pXCameraDev==NULL)
		{
			...
			if (nDevIndex<nDevCount)
			{
				NEWRECORD( pXCameraDev ,new XCameraDev(nDevIndex) );
NULLReturnRet( pXCameraDev , -1 ) 
...
void XCameraDevMgr::RemoveImageSink(unsigned int nDevIndex,VIDEC_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapXCameraDevInst);
...
void XCameraDevMgr::SetOSDText(unsigned int nDevIndex,const char*cszText,int nAlign,int nTop,int nLeft)
{
	XAutoLock l(g_csMapXCameraDevInst);
	...
	{
		pXCameraDev=iter->second.pXCameraDev;
		pXCameraDev->SetOSDText(cszText,nAlign,nTop,nLeft);
	}
}
