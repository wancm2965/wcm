...
void XImageSourceAH400::SetOSDText(const char*cszText,int nAlign,int nTop,int nLeft)
{
	if (m_nDevID>=0)
	{
		AH400DevMgr::SetOSDText(m_nDevID,cszText,nAlign,nTop,nLeft);
	}
}
...
int XImageSourceAH400::AddImageSink(VIDEC_ImageSink*pImageSink)
{
	return XImageSource::AddImageSink(pImageSink);
}

void XImageSourceAH400::RemoveImageSink(VIDEC_ImageSink*pImageSink)
{
	XImageSource::RemoveImageSink(pImageSink);
}

void XImageSourceAH400::OnAttachedToImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnAttachedToImageSource(pImageSource);
}

void XImageSourceAH400::OnDetachedFromImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnDetachedFromImageSource(pImageSource);
}
void XImageSourceAH400::OnReceivedPreSend(int& nSend)
{
	nSend = PreSendImage();
}
void XImageSourceAH400::OnReceivedImage(VIDEC_Image*pImage)
{
#ifdef _DEMO
...
int XImageSourceAH400::AddStreamSink(VIDEC_StreamSink*pStreamSink)
{
	return XStreamSource::AddStreamSink(pStreamSink);
}

void XImageSourceAH400::RemoveStreamSink(VIDEC_StreamSink*pStreamSink)
{
	XStreamSource::RemoveStreamSink(pStreamSink);
}

void XImageSourceAH400::OnAttachedToStreamSource(VIDEC_StreamSource*pStreamSource)
{
	XStreamSink::OnAttachedToStreamSource(pStreamSource);
}

void XImageSourceAH400::OnDetachedFromStreamSource(VIDEC_StreamSource*pStreamSource)
{
	XStreamSink::OnDetachedFromStreamSource(pStreamSource);
}

void XImageSourceAH400::OnReceivedStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen,bool bKeyFrame,int nWidth,int nHeight,int nHeaderLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
void XImageSourceAH400::OnReceivedSubStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen,bool bKeyFrame,int nWidth,int nHeight,int nHeaderLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
void XImageSourceAH400::OnReceivedAudioStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
VIDEC_AH400Dev*VIDEC_AH400Dev::Create(void)
{
	//return new XImageSourceAH400();
	NEWRECORDret3(  VIDEC_AH400Dev* ,p , new XImageSourceAH400()  )  
}

//取设备个数
int VIDEC_AH400Dev::GetDevCount(void)
{
	return AH400DevMgr::GetDevCount();
}

//取设备名称
int VIDEC_AH400Dev::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
	return AH400DevMgr::GetDevName(nDevIndex,szName,nMaxCount);
}

//列出可裁剪的格式的个数
int VIDEC_AH400Dev::GetFormatCount(void)
{
	return AH400DevMgr::GetFormatCount();
}
	
//取格式名称
int VIDEC_AH400Dev::GetFormatName(int nIndex,char*szName,int nMaxCount)
{
	return AH400DevMgr::GetFormatName(nIndex,szName,nMaxCount);
}

//取格式大小
int VIDEC_AH400Dev::GetFormatSize(int nIndex,int&nWidth,int&nHeight)
{
