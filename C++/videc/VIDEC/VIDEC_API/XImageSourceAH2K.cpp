...
void XImageSourceAH2K::SetOSDText(const char*cszText,int nAlign,int nTop,int nLeft)
{
	if (m_nDevID>=0)
	{
		AH2KDevMgr::SetOSDText(m_nDevID,cszText,nAlign,nTop,nLeft);
	}
}
...
int XImageSourceAH2K::AddImageSink(VIDEC_ImageSink*pImageSink)
{
	return XImageSource::AddImageSink(pImageSink);
}

void XImageSourceAH2K::RemoveImageSink(VIDEC_ImageSink*pImageSink)
{
	XImageSource::RemoveImageSink(pImageSink);
}

void XImageSourceAH2K::OnAttachedToImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnAttachedToImageSource(pImageSource);
}

void XImageSourceAH2K::OnDetachedFromImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnDetachedFromImageSource(pImageSource);
}

void XImageSourceAH2K::OnReceivedPreSend(int& nSend)
{
	nSend = PreSendImage();
}
void XImageSourceAH2K::OnReceivedImage(VIDEC_Image*pImage)
{
...
int XImageSourceAH2K::AddStreamSink(VIDEC_StreamSink*pStreamSink)
{
	return XStreamSource::AddStreamSink(pStreamSink);
}

void XImageSourceAH2K::RemoveStreamSink(VIDEC_StreamSink*pStreamSink)
{
	XStreamSource::RemoveStreamSink(pStreamSink);
}

void XImageSourceAH2K::OnAttachedToStreamSource(VIDEC_StreamSource*pStreamSource)
{
	XStreamSink::OnAttachedToStreamSource(pStreamSource);
}

void XImageSourceAH2K::OnDetachedFromStreamSource(VIDEC_StreamSource*pStreamSource)
{
	XStreamSink::OnDetachedFromStreamSource(pStreamSource);
}

void XImageSourceAH2K::OnReceivedStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen,bool bKeyFrame,int nWidth,int nHeight,int nHeaderLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
void XImageSourceAH2K::OnReceivedSubStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen,bool bKeyFrame,int nWidth,int nHeight,int nHeaderLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
void XImageSourceAH2K::OnReceivedAudioStreamData(VIDEC_StreamSource*pStreamSource,unsigned char*pData,int nLen)
{
#ifdef _DEMO
	unsigned long ulTimestamp=XGetTimestamp();
	if (ulTimestamp<m_ulFirstTimestamp)
...
VIDEC_AH2KDev*VIDEC_AH2KDev::Create(void)
{
	//return new XImageSourceAH2K();
	NEWRECORDret3(  VIDEC_AH2KDev* ,p , new XImageSourceAH2K() )  
}

//取设备个数
int VIDEC_AH2KDev::GetDevCount(void)
{
	return AH2KDevMgr::GetDevCount();
}

//取设备名称
int VIDEC_AH2KDev::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
	return AH2KDevMgr::GetDevName(nDevIndex,szName,nMaxCount);
}

int VIDEC_AH2KDev::GetDevType(unsigned int nDevIndex,VIDEC_DEV_TYPE&nDevType)
{
	return AH2KDevMgr::GetDevType(nDevIndex,nDevType);
}

bool VIDEC_AH2KDev::HasAudioInput(unsigned int nDevIndex)
{
	return AH2KDevMgr::HasAudioInput(nDevIndex);
}

bool VIDEC_AH2KDev::SupportHardwareEncoding(unsigned int nDevIndex)
{
	return AH2KDevMgr::SupportHardwareEncoding(nDevIndex);
}

//列出可裁剪的格式的个数
int VIDEC_AH2KDev::GetFormatCount(void)
{
	return AH2KDevMgr::GetFormatCount();
}
	
//取格式名称
int VIDEC_AH2KDev::GetFormatName(int nIndex,char*szName,int nMaxCount)
{
	return AH2KDevMgr::GetFormatName(nIndex,szName,nMaxCount);
}

//取格式大小
int VIDEC_AH2KDev::GetFormatSize(int nIndex,int&nWidth,int&nHeight)
{
