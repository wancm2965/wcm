...
#define MAX_FILE_SIZE	2013265920

VIDEC_AVIWriter*VIDEC_AVIWriter::Create(void)
{
	//return new AVIWriter();
	NEWRECORDret3(  VIDEC_AVIWriter* ,p , new AVIWriter() ) 
}
...
*返回值：成功返回0，失败返回-1
*****************************************************************************/
int AVIWriter::Open(const char*cszFileName,int nStandardFormat,int nMaxFileSize, int CacheTimeMS)
{
	m_CacheTimeMS = CacheTimeMS;
	...
	if (nStandardFormat<WRITER_STANDARD_FORMAT_NONE ||
		nStandardFormat>WRITER_STANDARD_FORMAT_MP4)
	{
		nStandardFormat=WRITER_STANDARD_FORMAT_MP4;
	}
...
int AVIWriter::WriteVideo(unsigned char*pPacketData,int nPacketLen)
{
	...
	XVideoBuffer*pXVideoBuffer;
	XAutoLock l(m_csListXVideoBuffer);
	NEWRECORD( pXVideoBuffer , new XVideoBuffer );
	//NULLReturnRet( pXVideoBuffer , -1 ) 
	if (pXVideoBuffer)
	{
		pXVideoBuffer->pBuffer=MALLOCMSG(nPacketLen);
...
int AVIWriter::WriteAudio(unsigned char*pSampleData,int nSampleDataLen,int nSampleRate,unsigned long nSynTimestamp)
{
	...
	XAudioBuffer*pXAudioBuffer;
	XAutoLock l(m_csListXAudioBuffer);
	NEWRECORD( pXAudioBuffer , new XAudioBuffer );
	//NULLReturnRet( pXAudioBuffer , -1 ) 
	if (pXAudioBuffer)
	{
		pXAudioBuffer->pBuffer=MALLOCMSG(nSampleDataLen);
...
bool AVIWriter::DoWriteVideo(void)
{
	...
	if (pXVideoBuffer!=NULL)
	{
		...
		if (bKeyFrame)
		{
			...
			if (VIDEC_HEADER_EXT_GET_DOUBLE_FIELD(pData))
			{
				nStandardFormat=WRITER_STANDARD_FORMAT_NONE;
			}
			if (nStandardFormat==WRITER_STANDARD_FORMAT_NONE)
			{
				nWidth=VIDEC_HEADER_EXT_GET_VIRTUAL_WIDTH(pData);
				nHeight=VIDEC_HEADER_EXT_GET_VIRTUAL_HEIGHT(pData);
			}
			else
			{
				nWidth=VIDEC_HEADER_EXT_GET_ACTUAL_WIDTH(pData);
				nHeight=VIDEC_HEADER_EXT_GET_ACTUAL_HEIGHT(pData);
			}
				...
				m_bOpenVideoStream==false)
			{
				...
				VIDEC_CODEC_TYPE codecType=(VIDEC_CODEC_TYPE)VIDEC_HEADER_EXT_GET_CODEC_ID(pData);
				...
				if (m_pFileWriter!=NULL && (codecType==VIDEC_CODEC_H263 || codecType==VIDEC_CODEC_H263P || codecType==VIDEC_CODEC_H263PP))
				{
					bNewFile=true;
				}
				...
				if (m_pFileWriter==NULL)
				{
					...
					switch (nStandardFormat)
					{
					case WRITER_STANDARD_FORMAT_AVI:
						{
							NEWRECORD( m_pFileWriter ,new AVIFileWriter());
						}
						break;
					case WRITER_STANDARD_FORMAT_MKV:
						{
							NEWRECORD( m_pFileWriter ,new MKVFileWriter() );
						}
						break;
					case WRITER_STANDARD_FORMAT_MP4:
					default:
						{
							NEWRECORD( m_pFileWriter , new MP4FileWriter() );
						}
						break;
					}
					NULLReturnRet( m_pFileWriter , bRet ) 
		...
		if (m_nSampleRate!=pXAudioBuffer->nSampleRate || (m_bRenewFile && nTimestamp-m_nRenewFileTimestamp>=60000))
		{
			...
			if (m_pFileWriter==NULL)
			{
				...
				case WRITER_STANDARD_FORMAT_MKV:
					...
					break;
				case WRITER_STANDARD_FORMAT_MP3:
					{
						NEWRECORD( m_pFileWriter , new MP3FileWriter() );
					}
					break;
				case WRITER_STANDARD_FORMAT_MP4:
				default:
...
unsigned long AVIWriter::GetVideoBufferedMS(void)
{
	...
	{
		XVideoBuffer*pXVideoBuffer=(XVideoBuffer*)m_ListXVideoBuffer.front();
		unsigned long ulFirstTimestamp=VIDEC_HEADER_EXT_GET_TIMESTAMP(pXVideoBuffer->pBuffer);
		pXVideoBuffer=(XVideoBuffer*)m_ListXVideoBuffer.back();
		unsigned long ulLastTimestamp=VIDEC_HEADER_EXT_GET_TIMESTAMP(pXVideoBuffer->pBuffer);
		unsigned long ulDuration=ulLastTimestamp-ulFirstTimestamp+20;
		return ulDuration;
	}
	else
