...
VIDEC_Image*VIDEC_Image::Create(unsigned char*pData,int nLen,int nWidth,int nHeight,int nFrameRate)
{
	if (pData==NULL || nLen<=0)
		return NULL;

	unsigned char*pImageData=(unsigned char*)MALLOCMSG(nLen+(nLen>>2));
	if (pImageData!=NULL)
	{
		fastmemcpy(pImageData,pData,nLen);
		NEWRECORD3( XImage*,pXImage ,new XImage(pImageData,nLen) );
NULLReturnRet( pXImage , NULL ) 
		pXImage->AddRef();
...
VIDEC_Image*VIDEC_Image::CreateAutoFree(unsigned char*pData,int nLen,int nWidth,int nHeight,int nFrameRate,int nHeaderOffset)
{
	if (pData==NULL || nLen<=0)
		return NULL;

	NEWRECORD3( XImage*,pXImage ,new XImage(pData,nLen,nHeaderOffset) );
	NULLReturnRet( pXImage , NULL ) ;
...
XImage::XImage(void*pData, int nLen, int nBufOffset, int nAH2KNew)
: m_cRef(0)
...
XImage::~XImage(void)
{
	...
	{
		//SetMemCanUse((unsigned char*)m_pBuf);
	}
