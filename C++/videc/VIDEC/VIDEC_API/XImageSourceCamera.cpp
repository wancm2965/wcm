...
VIDEC_CameraDev*VIDEC_CameraDev::Create(void)
{
	//return new XImageSourceCamera();
	NEWRECORDret3(  VIDEC_CameraDev* ,p , new XImageSourceCamera()  )  
}
...
//取设备名称
int VIDEC_CameraDev::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
...
void XImageSourceCamera::SetOSDText(const char*cszText,int nAlign,int nTop,int nLeft)
{
	if (m_nDevID>=0)
	{
		XCameraDevMgr::SetOSDText(m_nDevID,cszText,nAlign,nTop,nLeft);
	}
}
...
int XImageSourceCamera::AddImageSink(VIDEC_ImageSink*pImageSink)
{
	return XImageSource::AddImageSink(pImageSink);
}

void XImageSourceCamera::RemoveImageSink(VIDEC_ImageSink*pImageSink)
{
	XImageSource::RemoveImageSink(pImageSink);
}

void XImageSourceCamera::OnAttachedToImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnAttachedToImageSource(pImageSource);
}

void XImageSourceCamera::OnDetachedFromImageSource(VIDEC_ImageSource*pImageSource)
{
	XImageSink::OnDetachedFromImageSource(pImageSource);
}

void XImageSourceCamera::OnReceivedPreSend(int& nSend)
{
	nSend = PreSendImage();
}

void XImageSourceCamera::OnReceivedImage(VIDEC_Image*pImage)
{
#ifdef _DEMO
