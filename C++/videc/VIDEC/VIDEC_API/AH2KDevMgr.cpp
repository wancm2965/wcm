...
{
	int nDevCount=0;
	nDevCount+=VIDEC_AH2K::GetDevCount();
	return nDevCount;
}
...
int AH2KDevMgr::AddStreamSink(unsigned int nDevIndex,VIDEC_StreamSink*pStreamSink)
{
	XAutoLock l(g_csMapAH2KDevInst);
	...
	if (iter!=g_MapAH2KDevInst.end())
	{
		if (iter->second.pAH2KDev==NULL)
		{
			...
			{
				NEWRECORD( pAH2KDev ,new AH2KDev(nDevIndex) );
				NULLReturnRet( pAH2KDev  , -1)  
...
void AH2KDevMgr::RemoveStreamSink(unsigned int nDevIndex,VIDEC_StreamSink*pStreamSink)
{
	XAutoLock l(g_csMapAH2KDevInst);
...
int AH2KDevMgr::GetDevName(unsigned int nDevIndex,char*szName,int nMaxCount)
{
	...
	if (nDevIndex<nDevCount)
	{
		return VIDEC_AH2K::GetDevName(nDevIndex,szName,nMaxCount);
	}
...
int AH2KDevMgr::GetDevType(unsigned int nDevIndex,VIDEC_DEV_TYPE&nDevType)
{
	unsigned int nDevCount=0;
	nDevCount=VIDEC_AH2K::GetDevCount();
	if (nDevIndex<nDevCount)
	{
		return VIDEC_AH2K::GetDevType(nDevIndex,nDevType);
	}
...
bool AH2KDevMgr::HasAudioInput(unsigned int nDevIndex)
{
	...
	if (nDevIndex<nDevCount)
	{
		return VIDEC_AH2K::HasAudioInput(nDevIndex);
	}
	return false;
}
...
bool AH2KDevMgr::SupportHardwareEncoding(unsigned int nDevIndex)
{
	...
	if (nDevIndex<nDevCount)
	{
		return VIDEC_AH2K::SupportHardwareEncoding(nDevIndex);
	}
	return false;
}

int AH2KDevMgr::AddImageSink(unsigned int nDevIndex,VIDEC_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapAH2KDevInst);
...
void AH2KDevMgr::RemoveImageSink(unsigned int nDevIndex,VIDEC_ImageSink*pImageSink)
{
	XAutoLock l(g_csMapAH2KDevInst);
...
void AH2KDevMgr::SetOSDText(unsigned int nDevIndex,const char*cszText,int nAlign,int nTop,int nLeft)
{
	XAutoLock l(g_csMapAH2KDevInst);
	...
	{
		pAH2KDev=iter->second.pAH2KDev;
		pAH2KDev->SetOSDText(cszText,nAlign,nTop,nLeft);
	}
}
