                ...
                int category_balances[MAX_NUM_CATEGORIZATION_CONTROL_POSSIBILITIES - 1])
{
    ...
    */
    ...
    do
    {
        ...
        for (region = 0;  region < number_of_regions;  region++)
             expected_number_of_code_bits += expected_bits_table1[power_categories[region]];
    ...
    for (i = 0;  i < num_categorization_control_possibilities - 1;  i++)
    {
        if (max + min <= 2*number_of_available_bits)
        {
            ...
            max -= expected_bits_table1[max_rate_categories[raw_min_index]];
        ...
        else
        {
            ...
            min -= expected_bits_table1[min_rate_categories[raw_max_index]];
