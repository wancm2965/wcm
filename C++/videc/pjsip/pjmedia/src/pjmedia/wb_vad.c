...
)
{
	...
	for (i = 0; i < frameSize; i++) {
		st->tmp_buf[i] = in[i]/2.0f;
	}
	/* run the filter bank */
	for (i = 0;i < frameSize/2; i++) {
		filter5(&st->tmp_buf[2*i],&st->tmp_buf[2*i+1],st->a_data5[0]);
	}
	for (i = 0;i < frameSize/4; i++) {
		filter5(&st->tmp_buf[4*i],&st->tmp_buf[4*i+2],st->a_data5[1]);
		filter5(&st->tmp_buf[4*i+1],&st->tmp_buf[4*i+3],st->a_data5[2]);
	}
	for (i = 0; i < frameSize/8; i++)
	{
		filter5(&st->tmp_buf[8*i], &st->tmp_buf[8*i+4], st->a_data5[3]);
		filter5(&st->tmp_buf[8*i+2], &st->tmp_buf[8*i+6], st->a_data5[4]);
		filter3(&st->tmp_buf[8*i+3],&st->tmp_buf[8*i+7],&st->a_data3[0]);
	}
	for (i = 0; i < frameSize/16; i++)
	{
		filter3(&st->tmp_buf[16*i+0], &st->tmp_buf[16*i+8], &st->a_data3[1]);
		filter3(&st->tmp_buf[16*i+4], &st->tmp_buf[16*i+12], &st->a_data3[2]);
		filter3(&st->tmp_buf[16*i+6], &st->tmp_buf[16*i+14], &st->a_data3[3]);
	}
	for (i = 0; i < frameSize/32; i++)
	{
		filter3(&st->tmp_buf[32*i+0], &st->tmp_buf[32*i+16], &st->a_data3[4]);
		filter3(&st->tmp_buf[32*i+8], &st->tmp_buf[32*i+24], &st->a_data3[5]);
	}
	/* calculate levels in each frequency band */
	/* 4800 - 6400 Hz*/
	level[11] = level_calculation(st->tmp_buf, &st->sub_level[11],
		frameSize/4-48, frameSize/4, 4, 1, 0.25);
	/* 4000 - 4800 Hz*/
	level[10] = level_calculation(st->tmp_buf, &st->sub_level[10],
		frameSize/8-24, frameSize/8, 8, 7, 0.5);
	/* 3200 - 4000 Hz*/
	level[9] = level_calculation(st->tmp_buf, &st->sub_level[9],
		frameSize/8-24, frameSize/8, 8, 3, 0.5);
	/* 2400 - 3200 Hz*/
	level[8] = level_calculation(st->tmp_buf, &st->sub_level[8],
		frameSize/8-24, frameSize/8, 8, 2, 0.5);
	/* 2000 - 2400 Hz*/
	level[7] = level_calculation(st->tmp_buf, &st->sub_level[7],
		frameSize/16-12, frameSize/16, 16, 14, 1.0);
	/* 1600 - 2000 Hz*/
	level[6] = level_calculation(st->tmp_buf, &st->sub_level[6],
		frameSize/16-12, frameSize/16, 16, 6, 1.0);
	/* 1200 - 1600 Hz*/
	level[5] = level_calculation(st->tmp_buf, &st->sub_level[5],
		frameSize/16-12, frameSize/16, 16, 4, 1.0);
	/* 800 - 1200 Hz*/
	level[4] = level_calculation(st->tmp_buf, &st->sub_level[4],
		frameSize/16-12, frameSize/16, 16, 12, 1.0);
	/* 600 - 800 Hz*/
	level[3] = level_calculation(st->tmp_buf, &st->sub_level[3],
		frameSize/32-6, frameSize/32, 32, 8, 2.0);
	/* 400 - 600 Hz*/
	level[2] = level_calculation(st->tmp_buf, &st->sub_level[2],
		frameSize/32-6, frameSize/32, 32, 24, 2.0);
	/* 200 - 400 Hz*/
	level[1] = level_calculation(st->tmp_buf, &st->sub_level[1],
		frameSize/32-6, frameSize/32, 32, 16, 2.0);
	/* 0 - 200 Hz*/
	level[0] = level_calculation(st->tmp_buf, &st->sub_level[0],
		frameSize/32-6, frameSize/32, 32, 0, 2.0);
}
/******************************************************************************
*
				 ...
				 )
{
	...
	*state = NULL;
	...
	// add by ryan liu
	if ( ( s->tmp_buf = (float *) malloc( sizeof(float)*frameSize ) ) == NULL )
	{
		fprintf(stderr, "vad_init: can not malloc tmp_buf\n");
		return -1;
	}
	memset(s->tmp_buf, 0, sizeof(float)*frameSize);

	//xmc add
	if ( ( s->in_buf = (float *) malloc( sizeof(float)*frameSize ) ) == NULL )
	{
		fprintf(stderr, "vad_init: can not malloc in_buf\n");
		return -1;
	}
	memset(s->in_buf, 0, sizeof(float)*frameSize);
	
	wb_vad_reset(s);

	s->frame_size = frameSize;  // add by ryan liu
...
short wb_vad_ext(VadVars * st, short in_buf[])
{
	...
	for(i=0; i<st->frame_size; i++)		
	{	
		st->in_buf[i] = (float)in_buf[i];
		if(st->in_buf[i]>65535/2)
			st->in_buf[i]=st->in_buf[i]-65536;
	}

	vad =  wb_vad(st,st->in_buf);
	//if( vad == 0 )
	//{
	//	st->m_nDelayCount ++;
	//	if( st->m_nDelayCount < 500 )
	//	{
	//		vad = 1;
	//	}
	//	else
