				       ...
				       pjmedia_snd_port *snd_port )
{
    ...
    /* Get device caps */
	// {{ liurui : 20120229 only capture or player device.
	if (snd_port->aud_param.dir & PJMEDIA_DIR_PLAYBACK) 
	{
		pjmedia_aud_dev_info dev_info;
		status = pjmedia_aud_dev_get_info(snd_port->aud_param.play_id, 
			&dev_info);
		if (status != PJ_SUCCESS)
		{
			snd_port->aud_caps = 0;
		...
		else
		{
			...
			if(dev_info.input_count > 0 && dev_info.output_count == 0)
			{
				snd_port->aud_param.clock_rate = 48000;
				snd_port->aud_param.samples_per_frame = 480;
	...
	status = pjmedia_snd_port_set_ec(snd_port, pool, 
		//{{xmc
					 snd_port->aud_param.ec_tail_ms, snd_port->prm_ec_options);
		//}}
#else
	status = pjmedia_snd_port_set_ec(snd_port, pool, 
					 snd_port->aud_param.ec_tail_ms,
					     ...
					     unsigned options)
{
	...
	if (snd_port->ec_state) {
	    //pjmedia_echo_destroy(snd_port->ec_state);
	    //snd_port->ec_state = NULL;
		return pjmedia_echo_set_options(snd_port->ec_state, options);
	}
