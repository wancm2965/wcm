				  ...
				  struct sdp_state *sdp_state)
{
	...
	//{{gaoshizhong 20121120 Add ice use
					 ...
					 unsigned media_index)
{
    ...
    if (tp_ice->rem_offer_state.match_comp_cnt==0) {
		//{{gaoshizhong 20121125 Add use to ice 因定时器还要使用ice所以去掉,要么是如对方没有ICE属性直接挂断处理
	//set_no_ice(tp_ice, "No ICE found in SDP offer", PJ_SUCCESS);
		//}}gaoshizhong 20121125 Add use to ice
	return PJ_SUCCESS;
    }
...
PJ_DEF(pj_status_t) pj_get_ice_enum_cands(pjmedia_transport *tp,
										   unsigned comp_id,
										   unsigned *count,
										   pj_ice_sess_cand cand[])
{
	struct transport_ice *tp_ice = (struct transport_ice*)tp;
	int status = pj_ice_strans_enum_cands(tp_ice->ice_st, comp_id,count, cand);
	//因有时会调set_no_ice会为空值所以不要调下面的
	//pj_ice_strans_comp *comp = tp_ice->ice_st->comp[comp_id - 1];
	//pj_memcpy(cand, &comp->cand_list[comp->default_cand], sizeof(pj_ice_sess_cand));
	return status;
}

//参看 pj_ice_sess_add_cand update_comp_check find_comp
pj_ice_sess_comp * ice_find_comp(const pj_ice_sess *ice, unsigned comp_id)
{
	pj_assert(comp_id > 0 && comp_id <= ice->comp_cnt);
	return (pj_ice_sess_comp*) &ice->comp[comp_id-1];
}

//注意版本变化要与pjnath/ice_strans.c定义相同
struct pj_ice_strans
{
	...
	void	   **comp;	/**< Components array.		*/ //此类型改为void
...
PJ_DEF(pj_status_t) pj_add_ice_valid_check(pjmedia_transport *tp,
										   pj_uint8_t comp_id,
										   ...
										   const pj_sockaddr_t *rem_addr)
{
	int status=-1;
	struct transport_ice *tp_ice = (struct transport_ice*)tp;
	struct pj_ice_sess *ice=tp_ice->ice_st->ice;
	pj_ice_sess_check * check;
	pj_ice_sess_comp *comp=NULL;

	check = PJ_POOL_ZALLOC_T(tp_ice->pool, struct pj_ice_sess_check);
	check->lcand=PJ_POOL_ZALLOC_T(tp_ice->pool, struct pj_ice_sess_cand);
	check->rcand=PJ_POOL_ZALLOC_T(tp_ice->pool, struct pj_ice_sess_cand);
	//设置参看pj_ice_sess_send_data
	check->lcand->comp_id=check->rcand->comp_id=comp_id;
	check->lcand->transport_id=check->rcand->transport_id=transport_id;
	check->lcand->type=check->rcand->type=type;

	pj_sockaddr_cp(&check->lcand->addr, local_addr);
	pj_sockaddr_cp(&check->rcand->addr, rem_addr);
	if(ice)
		comp = ice_find_comp(ice, comp_id);
	if(comp)
	{
...
//Parse a=candidate line parse_cand
PJ_DEF(pj_status_t) pj_parse_cand(pjmedia_transport *tp,
							  pj_pool_t *pool,
							  const pj_str_t *orig_input,
							  pj_ice_sess_cand *cand)
{
	return parse_cand(tp->name,pool,orig_input,cand);
}
