								 ...
								 pjmedia_frame *frame )
{
	...
	*/
	...
	{
		/* Encode! */
		status = pjmedia_codec_encode( stream->codec, frame, channel->out_pkt_size - 
			sizeof(pjmedia_rtp_hdr),
			&frame_out);
		if (status != PJ_SUCCESS) 
		{
...
#undef M
    ...
    *p++ = '.'; *p++ = 'o'; *p++ = 'r'; *p++ = 'g';
	...
	//{
	//	if( stream->port.pVadVars == NULL && ( GetDummyEngineFlag() == 1 ) )
	//	{
	//		int sampleCount = (stream->codec_param.info.clock_rate/1000)*stream->codec_param.info.frm_ptime;
	//		wb_vad_init(&(stream->port.pVadVars), sampleCount);
	//	}
	//}
	// }}
	...
	*/
    ...
    status = create_channel( pool, stream, PJMEDIA_DIR_DECODING, 
		info->fmt.pt, info, &stream->dec); //liurui
...
PJ_DEF(pj_status_t) pjmedia_stream_destroy( pjmedia_stream *stream )
{
	...
	// {{liurui
	if(stream->port.pVadVars != NULL )
	{
		wb_vad_exit(&(stream->port.pVadVars));
		stream->port.pVadVars = NULL;
	}
	// }}
...
{
	stream->vad_enabled=vad;

	stream->vad_enabled= 1; //liurui !
	if( GetDummyEngineFlag() == 1 ) //liurui add for DummyEngine
		stream->vad_enabled = 0;
	return  PJ_SUCCESS;
}