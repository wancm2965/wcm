					 ...
					 pjmedia_conf **p_conf )
{
	...
	// {{ liurui
	//if( GetDummyEngineFlag() == 1 )
	//	wb_vad_init(&(conf->master_port->pVadVars), samples_per_frame);
	// }}
...
PJ_DEF(pj_status_t) pjmedia_conf_destroy( pjmedia_conf *conf )
{
	...
	// {{liurui
	if (conf->master_port->pVadVars!=NULL)
	{
		wb_vad_exit(&(conf->master_port->pVadVars));
		conf->master_port->pVadVars = NULL;
	}
	// }}
