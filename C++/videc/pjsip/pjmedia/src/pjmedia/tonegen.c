				     ...
				     pjmedia_frame *frame)
{
	...
	if (tonegen->dig_samples>=(tonegen->digits[tonegen->cur_digit].on_msec+
		tonegen->digits[tonegen->cur_digit].off_msec)* clock_rate / 1000)
	{
	...
	while (dst < end) 
	{
		...
		/* Init tonegen */
		if (tonegen->dig_samples == 0 && (tonegen->count!=1 || !(dig->flags & PJMEDIA_TONE_INITIALIZED)))
		{
		...
		if (tonegen->dig_samples < on_samp) 
		{
			...
			if (cnt > required)
				cnt = required;
			generate_tone(&tonegen->state, PJMEDIA_PIA_CCNT(&port->info), cnt, dst);

			dst += cnt;
			tonegen->dig_samples += cnt;
			required -= cnt;

			if ((dig->flags & PJMEDIA_TONE_ENABLE_FADE) && tonegen->dig_samples == cnt) 
			{
				/* Fade in */
				short *samp = (dst - cnt);
				short *end;
			...
			} else if ((dig->flags & PJMEDIA_TONE_ENABLE_FADE) && tonegen->dig_samples==on_samp) 
			{
				/* Fade out */
	...
	// liurui Testing
	//{
	//	short * pSample = (short*)frame->buf;
	//	DetectProcess(pSample);
	//}
	...
	//if( m_pToneFile != NULL )
	//{
	//	fwrite(frame->buf, 1, frame->size, m_pToneFile);
	//}
