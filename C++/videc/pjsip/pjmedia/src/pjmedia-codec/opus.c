...
/* Prototypes for OPUS codec factory */
...
/* Prototypes for OPUS codec implementation. */
...
static pjmedia_codec_factory_op codec_factory_op =
{
	...
	&alloc_codec,
	&dealloc_codec,
	&pjmedia_codec_opus_deinit //liurui add for 2.0
};


/* Structure of OPUS mode */
...
static struct codec_factory {
	...
	unsigned		     mode_count;    /**< Number of OPUS modes.    */
	codec_mode		     modes[MAX_CODEC_MODES]; /**< The OPUS modes. */
	unsigned		     mode_rsv_start;/**< Start index of OPUS non-
...
/* 
* Helper function to validate OPUS mode. Valid modes are defined as:
...
static pj_bool_t validate_mode(unsigned sample_rate, unsigned bitrate)
{
	...
	else if (sample_rate == FWB_SAMPLE_RATE) 
	{
...
PJ_DEF(pj_status_t) pjmedia_codec_opus_init( pjmedia_endpt *endpt )
{
	...
	//mode->pt = PJMEDIA_RTP_PT_OPUS_24;
	...
	//mode = &codec_factory.modes[codec_factory.mode_count++];
	//mode->enabled = PJ_TRUE;
	//mode->pt = PJMEDIA_RTP_PT_OPUS_32;
	//mode->sample_rate = WB_SAMPLE_RATE;
	//mode->bitrate = 32000;
	//pj_utoa(mode->bitrate, mode->bitrate_str);
	...
	mode = &codec_factory.modes[codec_factory.mode_count++];
	mode->enabled = PJ_TRUE;
	mode->pt = PJMEDIA_RTP_PT_OPUS_48;
	mode->sample_rate = FWB_SAMPLE_RATE;
	mode->bitrate = 48000;
	pj_utoa(mode->bitrate, mode->bitrate_str);
	...
	/* Create OPUS codec factory. */
	codec_factory.base.op = &codec_factory_op;
	codec_factory.base.factory_data = NULL;
	codec_factory.endpt = endpt;

	codec_factory.pool = pjmedia_endpt_create_pool(endpt, "OPUS codec",
		4000, 4000);
	if (!codec_factory.pool)
		return PJ_ENOMEM;

	/* Create mutex. */
	status = pj_mutex_create_simple(codec_factory.pool, "OPUS codec",
		&codec_factory.mutex);
...
/**
* Enable and disable OPUS modes, including non-standard modes.
*/
PJ_DEF(pj_status_t) pjmedia_codec_opus_set_mode(unsigned sample_rate, 
												unsigned bitrate, 
												pj_bool_t enabled)
{
	unsigned i;
							  ...
							  const pjmedia_codec_info *info )
{
	...
	if (info->clock_rate != WB_SAMPLE_RATE && 
		...
		info->clock_rate != FWB_SAMPLE_RATE)
	{
		return PJMEDIA_CODEC_EUNSUP;
	}
							   ...
							   pjmedia_codec **p_codec)
{
	...
	pool = pjmedia_endpt_create_pool(codec_factory.endpt, "opus", 512, 512);
							  ...
							  pjmedia_codec_param *attr )
{
	...
	codec_data->enc_st= OpenOpusEncoder(1, attr->info.clock_rate, codec_data->bitrate, codec_data->samples_per_frame);//g722_1_encode_init(NULL,codec_data->bitrate,attr->info.clock_rate);

	/* Initialize decoder state */
	codec_data->dec_st= OpenOpusDecoder(1, attr->info.clock_rate);//g722_1_decode_init(NULL,codec_data->bitrate,attr->info.clock_rate);
...
static pj_status_t codec_close( pjmedia_codec *codec )
{
	...
	{
		CloseOpusEncoder(codec_data->enc_st);
		codec_data->enc_st=NULL;
	}
	if (codec_data->dec_st!=NULL)
	{
		CloseOpusDecoder(codec_data->dec_st);
...
struct pjmedia_frame *output)
{
	...
	codec_data->frame_size = (pj_uint16_t)OpusEncoderFrame(codec_data->enc_st, input->buf, codec_data->samples_per_frame,  output->buf);
;
...
struct pjmedia_frame *output)
{
	...
	if (input) {
		...
		OpusDecoderFrame(codec_data->dec_st, input->buf, codec_data->frame_size , output->buf);
		output->timestamp = input->timestamp;
	...
	else 
	{
		...
		OpusFillInFrame(codec_data->dec_st, NULL, 0, output->buf);
		//output->timestamp = input->timestamp;
	}
