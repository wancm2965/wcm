...
//#include "../../../AUDEC_agc/XAGCExport.h"
#define WEBRTC_SPL_WORD16_MAX       32767
#define WEBRTC_SPL_WORD16_MIN       -32768
#define WEBRTC_SPL_SAT(a, b, c)         (b > a ? a : b < c ? c : b)

//FILE * encPcmFile = NULL;
...
#include "../../../third_party/g7221/g722_1.h"
//#include "../../../../AUDEC/AUDEC_G7221/g722_1.h"
...
static pjmedia_codec_factory_op codec_factory_op =
{
	...
	&alloc_codec,
	&dealloc_codec,
	&pjmedia_codec_g7221_deinit //liurui add for 2.0
};
...
typedef struct codec_private {
	...
	*/
	...
	g722_1_encode_state_t* enc_st;

	/* Decoder specific state */
	g722_1_decode_state_t*	dec_st;

	// liurui add
	void * playAgcInstance;

	int m_mixerFileCount;
							  ...
							  pjmedia_codec_param *attr )
{
	...
	codec_data->enc_st=g722_1_encode_init(NULL,codec_data->bitrate,attr->info.clock_rate);

	/* Initialize decoder state */
	codec_data->dec_st=g722_1_decode_init(NULL,codec_data->bitrate,attr->info.clock_rate);

	// liurui
	//if( attr->info.clock_rate == 16000) //Polycom
	//	codec_data->playAgcInstance = OpenPlayAGC(codec_data->dec_st->frame_size>>1, attr->info.clock_rate );

	//m_gMixerCount ++;
	//codec_data->m_mixerFileCount = m_gMixerCount;
	return PJ_SUCCESS;
}
...
static pj_status_t codec_close( pjmedia_codec *codec )
{
	...
	{
		g722_1_encode_release(codec_data->enc_st);
		codec_data->enc_st=NULL;
	}
	if (codec_data->dec_st!=NULL)
	{
		g722_1_decode_release(codec_data->dec_st);
		codec_data->dec_st=NULL;
	}

	// liurui
	//if (codec_data->playAgcInstance!=NULL)
	//{
	//	ClosePlayAGC(codec_data->playAgcInstance);
	//	codec_data->playAgcInstance=NULL;
	//}
...
struct pjmedia_frame *output)
{
	...
	//if( encPcmFile != NULL && codec_data->m_mixerFileCount == 1)
	//	fwrite(input->buf, 1, input->size,  encPcmFile);
	...
	//{
	//	short * inData = (short*)input->buf;
	//	inData[i] = (inData[i] >> 1 );
	//}

	g722_1_encode(codec_data->enc_st,output->buf,input->buf,codec_data->samples_per_frame);
								...
								struct pjmedia_frame *output)
{
	...
	if (input) {
		...
		g722_1_decode(codec_data->dec_st,output->buf,input->buf,codec_data->frame_size);
		output->timestamp = input->timestamp;

	} else {
		uint8_t tmp[MAX_FRAME_SIZE]={0};
		g722_1_fillin(codec_data->dec_st,output->buf,tmp,codec_data->frame_size);
	}


	//for (i=0; i<codec_data->samples_per_frame; ++i) 
	//{
	//	short * outData = (short*)output->buf;
