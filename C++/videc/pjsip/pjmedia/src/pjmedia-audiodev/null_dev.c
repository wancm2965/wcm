...
#include <windows.h>
static CRITICAL_SECTION m_NullAudioCritSec;
...
// {{ liurui
static struct null_audio_stream * m_gNullStream = NULL;
					      ...
					      pjmedia_aud_stream **p_aud_strm)
{
    ...
    *p_aud_strm = &strm->base;
	...
	// {{liurui
	m_gNullStream = (struct null_audio_stream*)&strm->base;
	strm->rec_thread_exited = 1;
	strm->play_thread_exited = 1;
	InitializeCriticalSection(&m_NullAudioCritSec);
	// }}
...
static pj_status_t null_stream_destroy(pjmedia_aud_stream *strm)
{
	...
	m_gNullStream = NULL;  
	DeleteCriticalSection(&m_NullAudioCritSec);
...
void NullDeviceCaptureCallBack(short * data, unsigned int len)
{
	pjmedia_frame pcm_frame, *frame;
	struct null_audio_stream *strm = (struct null_audio_stream*)m_gNullStream;
	if(strm == NULL)
	{
		if(data != NULL)
	...
	if (strm->rec_thread_initialized == 0 || !pj_thread_is_registered()) 
	{
		...
		pj_thread_register("null_rec", strm->rec_thread_desc, 
			&strm->rec_thread);
	...
	if(strm != NULL)
	{
		...
		EnterCriticalSection(&m_NullAudioCritSec);
		(*strm->rec_cb)(strm->user_data, frame);
		LeaveCriticalSection(&m_NullAudioCritSec);
	}
	return;
...
void NullDevicePlayCallBack(unsigned char * data, unsigned int len)
{
	pjmedia_frame pcm_frame, *frame;
	struct null_audio_stream *strm = (struct null_audio_stream*)m_gNullStream;
	if(strm == NULL)
	...
	if (strm->play_thread_initialized == 0 || !pj_thread_is_registered()) 
	{
		...
		pj_thread_register("null_play", strm->play_thread_desc,
			&strm->play_thread);
