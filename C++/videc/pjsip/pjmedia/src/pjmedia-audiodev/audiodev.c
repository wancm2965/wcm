...
#define PJMEDIA_AUDIO_DEV_HAS_AUDEC 1

#define DEFINE_CAP(name, info)	{name, info}

static unsigned int m_gIsDummyEngine = 0;

void SetDummyEngineFlag( int nFlag )
{
	m_gIsDummyEngine = nFlag;
}

unsigned int GetDummyEngineFlag( void )
{
	return m_gIsDummyEngine;
}
...
#if PJMEDIA_AUDIO_DEV_HAS_AUDEC
...
#if PJMEDIA_AUDIO_DEV_HAS_AUDEC
	if( m_gIsDummyEngine == 0 )
		aud_subsys.drv[aud_subsys.drv_cnt++].create = &pjmedia_audec_factory;
	else
					      ...
					      pjmedia_aud_stream **p_aud_strm)
{
	...
	if (param.dir & PJMEDIA_DIR_CAPTURE) {
		...
		//if (status != PJ_SUCCESS)
		//    return status;
		if(status == PJ_SUCCESS) //liurui modify
		{
			param.rec_id = index;
			f = rec_f;
		}
