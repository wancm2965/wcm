...
//{{gaoshizhong 20130428 Add
SIPMSG_Change_CallBackT * SIPMSG_Change_CallBack=NULL;
PJ_DEF(void) SetSIPMSGChangeHandle(SIPMSG_Change_CallBackT SIPMSGChange)
{
	if(SIPMSGChange!=NULL)
		SIPMSG_Change_CallBack=SIPMSGChange;
}
//}}gaoshizhong 20130428 Add
				    ...
				    char *buf, pj_size_t size)
{
	...
	*p = '\0';
	...
	//{{gaoshizhong 20130521 Add
	if(SIPMSG_Change_CallBack!=NULL)
	{
		int nMsgLen=SIPMSG_Change_CallBack(buf,p-buf,msg);
		if(nMsgLen>0) return nMsgLen;
	}
	//}}gaoshizhong 20130521 Add
...
//{{gaoshizhong 20141106 Add
PJ_DEF(int) pjsip_get_status_code(pj_str_t * pjStatus)
{
	int nCount =0 ;
	int nCode =0;
	int i=0;
	...
	nCount =(int)(sizeof(status_phrase)/sizeof(status_phrase[0]) ) ;
	nCode =0;
	for(i=0;i<nCount;i++)
	{
		if(pj_stricmp(&status_phrase[i],pjStatus) ==0)
		{
			nCode =i;
			break;
		}
