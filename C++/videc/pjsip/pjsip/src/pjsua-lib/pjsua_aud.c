				     ...
				     const pjmedia_sdp_session *remote_sdp)
{
	...
	if (pjsua_var.media_cfg.no_vad) {
	    ...
	    si->param->setting.cng = 0;
	}
	//{{xmc 20091010
	else
	{
		si->param->setting.vad = 1;
		si->param->setting.cng = 1;
	}
	//}}
				    ...
				    unsigned bits_per_sample)
{
	...
	status = pjmedia_aud_dev_default_param(playback_dev, param); 
    if (status != PJ_SUCCESS) 
	{
		NoDevFlag ++;
    }
...
#if 1
	if (!pjsua_var.is_mswitch && param->base.ext_fmt.id == PJMEDIA_FORMAT_PCM && PJMEDIA_PIA_SRATE(&conf_port->info) != 48000)
	{
