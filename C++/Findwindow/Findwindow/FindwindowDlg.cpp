...
{
	// TODO: 在此添加控件通知处理程序代码
	CWnd* hwn = FindWindow(_T("CAvconStartupDlg"),NULL);
	if (hwn)
	{
		::MessageBox(NULL,_T("123"),_T("333"),MB_OK);
	}
	else
	{
		hwn = FindWindow(NULL,_T("CAvconStartupDlg"));
		if (hwn)
		{
			::MessageBox(NULL,_T("333"),_T("666"),MB_OK);
		}
		::PostMessage(hwn->GetSafeHwnd(),WM_QUIT,0,0);
	}
}
