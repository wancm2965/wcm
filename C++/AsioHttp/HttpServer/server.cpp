...
server::server(const std::string& address, const std::string& port,
  ...
  : io_context_pool_(io_context_pool_size),
    ...
    new_connection_(),
	request_handlers_(io_context_pool_size,doc_root)
{
...
void server::start_accept()
{
  new_connection_.reset(new connection(
        io_context_pool_.get_io_context(), request_handlers_.get_request_handler()));
...
void server::AsyncReply(void* handler, const char* reply)
{
	connection_ptr connet((connection*)handler);
	connet->AsyncReply(reply);
}
} // namespace server2
} // namespace http
