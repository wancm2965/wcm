...
//#include "FFmpegrecord/AmfByteStream.h"
...
int Se(BYTE *pBuff, UINT nLen, UINT &nStartBit)
{
    ...
    double k=UeVal;
    int nValue=ceil(k/2);//ceil函數：ceil函數的作用是求不小於给定實數的最小整數。ceil(2)=ceil(1.2)=cei(1.5)=2.00
    if (UeVal % 2==0)
        nValue=-nValue;
    return nValue;
...
void ParseH264Frame(const char* nalsbuf, int size, char* outBuf, int& outLen,
    ...
    bool& isKeyframe, int& width, int& height,int& nfps)
{
    int tmp_len = 0;
    ...
    while (start < end) 
    {
        ...
        if (nal_type == 7 && spsBuf)        /* SPS */
        {
			...
			h264_decode_seq_parameter_set((BYTE*)start + 4, size, width, height, nfps);
        }
...
bool GetWidthHeightFromFrame(const char* frameBuf, int bufLen, int& width, int& height)
{
    ...
    while (nal_start < end)
    {
        ...
        {
			int nfps = 0;
			h264_decode_seq_parameter_set((BYTE*)nal_start, nal_size, width, height, nfps);
            return true;
        }
        nal_start = nal_end;
    }
