...
class KBASE_API KFormatDateTime
{
public:
	KFormatDateTime();
	~KFormatDateTime();
	...
	void GetDate(std::string& strDate)
	{
		char szBuffer[128];
#ifdef WIN32
		sprintf_s(szBuffer,"%04d-%02d-%02d",m_nYear,m_nMonth,m_nMDay);
#else
	...
	void GetTime(std::string& strTime)
	{
		char szBuffer[128];
#ifdef WIN32
		sprintf_s(szBuffer,"%02d:%02d:%02d",m_nHour,m_nMinute,m_nSecond);
#else		
	...
	void GetDateTime(std::string& datetime)
	{
		char szBuffer[128];
#ifdef WIN32
		sprintf_s(szBuffer,"%04d-%02d-%02d %02d:%02d:%02d",m_nYear,m_nMonth,m_nMDay,m_nHour,m_nMinute,m_nSecond);
#else
...
// KDateTime 类定义
//=======================================================================================
KBASE_API void GetNow(KFormatDateTime& datetime);
KBASE_API void GetFormatDateTime(const std::string& strDateTime,KFormatDateTime& datetime);
KBASE_API bool KYMD2JDay(int year,int month,int day,unsigned long& jday);
KBASE_API void KJDay2YMD(unsigned long jday,KFormatDateTime& datetime);
KBASE_API void KGetWeekDay(int year,int month,int day,int& weekday);
