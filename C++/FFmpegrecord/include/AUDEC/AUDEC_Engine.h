...
// {{ liurui
class AUDEC_ENGINE_API AUDEC_DTMFCallback
{
public:
	AUDEC_DTMFCallback(void){};
	virtual~AUDEC_DTMFCallback(void){};
public:
	virtual void OnRecvDTMF(char dtmf)=0;
};
// }}
...
public:
	...
	virtual int SetDTMFCallback(AUDEC_DTMFCallback * pDTMFCallback)=0; //liurui
	virtual float GetDBLevel(void)=0; //2014/11/13
};
...
#define AUDEC_DTMF_INBAND			1
	...
	virtual int AUDEC_SetMicphoneMute(int nMute)=0;
	...
	static int AUDEC_GetMicLevel(void);

	virtual float AUDEC_GetMicphoneDB(void) = 0;
	virtual float AUDEC_GetSpeakerDB(void) = 0;
	virtual int   AUDEC_GetMicphoneMute(void) = 0;

	// {{ liurui 2013/03/14
	virtual int AUDEC_SetSpeakerMute(int nMute)=0;
	virtual int AUDEC_GetSpeakerMute(void) = 0;
	// }}
