...
/*-------------------------------------------------------------------*
 ...
 |  Output: TableType Code else TAB_NOTSET                           |
 *-------------------------------------------------------------------*/
DWORD dxfFindNextTableType( HDXF hDxf ) //函数作用是用来查找Table中Type的value类型
{
	PDXF	pDxf;
	DWORD	result;
	...
	// Find TableType Name
	if(FindParamFromDxfFile(pDxf, 0, "TABLE")) //寻找tables段中type类型  在DXF中查找key和value
	{
		// Read TableType Name
		ReadTableTypeName(pDxf); //查找table段下面的key和value 常见有DIMSTYLE,LTYPE,LSTYLE,LAYER
		result = pDxf->Read.CurrentTableType; //返回当前的table类型value
	}
	else
		result = TAB_NOTSET;
...
BOOL ReadDimStyleData( PDXF pDxf, PDXFDIMSTYLE pDimStyle )
{
	...
	while(GCode!=0)
	{
		...
		case 2:		// DimStyle Name
			strcpy_s(pDimStyle->Name, strValue);
			break;
		...
		case 6:		// 1st Arrow head
			strcpy_s(pDimStyle->DIMBLK1, strValue);
			break;
		case 7:		// 2nd Arrow head
			strcpy_s(pDimStyle->DIMBLK2, strValue);
			break;
		case 41:	// Arrow size
...
BOOL ReadLayerData( PDXF pDxf, PDXFLAYER pLayer )
{
	...
	while(GCode!=0)
	{
		...
		case 62:	// Layer Color
			...
			break;
		case 6:		// Linetype
			strcpy_s(pLayer->LineType, strValue);
			break;
 		}
		dxfStorePos(pDxf);
		ReadParamFromDxfFile(pDxf, GCode, strValue);
...
BOOL ReadStyleData( PDXF pDxf, PDXFSTYLE pStyle )
{
	...
	while(GCode!=0)
	{
		...
		{
		case 2:		// Style Name
			strcpy_s(pStyle->Name, strValue);
			break;
		case 3:		// Primary Font Filename
			strcpy_s(pStyle->PrimaryFontFilename, strValue);
			break;
		case 70:	// Standard flags
		...
		case 50:	// Oblique angle
			...
			break;
		case 1000: // Face name
			strcpy_s(pStyle->FaceName, strValue);
			break;
		}
		dxfStorePos(pDxf);
		ReadParamFromDxfFile(pDxf, GCode, strValue);
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TableType Code else TAB_NOTSET                           |
 *-------------------------------------------------------------------*/
DWORD dxfReadTableType( HDXF hDxf, LPVOID pTableType ) //函数作用是读取table段中各个type中的数据
{
	PDXF	pDxf;
	int		GCode;
	char	strValue[2048];
	...
	if((GCode==0) && (strcmp(strValue, "ENDTAB")==0)) //循环查找,直到一个table段type解析完成,以组码0和value值为ENDTAB结束
	{
