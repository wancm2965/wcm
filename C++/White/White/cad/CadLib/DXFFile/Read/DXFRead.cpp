...
HDXF dxfOpenFile( LPCTSTR FileName )
{
	...
	pDxf->Read.BufferDataSize = fread(pDxf->Read.ReadBuffer, 1, READBUFFERSIZE, pDxf->pStream);//从pStream读数据到ReadBuffer

	GlobalUnlock(hDxf);
	//OutputDebugString(pDxf->Read.ReadBuffer);
 ...
 *-------------------------------------------------------------------*/
DWORD dxfFindNextSection( HDXF hDxf ) /*此函数用来查找DXF中的各个段,  输出自定义的组码DWORD整数*/
{
	PDXF	pDxf;
	int		GCode;
	char	SectionName[32];
