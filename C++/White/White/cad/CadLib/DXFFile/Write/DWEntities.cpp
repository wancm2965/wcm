...
/*BOOL dxfSPline(HDXF hDxf,REALPOINT NormalVecetor,              
	int SplineMarks,                      
	int SplineOrder,                   
	int NodeNum,	                     
	int ControlNodeNum,                   
	...
	double *pNodePoint,                  
	double Weight,                         
	SimpleList<REALPOINT> *pControlNode,                 
...
BOOL dxfLWPolyLine( HDXF hDxf, PDXFENTVERTEX pVertex, int nVertex, unsigned short Flag )
{
	PDXF	pDxf;
	BOOL	result;
	...
	result = TRUE;
	// Entity type
	result &= dxfWriteParamString(hDxf, 0, "LWPOLYLINE");
	// Layer Name
	result &= dxfWriteParamString(hDxf, 8, pDxf->Write.CurrentLayer);
	// Line Color
	if((pDxf->Write.CurrentColor>0) && (pDxf->Write.CurrentColor<256))
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfDimLinear( HDXF hDxf, double x1, double y1, double x2, double y2, double x3, double y3, double angle, LPCWSTR text )
{
	PDXF			pDxf;
	DXF				oldDxf;
	BOOL			result;
	...
	case 0: // Centered
		...
		TextWidth = (wcslen(text) * pDimStyle->DIMTXT * 0.3) + pDimStyle->DIMGAP*2;
