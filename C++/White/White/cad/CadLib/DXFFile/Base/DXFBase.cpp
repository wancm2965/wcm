 ...
 *-------------------------------------------------------------------*/
BOOL dxfWriteParamString( HDXF hDxf, int GroupCode, LPCTSTR value )//将组码写入dxf流文件中
{
	PDXF	pDxf;
...
*/
	...
	ReadLine(pDxf, strGroupCode);  //strGroupCode为输出值
	GroupCode = atoi(strGroupCode);
	ReadLine(pDxf, valuestr);//用过valuestr之后对其进行改动
