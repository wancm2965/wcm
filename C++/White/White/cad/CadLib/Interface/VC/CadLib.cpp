...
{
	ISLIBRARYLOADED();
	return drwPaint(m_hDrawing, hdc, 0x000000);
}

double CDrawing::GetZoomLevel( )//获得缩放的比例
{
	ISLIBRARYLOADED();
...
BOOL CDrawing::SetZoomLevel(double ZoomLevel)
{
	...
	if(drwGetViewProperties(m_hDrawing, &drwview))
	{
		double cx = double(drwview.ViewLeft + ((drwview.WindowRight-drwview.WindowLeft)/2)*(1/(drwview.PPU*drwview.ZoomLevel)));
		double cy = double(drwview.ViewBottom + ((drwview.WindowBottom-drwview.WindowTop)/2)*(1/(drwview.PPU*drwview.ZoomLevel)));
...
// Configuration ---------------------------------------------------
BOOL CDrawing::SetLayer( LPCTSTR Name )//设置图层
{
	LAYER Layer;
...
OBJHANDLE CDrawing::InsertBlock( OBJHANDLE BlockObjhandle, double x, double y, double xScale, double yScale, double Rotation )
{
	...
	BlockHeader.Objhandle = BlockObjhandle;
	if (drwFindBlock(m_hDrawing, FIND_BYHANDLE, &BlockHeader) > 0)
	{
		strcpy(insert.BlockHeaderName, BlockHeader.Name);
	}
	insert.Point0.x = x;
	insert.Point0.y = y;
...
//	char		GenerationFlag;
	...
	text.TextData.Point1.x = x2;
	text.TextData.Point1.y = y2;
