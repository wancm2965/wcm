 ...
 *-------------------------------------------------------------------*/
BOOL GetDimRect(PDRAWING pDrawing, PENTITYHEADER pEntityHeader, PENTDIMENSION pDim, REALRECT* pRect)//获得标注的尺寸
{
	DIMSTYLE	DimStyle;
...
BOOL DimLinear( PDRAWING pDrawing, OBJHANDLE BlockObjhandle, PENTITYHEADER pDimHeader, PENTDIMENSION pDimension, PDIMSTYLE pDimStyle )
{
	...
	LPCWSTR			text = pDimension->DimText;				// dim-line's text (dafault is NULL)
	...
	//case 0: // Centered
		...
		Text.TextData.Point1.x = C1.x; Text.TextData.Point1.y = C1.y;
		...
		wcscpy_s(Text.strText, pDimension->DimText);
	...
	//	Text.TextData.WidthFactor = 0;
	//	Text.TextData.TextStyleObjhandle = pDimStyle->dimtxstyObjhandle;
	//	wcscpy(Text.strText, pDimension->DimText);
	//	drwAddEntity_Direct(pDrawing, BlockObjhandle, &EntityHeader, &Text);
	//	break;
	//}
