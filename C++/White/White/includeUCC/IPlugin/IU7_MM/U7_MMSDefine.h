...
typedef struct _MMS_H323SET
{
	...
	std::string		strDescp;		// 描述
	std::string		strCaption;		// 设备型号
	unsigned int	ulType;		// 设备类别：0是终端，1是MCU
	std::string		strIP;			// IP地址
	std::string		strCallID;		// 呼叫号
	unsigned long	ulConferenceID; // 呼叫的会议ID
	std::string		strMPID;		// 呼叫的MPID
...
{
	std::string		devid;			// 名称@域(创建设备时自动带上本域名)
	unsigned short	type;			// 设备类别：0是终端，1是MCU
	std::string		ipaddr;			// IP地址
	std::string		callid;			// 呼叫号
	unsigned long	conferenceid;	// 呼叫的会议ID
	std::string		desc;			// 描述
	_MMS_H323DEVICE()
	{
		type=0;
