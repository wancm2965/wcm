		...
		if ((lpDrawItemStruct->itemState & ODS_SELECTED)||m_Checked){
			...
			if(m_bNormal.m_hObject==NULL)
                ...
                if ((m_tracking)&&(m_bOver.m_hObject!=NULL))
					...
					if ((lpDrawItemStruct->itemState & ODS_CHECKED)&&(m_bFocus.m_hObject!=NULL))
...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetSkin(HBITMAP normal,HBITMAP down, HBITMAP mask,
						   HBITMAP over,  HBITMAP focus,HBITMAP disabled,
						   short drawmode,short border,short margin)
{
	...
	m_bMask.DeleteObject();	m_bFocus.DeleteObject();
	...
	if(focus==NULL)
		focus=down;
	m_bFocus.Attach(focus);
	if(disabled==NULL)
		disabled=normal;
	m_bDisabled.Attach(disabled);
...
void CxSkinButton::SetDispText(BOOL btext)
{
	m_bDispText=btext;
}
...
void CxSkinButton::AddTool(LPCTSTR strTip)
{
	if(m_tooltip.m_hWnd==NULL){
		if(m_tooltip.Create(this))	//first assignment
			m_tooltip.SetDelayTime(TTDT_INITIAL,1); //m_tooltip.AddTool(this, strTip);
	}
	strTipText = strTip;
...
void CxSkinButton::LoadBmpFromFile(CString bmpnormal,CString bmpfocus,CString bmpmask)
{
//	HBITMAP hbitmap=NULL;
	m_strbmpNormal = bmpnormal;
	m_strbmpFocus = bmpfocus;
	m_strbmpMask = bmpmask;

}

void  CxSkinButton::SetButtonFocus()
{

	m_hBitmapFocus =(HBITMAP)::LoadImage(NULL,m_strbmpFocus,IMAGE_BITMAP,0,0,LR_LOADFROMFILE ) ;
	if(m_hBitmapFocus==NULL){
		m_hBitmapFocus =(HBITMAP)::LoadImage(NULL,m_strbmpFocus,IMAGE_BITMAP,0,0,LR_LOADFROMFILE ) ;
	}

	m_hBitmapMask =(HBITMAP)::LoadImage(NULL,m_strbmpMask,IMAGE_BITMAP,0,0,LR_LOADFROMFILE ) ;
	if(m_hBitmapMask==NULL){
		m_hBitmapMask =(HBITMAP)::LoadImage(NULL,m_strbmpMask,IMAGE_BITMAP,0,0,LR_LOADFROMFILE ) ;
	}
	 SetSkin(m_hBitmapFocus,m_hBitmapFocus,m_hBitmapMask,m_hBitmapFocus,m_hBitmapFocus,m_hBitmapFocus,0,0,0);
	 Invalidate(FALSE);
}
void  CxSkinButton::SetButtonNormal()
{
	m_hBitmapNormal =(HBITMAP)::LoadImage(NULL,m_strbmpNormal,IMAGE_BITMAP,0,0,LR_LOADFROMFILE ) ;
	...
	if(m_hBitmapMask==NULL){
	 ...
	 SetSkin(m_hBitmapNormal,m_hBitmapNormal,m_hBitmapMask,m_hBitmapNormal,m_hBitmapNormal,m_hBitmapNormal,0,0,0);
	 Invalidate(FALSE);
}
