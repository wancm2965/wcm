...
BOOL CListBoxEBX::ShowWindow(int nFlag)
{
	if (nFlag==SW_SHOW)
	{
		SetCapture();
...
void CListBoxEBX::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if((lpDIS->itemAction & ODA_FOCUS) && (lpDIS->itemState & ODS_FOCUS))
	{
		...
		{
			pDC->FillSolidRect(rc,RGB(96,132,150));
	...
	(LPCTSTR lpszItem, int ix, COLORREF bg, COLORREF fg, UINT hal, UINT val, BOOL active, DWORD res)
{
	...
	if(m_pSuper->SetItemData(ix, (DWORD)plbi) == LB_ERR)
	{
...
void CListBoxEBX::OnLButtonDown(UINT nFlags, CPoint point)
{
