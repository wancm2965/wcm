...
void CListCtrlCl::PreSubclassWindow()
{
	...
	ModifyStyle(0,LVS_OWNERDRAWFIXED|LVS_REPORT);
...
void CListCtrlCl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for ( int nCol=0; GetColumn(nCol, &lvc); nCol++)
	{
		...
		{
			pDC->FillSolidRect(&rcTemp, RGB(96,132,150)/*GetSysColor(COLOR_HIGHLIGHT)*/) ;
...
void CListCtrlCl::OnPaint()
{
	...
	if( (GetStyle() & LVS_TYPEMASK) == LVS_REPORT )
	{
		...
		CPen newPen(PS_SOLID,1,RGB(63,100,116));
		CPen * pOldPen=dc.SelectObject (&newPen); 


		CHeaderCtrlCl* pHeader = (CHeaderCtrlCl*)GetDlgItem(0);
