...
public:
	virtual void OnNETEC_PinCallbackRecvdData(const char*pData,int32_t nLen)=0;
...
public:
	virtual int32_t Open(const char*cszPeerNodeID,uint32_t nPinID)=0;
	virtual int32_t Open(const char*cszPeerNodeID,uint32_t nPinID,const char*cszPeerMCUID,const char*cszPeerMCUIP,unsigned short nPeerMCUPort,const char*cszPeerMCUType)=0;
