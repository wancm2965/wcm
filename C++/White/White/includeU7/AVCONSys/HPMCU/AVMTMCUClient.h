...
public:
	virtual void OnIAVAudioReceiverNotifyReceivedAudioFrame(const char*pData,int32_t nLen)=0;
...
public:
	...
	virtual void GetPacketStatistics(uint32_t&ulTotalPackets,uint32_t&ulLostPackets,double&AvgPacketLossRate,double&CurPacketLossRate)=0;
};


NETEC_API IAVAudioReceiver*CreateIAVAudioReceiver(IAVAudioReceiverNotify&rIAVAudioReceiverNotify,uint32_t ulAudioChannelID,
												  const char*cszPeerMCUID,const char*cszPeerMCUIP,unsigned short usPeerMCUPort,const char*cszPeerMCUType,
												  const char*cszLocalMCUID,const char*cszLocalMCUIP,unsigned short usLocalMCUPort,const char*cszLocalMCUType,int32_t nForceUDP=0,bool bAudioCast=false);
...
NETEC_API IAVAudioSender*CreateIAVAudioSender(IAVAudioSenderNotify&rIAVAudioSenderNotify,uint32_t ulAudioChannelID,
...
public:
	virtual void OnIAVVideoReceiverNotifyReceivedVideoFrame(const char*pData,int32_t nLen)=0;
...
public:
	...
	virtual int32_t SendPTZToSender(void*pData,int32_t nLen)=0;
	virtual int32_t SendCtrlToSender(void*pData,int32_t nLen)=0;
	...
	virtual int32_t SetFrameRateControlCommand(unsigned char nCMD)=0;
	virtual int32_t SetFrameTypeControlCommand(unsigned char nCMD)=0;

	virtual int32_t SendDataToSender(void*pData,int32_t nLen)=0;
};


NETEC_API IAVVideoReceiver*CreateIAVVideoReceiver(IAVVideoReceiverNotify&rIAVVideoReceiverNotify,uint32_t ulVideoChannelID,
												  const char*cszPeerMCUID,const char*cszPeerMCUIP,unsigned short usPeerMCUPort,const char*cszPeerMCUType,
												  const char*cszLocalMCUID,const char*cszLocalMCUIP,unsigned short usLocalMCUPort,const char*cszLocalMCUType,int32_t nForceUDP=0,bool bVideoCast=false);
...
public:
	...
	virtual void OnIAVVideoSenderNotifyRecvdPTZ(void*pData,int32_t nLen){};
	virtual void OnIAVVideoSenderNotifyRecvdCtrl(void*pData,int32_t nLen){};
	virtual void OnIAVVideoSenderNotifyBitrateChanged(uint32_t nBitrate,uint32_t nFrameRate){};
};
...
public:
	...
	virtual void GetPacketStatistics(uint32_t&ulTotalPackets,uint32_t&ulLostPackets,double&AvgPacketLossRate,double&CurPacketLossRate)=0;
	virtual uint32_t GetBufferedTimeMS(int32_t nFrameType)=0;

	virtual void SetMaxBitrate(int32_t nMaxBitrate)=0;
};
NETEC_API IAVVideoSender*CreateIAVVideoSender(IAVVideoSenderNotify&rIAVVideoSenderNotify,uint32_t ulVideoChannelID,
												  const char*cszLocalMCUID,const char*cszLocalMCUIP,unsigned short usLocalMCUPort,const char*cszLocalMCUType);
...
NETEC_API IAVMTDataSessionClient*CreateIAVMTDataSessionClient(IAVMTDataSessionClientNotify&rNotify,uint32_t ulDataChannelID, 
															  const char*cszPeerMCUID,const char*cszPeerMCUIP,unsigned short usPeerMCUPort,const char*cszPeerMCUType,
															  const char*cszLocalMCUID,const char*cszLocalMCUIP,unsigned short usLocalMCUPort,const char*cszLocalMCUType);
