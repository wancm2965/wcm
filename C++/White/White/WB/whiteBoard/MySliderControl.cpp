...
{
	m_dcBk = NULL;
	m_crThumbColor = RGB(241, 255, 244);
	m_crThumbColorSelected = RGB(25, 11, 45);
	m_bRedraw = false;
	m_pBitmap = NULL;
}
...
void CMySliderControl::DrawThumb(CDC *pDC, LPNMCUSTOMDRAW lpcd)
{
    ...
    Rectangle(pDC->m_hDC, crect.left, crect.top, crect.right, crect.bottom);
...
BOOL CMySliderControl::OnEraseBkgnd(CDC* pDC)
{
	...
	SetSkinImage(strPath + _T("MediaCtrlBG.png"));
	return TRUE;
}
...
void CMySliderControl::SetSkinImage(CString strPath)
{
	if (!m_pBitmap)
	{
