...
public:
	...
	long get_TotalFrames()   //获得总帧数(frame_number+1) 
	{
		long result;
	...
	void TGotoFrame(LPCTSTR target, long FrameNum)  //   movie_clip跳转到指定帧数  
	{
	...
	CString GetVariable(LPCTSTR name)  //获取变量 
	{
		CString result;
		static BYTE parms[] = VTS_BSTR ;
	...
	CString TGetProperty(LPCTSTR target, long property)   // 获取的指定属性  
	{
		CString result;
		static BYTE parms[] = VTS_BSTR VTS_I4 ;
	...
	void TCallFrame(LPCTSTR target, long FrameNum)   // call指定帧上的action 
	{
		static BYTE parms[] = VTS_BSTR VTS_I4 ;
		InvokeHelper(0x9b, DISPATCH_METHOD, VT_EMPTY, NULL, parms, target, FrameNum);
	}
	void TCallLabel(LPCTSTR target, LPCTSTR label)  //call指定标签上的action  
	{
		static BYTE parms[] = VTS_BSTR VTS_BSTR ;
