...
CDxfFileWrite::CDxfFileWrite()
{
	...
	strResPath.Format(TEXT("%s\\CadIO.dll"), strPath);

	hinstCadIO = LoadLibrary(strResPath.GetBuffer());
...
BOOL CDxfFileWrite::Create(LPCTSTR filename, bool overwrite )
{
	ISLIBRARYLOADED();
...
CDrawing::CDrawing()
{
	...
	result &= ((drwSaveDataToFile = (BOOL(*)(HDRAWING, DWORD, const char *, HWND))
		GetProcAddress( hinstCadIO, "drwSaveDataToFile" )) != NULL);
	result &= ((drwLoadDataFromFile = (BOOL(*)(HDRAWING, OBJHANDLE, DWORD, const char *, HWND))
	...
	result &= ((drwPaint = (BOOL(*)(HDRAWING, HDC,COLORREF))
...
BOOL CDrawing::Paint( HDC hdc , COLORREF BKColor)
{
	ISLIBRARYLOADED();
	return drwPaint(m_hDrawing, hdc, BKColor);
}
...
BOOL CDrawing::SetZoomLevel(double ZoomLevel)
{
	...
	if(drwGetViewProperties(m_hDrawing, &drwview))
	{
		...
		drwview.ViewRight = cx + ((drwview.WindowRight-drwview.WindowLeft)/2)*(1/(drwview.PPU*drwview.ZoomLevel));
		drwview.ViewTop = cy + ((drwview.WindowBottom-drwview.WindowTop)/2)*(1/(drwview.PPU*drwview.ZoomLevel));
...
OBJHANDLE CDrawing::AddBlock( PBLOCKHEADER pBlockHeader, LPCTSTR strFileName )
{
	...
	{
		string FileName = ToStr(strFileName);
		drwLoadDataFromFile(m_hDrawing, result, 0, FileName.c_str(), NULL);
	}
	return result;
}
...
BOOL CDrawing::GetBoundsRealRect(PREALRECT rect)
{
	ISLIBRARYLOADED();
	return drwGetDrawingBorder(m_hDrawing,rect);
}
...
OBJHANDLE CDrawing::LWPolyLine( PENTVERTEX pVertex, int nVertex, unsigned short Flag )
{
	ENTLWPOLYLINE polyline;
	polyline.pVertex = pVertex;
	polyline.nVertex = nVertex;
	polyline.Flag = Flag;
	m_EntityHeader.EntityType = ENT_LWPOLYLINE;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &polyline);
}

OBJHANDLE CDrawing::SPline( SimpleList<REALPOINT> *pVertex, int nVertex, unsigned short Flag )
{
	ENTSPLINE spline;
	spline.pControlNode = pVertex;
	spline.ControlNodeNum = nVertex;

	m_EntityHeader.EntityType = ENT_SPLINE;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &spline);
}

OBJHANDLE CDrawing::Hatch( SimpleList<ENTHATCHPATHLINEDATA> *pHatchPathLines, int nVertex, unsigned short Flag )
{
	ENTHATCH hatch;
	((PENTHATCHPATHDATA)hatch.HatchPathData)->pHatchPathLineData=pHatchPathLines;

	m_EntityHeader.EntityType = ENT_HATCH;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &hatch);
}

OBJHANDLE CDrawing::MLine( SimpleList<MLINEVETEX> *pVertex, int nVertex, unsigned short Flag )
{
	ENTMLINE mline;
	mline.pVetexPoint=pVertex;
	mline.VetexNum=nVertex;

	m_EntityHeader.EntityType = ENT_MLINE;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &mline);
}
...
{
	ISLIBRARYLOADED();
	string strFileName = ToStr(FileName);
	return drwSaveDataToFile(m_hDrawing, 0, strFileName.c_str(), hWndProgress);
}
...
BOOL CDrawing::LoadDXFFile( LPCTSTR FileName, HWND hWndProgress )
{
	ISLIBRARYLOADED();
	string strFileName = ToStr(FileName);
	return drwLoadDataFromFile(m_hDrawing, NULL, 0, strFileName.c_str(), hWndProgress);
}
