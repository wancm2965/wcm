...
public:
	...
	BOOL Paint( HDC hdc , COLORREF BKColor =0);
	double GetZoomLevel( );
	...
	OBJHANDLE LWPolyLine( PENTVERTEX pVertex, int nVertex, unsigned short Flag=0 );
	OBJHANDLE SPline( SimpleList<REALPOINT> *pVertex, int nVertex, unsigned short Flag=0 );
	OBJHANDLE Hatch(SimpleList<ENTHATCHPATHLINEDATA> *pVertex, int nVertex, unsigned short Flag=0);
	OBJHANDLE MLine( SimpleList<MLINEVETEX> *pVertex, int nVertex, unsigned short Flag=0 );
	BOOL GetBoundsRealRect(PREALRECT rect);
...
private:
	...
	BOOL (*drwPaint)( HDRAWING hDrawing, HDC hdc,COLORREF color );
